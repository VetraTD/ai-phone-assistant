...
app.get("/api/calendar/auth-url", authenticate, async (req, res) => {
  try {
...
app.get("/api/calendar/status", authenticate, async (req, res) => {
  try {
    const businessId = await getBusinessIdForUser(req.authUser.id);
    if (!businessId) {
      return res.status(403).json({ error: "No business linked to this user" });
...
app.post("/api/calendar/sync", authSensitiveLimiter, authenticate, async (req, res) => {
  try {
    const businessId = await getBusinessIdForUser(req.authUser.id);
    if (!businessId) {
      return res.status(403).json({ error: "No business linked to this user" });
...
app.delete("/api/calendar/disconnect", authSensitiveLimiter, authenticate, async (req, res) => {
  try {
    const businessId = await getBusinessIdForUser(req.authUser.id);
    if (!businessId) {
      return res.status(403).json({ error: "No business linked to this user" });
...
// above: resolve the authenticated user's business_id via
// getBusinessIdForUser, then require every row touched to belong to it.
// ---------------------------------------------------------------------------
...
app.post("/api/knowledge", authSensitiveLimiter, authenticate, async (req, res) => {
  try {
    const userBusinessId = await getBusinessIdForUser(req.authUser.id);
    if (!userBusinessId) {
      return res.status(403).json({ error: "No business linked to this user" });
...
app.put("/api/knowledge/:id", authSensitiveLimiter, authenticate, async (req, res) => {
  try {
    const userBusinessId = await getBusinessIdForUser(req.authUser.id);
    if (!userBusinessId) {
      return res.status(403).json({ error: "No business linked to this user" });
...
app.delete("/api/knowledge/:id", authSensitiveLimiter, authenticate, async (req, res) => {
  try {
    const userBusinessId = await getBusinessIdForUser(req.authUser.id);
    if (!userBusinessId) {
      return res.status(403).json({ error: "No business linked to this user" });
