...
/** A complete marker anywhere in a string. */
const ANY_MARKER_RE = /[*`_~]*<<\s*intent\s*:\s*[a-zA-Z0-9_]{0,40}\s*>>[*`_~]*/gi;
...
export function createMarkerStripper({ allowedIntents = [] } = {}) {
  ...
  return {
    push(delta) {
      if (resolved) return { text: takeSeparator(delta), intent: null, rejected: null };
      buf += delta;
