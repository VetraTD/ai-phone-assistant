/** @type {Map<string, { history: Array<{ role: string, parts: Array<{ text: string }> }>, hasGreeted: boolean, lastProcessed?: { speechHash: string, timestamp: number, cachedTwiml?: string } }>} */
const stateByCallSid = new Map();

/**
 * Get or create conversation state for a call.
 * @param {string} callSid - Twilio Call SID
 * @returns {{ history: Array, hasGreeted: boolean, lastProcessed?: object }} Mutable state object
 */
export function getState(callSid) {
  let state = stateByCallSid.get(callSid);
  if (!state) {
    state = { history: [], hasGreeted: false };
