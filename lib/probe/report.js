...
export function buildVerdict({ serverStats, probeP50 = null } = {}) {
  ...
  if (cacheBroken) {
    // Once explicit caching is switched on, a 0% hit rate stops being expected
    // and becomes a bug — most likely cachedContent being dropped from the
    // per-request config, which silently re-bills the whole prefix at full price
    // (the SDK REPLACES rather than merges per-request config; see the comment
    // in services/gemini.js). promptCache.live is what separates "never created"
    // from "created and ignored"; the per-turn hit rate cannot.
    const explicitLive = (serverStats?.promptCache?.live ?? 0) > 0;
    notes.push(
      explicitLive
        ? `Prompt cache hit rate is ${cache.hitRatePctP50}% across ${cache.samples} turns, but ` +
            `${serverStats.promptCache.live} explicit cache(s) are LIVE. That is a BUG, not the ` +
            "expected baseline: the cache exists and is not being applied. Check that " +
            "cachedContent survives into the per-request config."
        : `Prompt cache hit rate is ${cache.hitRatePctP50}% across ${cache.samples} turns. This is ` +
            "EXPECTED while GEMINI_EXPLICIT_CACHE is off: implicit caching does not engage on " +
...
export function buildReport({ runId, callCount, probeTurns = [], serverStats = {}, startBootId } = {}) {
  ...
  const cache = serverStats.cache ?? {};
  const tt = serverStats.turnTaking ?? {};
  const markerLeaks = tt.intent_marker_leaks ?? 0;
  const fallbackTurns = tt.tts_fallback_turns ?? 0;

  // Regression gate for the four defects fixed on 2026-08-04.
  //
  // A report that only prints timings cannot answer "is the bug fixed", which
  // is the question a run is placed to settle. Each row below is a counter that
  // must be 0, with the failure it stands for named — so a non-zero value is
  // actionable on sight instead of needing someone to remember what it meant.
  const GATES = [
    {
      metric: "internal_term_leaks",
      value: tt.internal_term_leaks ?? 0,
      means: "the assistant tried to say API / webhook / a UUID / an HTTP code to a caller",
    },
    {
      metric: "loop_breaker_trips",
      value: tt.loop_breaker_trips ?? 0,
      means: "a barge-in start/stop loop got through the echo guard and settle window",
    },
    {
      metric: "silence_hangups",
      value: tt.silence_hangups ?? 0,
      means: "a call died on the silence ladder rather than ending on a goodbye",
    },
    {
      metric: "intent_marker_leaks",
      value: markerLeaks,
      means: "the stream-level marker strip missed one",
    },
    {
      metric: "text_channel_tool_calls",
      value: tt.text_channel_tool_calls ?? 0,
      means: "the model wrote a function call as text, so nothing ran and the caller heard it",
    },
    {
      metric: "promise_only_turns",
      value: tt.promise_only_turns ?? 0,
      means: "the assistant said it was checking something and then called no tool at all",
    },
  ];
  const failedGates = GATES.filter((g) => g.value > 0);
  const gateRows = GATES.map(
    (g) => `| \`${g.metric}\` | ${g.value} | ${g.value === 0 ? "ok" : "**FAIL**"} | ${g.means} |`
  ).join("\n");
  const gateWarning = failedGates.length
    ? `\n> **${failedGates.length} regression gate(s) tripped** — see "Regression gates" below.\n` +
      failedGates.map((g) => `> \`${g.metric}\` = ${g.value}: ${g.means}.\n`).join("")
    : "";
...
${restartWarning}${voiceWarning}${gateWarning}

## Regression gates

Every counter here must be 0. They are the caller-visible failures this run
exists to rule out; the latency numbers below say nothing about any of them.

| metric | value | verdict | a non-zero value means |
|---|---|---|---|
${gateRows}
