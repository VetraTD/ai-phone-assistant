...
/** Cache hit rates at or under this mean the stable prefix isn't working. */
...
const STAGE_GUIDANCE = {
  ...
  llm_ttfb_ms: {
    ...
    recommendation:
      "The Groq comparison is now worth running — but confirm the prompt cache is hitting first, " +
      "or the benchmark measures a broken prefix on both candidates and picks a winner for the " +
      "wrong reason.",
  },
  tts_ttfb_ms: {
...
export function buildVerdict({ serverStats, probeP50 = null } = {}) {
  ...
  if (cacheBroken) {
    notes.push(
      `Prompt cache hit rate is ${cache.hitRatePctP50}% across ${cache.samples} turns — the ` +
        "stable prefix in services/gemini.js is not being reused, so every turn re-processes it."
    );
  }
 ...
 */
export function buildReport({ runId, callCount, probeTurns = [], serverStats = {} } = {}) {
