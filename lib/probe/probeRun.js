 ...
 * @param {number} [opts.silenceMs=2000] - unvoiced time that counts as "the assistant stopped"
...
export function createProbeRun({
  ...
  now = () => performance.now(),
  replyTimeoutMs = 8000,
  // How much unvoiced audio counts as "the assistant has finished".
  //
  // Was 800ms, which is shorter than the gaps that occur INSIDE a real reply
  // (between streamed sentences, or while TTS catches up). The probe therefore
  // mistook a mid-reply pause for the end of the turn and started talking,
  // causing a genuine barge-in: 30 barge-ins recorded across a 12-call run
  // that scripted only 12. Every turn after an accidental interruption was
  // measured against the wrong boundary, producing impossible values (8ms
  // replies, negative ones) and phantom timeouts that read as the assistant
  // never answering.
  //
  // 2000ms is above the observed intra-reply gaps and matches llmTurn's own
  // stall threshold — past that the assistant considers itself stuck too. It
  // costs wall-clock per call but does not touch the measured quantity, which
  // is speech-end to first reply audio.
  silenceMs = 2000,
  gapMs = 400,
  vad = createVad(),
} = {}) {
  ...
  function handleInbound(mulawBuf) {
    ...
    try {
      ...
      //
      // This previously carried a `state !== STATE.GAP` guard, which defeated
      // it for the only case that matters. Entering the gap from LISTENING sets
      // gapStartAt and state = GAP in the same step, so the cancel could only
      // ever fire during the greeting; on a reply the countdown ran to
      // completion no matter how much the assistant kept talking. That is the
      // ~30 barge-ins against 12 scripted seen in every run to date, and each
      // one desyncs the turns that follow it.
      if (gapStartAt !== null) {
        gapStartAt = null;
        if (state === STATE.GAP) state = STATE.LISTENING;
      }
