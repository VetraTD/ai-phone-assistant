...
/**
 * @typedef {{afterLabel: string, onCounter: string, delayMs: number,
 *            armAfterLabel: string, pollWindowMs: number}} HangupPlan
 */

/** @returns {HangupPlan} */
export function makeHangupPlan({
  afterLabel = "",
  onCounter = "",
  delayMs = 0,
  armAfterLabel = "",
  ...
  const w = Number(pollWindowMs);
  return {
    afterLabel: String(afterLabel || ""),
    onCounter: String(onCounter || ""),
    delayMs: Number.isFinite(d) && d > 0 ? d : 0,
    armAfterLabel: String(armAfterLabel || ""),
    pollWindowMs: Number.isFinite(w) && w > 0 ? w : 12000,
  };
}
...
/**
 * May the counter poll run yet?
 *
 * THE ENDPOINT IS RATE LIMITED, and this cost two calls to learn. `/api` allows
 * 60 requests per 60 seconds (server.js), and a 150 ms poll is 400 a minute.
 * Measured 2026-09-12: the budget was gone nine seconds into a hundred-second
 * call, every later read came back 429, and `readCounters` returned null for
 * that exactly as it does for a bad token -- so the report said "did not answer
 * with that token", which was true of the first run and false of the second.
 *
 * So the poll is ARMED BY A SCRIPTED LINE rather than running all call. Arm it
 * on the turn you are waiting for -- for the LVX121 window that is
 * `demo_accept`, the caller's affirmative -- and a few seconds of fast polling
 * costs twenty requests instead of four hundred.
 *
 * With no arming line it polls from the start, which is right for a short run
 * and will exhaust the budget on a long one.
 *
 * AND IT DISARMS AGAIN. Arming alone was not enough, measured on a third call:
 * the poll started at the right moment and then kept running for the ~30
 * seconds of call that followed, which is another 120 requests at 250 ms. The
 * window being waited for is a few seconds wide, so the poll is bounded to
 * `pollWindowMs` after the arming line. Past that the counter is not going to
 * rise for the reason this run cares about, and spending the budget to find
 * that out costs the NEXT run its counters too.
 */
export function pollArmed(plan, armEndedAt, now = null) {
  if (!plan?.onCounter) return false;
  if (!plan.armAfterLabel) return true;
  // `!= null` and not a truthiness test: 0 is a legitimate millisecond value.
  if (armEndedAt == null) return false;
  if (now != null && now > armEndedAt + plan.pollWindowMs) return false;
  return true;
}
