...
export function buildDialPlan({
  ...
  // Token in the PATH, not a query string. Twilio does not carry a
  // <Stream url="..."> query string into the websocket handshake, so a
  // ?token= form arrives at the server empty, the upgrade is refused, and the
  // call dies in under a second with a 31920 that reads as a broken endpoint.
  const wsBase = baseUrl.replace(/^https/, "wss").replace(/\/$/, "");
  const streamUrl = `${wsBase}/twilio/probe-stream/${encodeURIComponent(debugToken)}`;
