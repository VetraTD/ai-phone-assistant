...
import { createTurnMetrics } from "./voice/metrics.js";
import { dispatchCapabilityEffects, mergeCapabilityState } from "./capabilities/effects.js";

// ---------------------------------------------------------------------------
// PARITY NOTE: this legacy v1 pipeline is a rollback escape hatch
// (PIPELINE_V2=false) and is intentionally NOT kept at feature parity with
// lib/voice/session.js. Only two safety fixes were ported from the 2026-07
// voice-fix work: the narrowed TRANSFER_TRIGGERS regex and the <Dial
// ringTone> ringback. A rollback restores pre-fix behavior for everything
// else (turn-taking, booking idempotency, locale voices, Spanish strings).
// ---------------------------------------------------------------------------
...
// person or a robot?" must NOT match (kept in sync with lib/voice/session.js).
...
/**
 * The actual Twilio REST redial for a transfer — the reusable "routine"
 * both the TRANSFER_TRIGGERS regex fast-path and the request_transfer tool
 * path call once they've decided a transfer should happen (Media Streams
 * doesn't support <Dial> directly, so this redirects the live call via the
 * REST API instead).
 * @param {string} callSid
 * @param {string} requestId
 * @param {string} transferNumber
 * @returns {Promise<boolean>} true if the redial call succeeded
 */
async function redialForTransfer(callSid, requestId, transferNumber) {
  try {
    const twilio = (await import("twilio")).default;
    const client = twilio(process.env.TWILIO_ACCOUNT_SID, process.env.TWILIO_AUTH_TOKEN);
    // ringTone forces Twilio-generated ringback so the caller hears ringing
    // even when the downstream carrier supplies no early media.
    await client.calls(callSid).update({
      twiml: `<Response><Dial ringTone="us">${escapeXml(transferNumber)}</Dial></Response>`,
    });
    log.info("transfer_outcome", { callSid, requestId, success: true, reason: "ok" });
    db.markCallTransferred(callSid).catch((err) => {
      log.error("mark_call_transferred_failed", { callSid, requestId, message: err?.message });
    });
    return true;
  } catch (err) {
...
function streamAudioToTwilio(state, mulawBuffer, markName) {
  ...
  // Latency instrumentation: first Twilio media frame of the turn. Guarded to
  // real turns only (see speakSentence) via state.processingTurn. The
  // `mulawBuffer.length > 0` check is not what excludes the inter-sentence
  // silence padding (that buffer is non-empty) — silence padding is safe
  // here because it's only ever sent before the 2nd+ sentence
  // (`addPause = sentenceIndex > 1`), i.e. always after sentence 1's real
  // audio has already set this mark; the length check just guards against a
  // stray zero-length buffer.
  if (state.processingTurn && mulawBuffer.length > 0) {
...
export async function handleMediaStreamConnection(ws, req) {
  ...
  async function speakSentence(text, markName) {
    ...
    try {
      ...
      // Latency instrumentation: first sentence of the turn to finish TTS.
      // Guarded to real turns only (state.processingTurn) so greeting /
      // silence-nudge speech — which aren't part of a caller-triggered turn —
      // don't pollute the next turn's marks (mark() ignores repeats by name).
      if (state.processingTurn) state.turnMetrics?.mark("tts_first_byte");
  ...
  function onBargeIn() {
    ...
    // Latency instrumentation: the interrupted turn ends here — its own
    // `turn-*-done` mark will never arrive since the audio was cleared.
  ...
  async function onTranscript(text) {
    ...
    log.debug("turn_start", {
      ...
      step: state.step,
      intent: state.intent,
      // Presence, not values. What a turn trace needs to answer is "had we
      // collected a name by now", and the name itself answers that no better.
      slots: {
        hasName: state.collectedName != null,
        hasPhone: state.collectedPhone != null,
    ...
    if (TRANSFER_TRIGGERS.test(text)) {
      ...
      if (canTransfer) {
        ...
        await redialForTransfer(callSid, requestId, transferNumber);
        return;
      }
    ...
    // Latency instrumentation: this Deepgram final transcript triggers the
    // turn. STT-final == what triggers the turn for now (Phase 0); Deepgram
    // audio-timestamp precision is out of scope for Phase 0.
    state.turnMetrics?.mark("speech_end");
    state.turnMetrics?.mark("stt_final");
    ...
    try {
      ...
      state.turnMetrics?.mark("llm_request");
      ...
      for await (const chunk of geminiService.getReplyStreaming(
        ...
        if (chunk.delta) {
          if (firstChunk) {
            ...
            state.turnMetrics?.mark("llm_first_chunk");
            firstChunk = false;
          }
      ...
      const { text: replyText, intentArgs, endCallArgs, toolResults, transferRequested } = reply;
      const turnLatencyMs = Date.now() - geminiStart;
      // Did the model actually say anything of its own this turn? Used below
      // to avoid re-speaking a redundant "Transferring you now" line when the
      // model's own reply already announced the transfer.
      const producedText = !firstChunk;
      ...
      // Capabilities own what is worth remembering; this pipeline only merges it.
      mergeCapabilityState(state, reply.capabilityState);
      ...
      // Capability-declared effects: step transitions, notifications, and the
      // history notes that stop the model redoing an action it already did.
      const capabilityNotes = dispatchCapabilityEffects(reply.capabilityEffects, {
        STEPS,
        setStep(nextStep, trigger) {
          state.step = nextStep;
          log.info("step_transition", { callSid, requestId, toStep: nextStep, trigger });
        },
        setCapabilityState: (patch) => mergeCapabilityState(state, patch),
        call: {
          callSid,
      ...
      // request_transfer tool call (language-agnostic fallback to the
      // TRANSFER_TRIGGERS regex escape-hatch above). Reuses redialForTransfer
      // — the same Twilio-redial routine the regex path calls — so a
      // non-English "let me speak to a person" actually transfers on this
      // pipeline too, not just the English keyword fast-path.
      if (transferRequested) {
        const transferNumber = config.transferPhoneNumber || TRANSFER_NUMBER;
        const canTransfer = !!transferNumber && resolveTransferAllowed(config);
        log.info("transfer_requested", { callSid, requestId, canTransfer, source: "tool" });
        if (canTransfer) {
          // Only speak the hardcoded announcement if the model didn't already
          // say something of its own this turn (avoid double-speaking).
          if (!producedText) {
            await speakFull("Transferring you now. Please hold.");
          }
          state.step = STEPS.ENDING;
          await redialForTransfer(callSid, requestId, transferNumber);
        } else {
          // ctx.transferAllowed (policy) was true when the tool ran, but no
          // transferNumber is actually configured — tell the caller rather
          // than silently failing to redial after the model promised a transfer.
          await speakFull("I'm sorry, I'm unable to transfer you at this time. Let me try to help you directly.");
        }
      }
      ...
      state.aiSpeaking = false;
      // Latency instrumentation: this turn errored out before reaching its
      // `turn-*-done` mark (or barge-in), so it would never otherwise call
      // finishTurn() — leaving its marks to leak into the next turn's deltas.
      // Close it out here (partial marks still yield useful pre-error timing).
      ...
      // Mirror the other speakFull call sites (hard-time-limit, transfer):
      // clear processingTurn before speaking so this error message's own
      // audio isn't mistaken for the next real turn's marks (see the
      // state.processingTurn guards in speakSentence/streamAudioToTwilio).
      // finally below still sets this too, which is harmless/idempotent.
      state.processingTurn = false;
    ...
    switch (msg.event) {
      ...
      case "start": {
          ...
          if (db.isEnabled()) {
            ...
            if (business) {
              ...
              if (dbId) {
                ...
                // See lib/voice/session.js — the same call-boundary write, so
                // the status callback works on a different instance.
        ...
        if (isDefaultGreeting) {
          // Mirrors lib/voice/session.js buildGreeting's fix: the default
          // greeting text is generic ("Hi, how can I help you today?"), so
          // prepending a time-of-day word produced a double greeting with no
          ...
          greetingText += `${tod}, thanks for calling ${businessName}. How can I help you today?`;
        } else {
      ...
      case "mark": {
        ...
        if (markName.endsWith("-done")) {
          if (state) {
            // Latency instrumentation: the turn's own completion mark (set
            // where the final `turn-${turnId}-done` mark is sent below, as
            // opposed to the `speak-*-done` / `greeting-done` marks used by
            // greeting and silence-nudge speech, which aren't part of a
            // caller-triggered turn) — this means the turn's audio has
            // finished playing to the caller.
