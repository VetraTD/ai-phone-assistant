...
export async function handleMediaStreamConnection(ws, req) {
  ...
  async function onTranscript(text) {
    ...
    try {
      ...
      const { text: replyText, appointmentArgs, intentArgs, endCallArgs, customerRequestArgs, toolResults, selectedAppointmentId, transferRequested } = reply;
      ...
      if (appointmentArgs && state.businessId) {
        ...
        notifications.sendCallerSms(config, state.callerNumber, "appointment_confirmation", {
        ...
        }).catch((err) => log.error("sms_followup_failed", { callSid, requestId, kind: "appointment_confirmation", reason: err?.message }));
      }
      ...
      if (customerRequestArgs && state.businessId) {
        db.createCustomerRequest({
          ...
          if (id) {
            ...
            notifications.sendCallerSms(config, state.callerNumber, "message_received", {
              name_part: customerRequestArgs.caller_name ? ` ${customerRequestArgs.caller_name}` : "",
              business: config.businessName,
              sla: notifications.MESSAGE_SLA_TEXT,
            }).catch((err) => log.error("sms_followup_failed", { callSid, requestId, kind: "message_received", reason: err?.message }));
          }
