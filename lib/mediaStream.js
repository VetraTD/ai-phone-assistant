...
export async function handleMediaStreamConnection(ws, req) {
  ...
  async function onTranscript(text) {
    ...
    try {
      ...
      // Note: reply.transferRequested (the request_transfer tool's stateEffect)
      // is intentionally left unhandled here. mediaStream.js is the legacy
      // pipeline (superseded by lib/voice/session.js under PIPELINE_V2) and
      // has no single "apply state effects" chokepoint to wire it into
      // cleanly — see this file's TRANSFER_TRIGGERS regex block above for the
      // only transfer path it supports (English keyword escape-hatch, which
      // still works unchanged). A non-English "let me speak to someone" on
      // this legacy path will still get a spoken response from the model
      // (whose text asks the caller to hold for a transfer), it just won't
      // actually redial — acceptable for a path being phased out.
