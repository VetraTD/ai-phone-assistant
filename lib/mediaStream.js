...
export async function handleMediaStreamConnection(ws, req) {
  ...
  async function onSilence() {
    if (!state || state.aiSpeaking || state.processingTurn) {
      // AI is still talking or thinking — not real caller silence. Don't
      // advance silenceStage; just reschedule so we check again later.
      clearTimeout(silenceTimer);
      silenceTimer = setTimeout(() => onSilence(), SILENCE_PROMPT_MS);
      return;
    }
    ...
    switch (msg.event) {
      ...
      case "mark": {
        ...
        if (markName.endsWith("-done")) {
          if (state) {
            state.aiSpeaking = false;
            // Restart the silence clock from *now* (10 s of real post-AI silence).
            // Do NOT call resetSilenceTimer() here — that resets silenceStage to 0,
            // which breaks the "Are you still there?" → hangup escalation flow.
            clearTimeout(silenceTimer);
            silenceTimer = setTimeout(() => onSilence(), SILENCE_PROMPT_MS);
