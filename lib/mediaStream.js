...
export async function handleMediaStreamConnection(ws, req) {
  ...
  // Transcript debounce — merges Deepgram speech_final fragments that arrive
  // close together (Deepgram fires speech_final on phrase boundaries, not just
  // end-of-turn, so a user can pause mid-sentence and trigger a premature response).
  ...
  async function onSilence() {
    if (!state || state.aiSpeaking) {
      // AI is still talking — this isn't real silence. Reset the stage counter
      // so the 10 s clock starts fresh once the AI finishes speaking.
      silenceStage = 0;
      silenceTimer = setTimeout(() => onSilence(), SILENCE_PROMPT_MS);
      return;
    }
  ...
  function enqueueTranscript(text) {
    ...
    const delay = isComplete ? 100 : 400;
  ...
  async function onTranscript(text) {
    ...
    // Barge-in: if AI is speaking and user speaks, interrupt.
    // Disabled via BARGE_IN_ENABLED flag while reliability issues are being fixed.
    ...
    switch (msg.event) {
      ...
      case "mark": {
        ...
        if (markName.endsWith("-done")) {
          // All audio for this turn has finished playing.
          // Reset the silence clock from *now* so "Are you still there?" only
          // fires after 10 s of real post-AI silence, not 10 s from user's last word.
          if (state) {
            state.aiSpeaking = false;
            resetSilenceTimer();
          }
