...
function namesTime(text, p) {
  ...
  // BARE HOUR, NO MERIDIEM -- and the lookahead is the whole point.
  //
  // Found by running the fixtures: without it, "at 1 AM" matched the 1 PM slot.
  // The bare branch fired because nothing consumed the "AM", so a sentence that
  // explicitly CONTRADICTS the slot read as one that merely omitted the
  // meridiem. Those are opposites, and the wrong one books an appointment
  // twelve hours from what the caller heard.
  const noMerid = `(?!\\s*(?:o'?clock\\s*)?(?:[ap]\\.?\\s?m\\.?|${PM_PHRASES}|${AM_PHRASES}))`;
