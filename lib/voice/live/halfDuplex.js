...
export function createHalfDuplexGate({ lookbackMs = 500, bargeMs = DEFAULT_BARGE_MS } = {}) {
   ...
   * @returns {{forward: Buffer[], barge: boolean}}
   */
  function push({ frame, playing, isActive, voicedRunMs }) {
    if (!playing) {
      ...
      lookback.length = 0;
      barged = false;
      return { forward: frame ? [frame] : [], barge: false };
    }
    ...
    if (interrupting && !barged) {
      ...
      return { forward: frame ? [...held, frame] : held, barge: true };
    }
    ...
    if (frame) {
      lookback.push(frame);
      while (lookback.length > maxFrames) lookback.shift();
    }
    return { forward: [], barge: false };
  }
