...
export function selectStrategy(env = process.env) {
  ...
  return createVendorAd({
    ...
    // Unset means the vendor's own default, which is what every call before
    // 2026-09-16 ran. 0 is treated as unset for the same reason: a variable
    // cleared to "0" must not be read as "end the turn instantly".
    silenceDurationMs: intFromEnv(env.LIVE_VAD_SILENCE_MS, 0) || undefined,
  });
}
