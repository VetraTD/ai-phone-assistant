...
export function createCallSummary({ arm, model = null, surface = null, languagePinned = null }) {
  ...
  // absence.
  // `no_ask` is LVX132's, added 2026-09-17: the hang-up refused because this
  // caller was never asked whether they needed anything else.
  //
  // IT IS NO LONGER CAPPED AT ONE PER CALL. LVX157 scoped that latch to a
  // completed ACTION rather than to the call, because CA84dc64 spent it at
  // 20:56:22 on a cancel the caller abandoned and the sign-off after the
  // booking two minutes later went through ungated. So a 2 here now means two
  // separate pieces of work, not a broken latch -- read it against
  // `closing.ask_rearmed_by_action`, which counts exactly the re-arms.
  const endCallRefusals = { hesitation: 0, generic: 0, no_ask: 0 };
  ...
  return {
    ...
    recordEndCallRefusal: (kind) => {
      if (kind === "hesitation" || kind === "generic" || kind === "no_ask") endCallRefusals[kind] += 1;
    },
    recordClose: (reason) => {
    ...
    build() {
      return {
        ...
        // Output voice only; the API has no input-language control. See
        // lib/voice/live/client.js.
        output_language_pinned: pinned,
        turns,
