...
const AVAILABILITY_SHAPES = {
  check_appointment_availability: {
    /** Booking tools whose slot this check is allowed to authorise. */
...
export function createToolGuards({ declarations = [], config = {} } = {}) {
  ...
  function before(fc) {
    ...
    if (isWriteTool(fc.name) && completedWrites.has(callKey(fc))) {
    ...
    if (key && verifiedSlots.has(key)) return { allow: true };
  ...
  function after(fc, result) {
    ...
    if (isWriteTool(fc.name) && response && response.success !== false) {
...
/** Booking-shaped tool names, for the unarmed counter only — never for gating. */
function looksLikeBooking(name) {
  return /^(book|schedule)_/.test(name);
}
