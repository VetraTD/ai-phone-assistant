...
/**
 * Reads that may be answered from a memo WITHIN one turn.
 *
 * AN ALLOWLIST, and the obvious alternative is wrong. `!isWriteTool(name)` looks
 * like the right predicate -- it is the complement of the rule directly above --
 * but `isWriteTool` means "in its pack's actionTools", and two packs declare
 * `actionTools: []`: capabilities/transfer.js:53 and capabilities/smsConsent.js:95.
 * So `request_transfer` and `record_sms_consent` both answer FALSE to
 * `isWriteTool` and would be memoed.
 *
 * `request_transfer` is not a read. Its entire value is
 * `stateEffects.transferRequested`, and a memoed call returns `{allow: false}`,
 * which short-circuits before `stateEffects` is read at all. That is verbatim
 * the end_call failure written up at the top of this file: the engine never
 * learns what the model asked for, and nothing can act on it.
 *
 * So the members are named. Every one of them answers a question about a diary
 * or a record, and answers it the same way twice inside one turn.
 */
 ...
 */
export function createToolGuards({ declarations = [], config = {}, callSid = null } = {}) {
  ...
  /**
   * Successful READ responses for THIS TURN, keyed by name + canonical args.
   *
   * ---------------------------------------------------------------------------
   * Scoped to a turn, deliberately, and the other two scopes are both wrong
   * ---------------------------------------------------------------------------
   *
   * Per CALL would answer a later question from a stale diary. Availability
   * genuinely changes while a caller is on the line -- somebody else books --
   * and "let me check that again" is a legitimate thing for a receptionist to
   * do on the next turn.
   *
   * Not at all is what we had, and it was measured on two real calls on
   * 2026-09-16, one per model, with the tool returning in 4-101 ms every time:
   * `check_appointment_availability` three times in one call on gemini-3.8-live
   * (38 slots listed, no booking, the caller hung up) and five times on
   * gemini-3.1 (73 slots). So this is not a 3.8 defect that 3.1 lacks, which is
   * how it was first filed. Both re-fire.
   *
   * Within one turn the answer cannot meaningfully change, and that is exactly
   * the window where the repeats happen. Two day queries inside one turn can
   * also return two DIFFERENT lists, which lets the model offer a time its own
   * later lookup no longer shows.
   *
   * Reassigned rather than mutated by resetTurn(), so a stale reference cannot
   * be held by anything.
   */
  ...
  const counts = {
    ...
    // Declared here at zero rather than created on first hit. counts() is spread
    // straight into live_call_summary.guards, so a key that appears only on a
    // call where the memo fired would be readable only as a fault -- and a
    // fault-only counter reads zero for a clean run and for a run that never got
    // there, which is the complaint written twenty lines above this one.
    read_memo_hit: 0,
  };
  ...
  function before(fc) {
    ...
    // A read this turn has already answered. Handed back rather than refused:
    // the model is waiting on a response to THIS call id, and a Live session
    // left holding an unanswered tool call does not error, it waits, and the
    // caller hears silence.
    if (MEMOABLE_READS.has(fc.name) && readMemo.has(callKey(fc))) {
      counts.read_memo_hit += 1;
      bumpCounter("live_guard_read_memo_hit");
      log.info("live_guard_read_memo_hit", { tool: fc.name });
      return {
        allow: false,
        reason: "read_memo",
        functionResponse: { ...readMemo.get(callKey(fc)), id: fc.id },
      };
    }
  ...
  function after(fc, result) {
    ...
    // -----------------------------------------------------------------------
    // WHAT TIME DID THIS WRITE TARGET, AND ON WHAT EVIDENCE.
    //
    // No log event has ever recorded a write's target, so no change to this path
    // could be validated against history -- the complaint that opened this work.
    // The obvious fix is to log the datetime, and that is refused here:
    // lib/postCallVerify.js states that "the caller's name, number and
    // appointment time are all in scope and none of them belong in a log line",
    // and weekday + local time + days-ahead against this line's own timestamp
    // reconstructs the instant exactly while passing the PHI lint only by
    // evading its name matcher. That is the LVX24 move.
    //
    // So the question is answered RELATIONALLY instead, which is the form every
    // actual investigation needs:
    //
    //   slot_point_verified  a real check confirmed THIS time, for the time
    //                        actually asked about.
    //   slot_listed_only     the time was only ever one of several a day query
    //                        or an alternatives list put on the table. The write
    //                        landed on something the caller was shown, not on
    //                        something they were asked about -- the shape in
    //                        which a wrong time reaches the database while every
    //                        guard passes.
    //   slot_unverified      no availability response ever carried it. The
    //                        booking invariant refuses this, and a non-zero count
    //                        means the invariant is the only thing that stopped it.
    //
    // Counts and booleans, no datetime, no name, no phone.
    // -----------------------------------------------------------------------
    const targetArg = shape?.booking?.[fc.name];
    if (targetArg) {
      const key = slotKey(fc.args?.[targetArg]);
      // -----------------------------------------------------------------------
      // LVX160. WHICH ARGUMENTS THE MODEL ACTUALLY SENT.
      //
      // On CAad88df4a a booking was refused because `in_addition_to_existing`
      // was not set, the refusal told the model to set it, the model asked the
      // caller the right question TWICE and never re-sent the call with it.
      // Establishing that took reading the refusal branch and the model's own
      // sentences and inferring backwards, because tool ARGUMENTS have never
      // been logged anywhere. "The flag was never set" was a deduction, not a
      // fact, on a call that cost the caller their booking.
      //
      // KEYS AND BOOLEANS ONLY, which is the same line the rest of this block
      // draws. An argument NAME is schema, not caller content, and a boolean is
      // a branch -- so `client_name` appears as a key here and never as a value.
      //
      // COMPUTED HERE RATHER THAN INLINE IN THE log.info BELOW, and not for
      // style: tests/logPhiLint.test.js reads the raw TEXT of a logger's
      // argument object, comments included, and the sentence listing what is
      // excluded tripped its shorthand-property matcher on the word it was
      // promising not to log. The lint was right to be literal; the prose moved.
      //
      // On every write, not just refused ones. A refusal is where the question
      // gets asked, but "what did a SUCCESSFUL call look like" is the control.
      // -----------------------------------------------------------------------
      const argKeys = Object.keys(fc.args || {}).sort();
      const argFlags = Object.fromEntries(
        Object.entries(fc.args || {}).filter(([, v]) => typeof v === "boolean")
      );
      log.info("write_target", {
        callSid,
        tool: fc.name,
        outcome: response?.success === true ? "written" : response?.gated === true ? "held" : "refused",
        slot_parsed: Boolean(key),
        slot_point_verified: Boolean(key && pointVerifiedSlots.has(key)),
        slot_listed_only: Boolean(key && verifiedSlots.has(key) && !pointVerifiedSlots.has(key)),
        slot_unverified: Boolean(key && !verifiedSlots.has(key)),
        point_verified_slots: pointVerifiedSlots.size,
        verified_slots: verifiedSlots.size,
        arg_keys: argKeys,
        arg_flags: argFlags,
      });
      if (response?.success === true && key && !pointVerifiedSlots.has(key)) {
        // A row now exists at a time nothing ever point-checked. Counted rather
        // than refused: the availability invariant deliberately accepts a listed
        // slot, because that is how a caller picks from offered alternatives.
        // This says how often that happens, which nothing has measured.
        bumpCounter("write_landed_on_listed_slot");
      }
    }
    ...
    // The same rule the write cache applies, for the same reason: a FAILED read
    // stays retryable, so a transient backend error does not freeze into the
    // turn and leave the model unable to get an answer at all.
    //
    // The whole functionResponse is stored, not just `response`. before() hands
    // it back with the new call's id swapped in, and a bare response object
    // would produce `{...response, id}` -- a functionResponse with no `response`
    // field, which the model reads as an empty answer.
    if (MEMOABLE_READS.has(fc.name) && response && response.success !== false) {
      readMemo.set(callKey(fc), result.functionResponse);
    }
  }
  ...
  return {
    ...
    after,
    /**
     * A new turn is a new question. Forget what the reads answered.
     *
     * `verifiedSlots` and `completedWrites` are deliberately untouched: both are
     * call-scoped facts. A slot an availability check confirmed at the start of
     * a call is still a slot the caller was offered, and a write that already
     * created a row still must not create a second one.
     */
    resetTurn() {
      readMemo = new Map();
    },
    ...
    /**
     * WHICH slots, not how many — the candidate set a post-call recovery may
     * choose from, and nothing wider.
     *
     * Every member got here from an availability tool's own response: a list
     * field (a day query's open slots, or the `alternatives` on a taken point
     * check) or the requested argument when a point check came back
     * `available: true`. So membership means "an availability response presented
     * this time as open", which is the only claim a recovery needs from it.
     *
     * Sorted, because an unordered set presented to a reader as a numbered list
     * would renumber itself between runs and make one verdict unreproducible.
     *
     * This does NOT say which time the caller chose, and the comment on
     * pointVerifiedSlots is the reason it cannot: a day query can put sixteen
     * slots in here, so "the time this call verified" is ambiguous sixteen ways.
     * That ambiguity is what a reverted design tried to resolve by itself and
     * got wrong. A caller's choice has to come from somewhere else; this only
     * bounds the options.
     *
     * Times, not caller data — a naive local timestamp to minute precision.
     */
    verifiedSlotList() {
      return [...verifiedSlots].sort();
    },
    counts() {
      return { ...counts };
    },
