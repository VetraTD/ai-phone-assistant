...
import { liveSurface, LIVE_MODEL_DEFAULT } from "./surface.js";

export { liveSurface, LIVE_MODEL_DEFAULT } from "./surface.js";
  ...
  if (!apiKey) {
    throw new Error(
      "GEMINI_API_KEY is not set and LIVE_SURFACE is not vertex. The key belongs in Secret " +
        "Manager, added directly with `gcloud secrets versions add gemini-api-key` — " +
 ...
 *
 * ---------------------------------------------------------------------------
 * WHAT `languagePinned` DOES NOT MEAN, because it has now misled two readers
 * into diagnosing a problem that cannot exist.
 * ---------------------------------------------------------------------------
 *
 * It is `Boolean(config.speechConfig.languageCode)`, and speechConfig governs
 * SPEECH GENERATION -- the voice the assistant speaks with and the language it
 * speaks. It says NOTHING about the language the caller's audio is recognised
 * as, and it is logged as `output_language_pinned` for that reason.
 *
 ...
 * `inputAudioTranscription: {}` is the only form it can take;
 * `RealtimeInputConfig` carries only automaticActivityDetection,
 * activityHandling and turnCoverage; and there is no top-level language field on
 * LiveConnectConfig. lib/transcriptUtils.js had already recorded the read half --
 * no confidence, stability or language field is exposed on inputTranscription.
 *
 * This matters because caller turns DO come back in the wrong language. On
 * 2026-09-12 an English caller asking to book a strategy call arrived as fluent
 * French and the call was lost, on a session reporting the pin as true. A field
 * that reads as "language is pinned" next to a transcript in French sends the
 * reader looking for a bug in our code. There isn't one here.
 * ---------------------------------------------------------------------------
 *
 * @param {object} opts
