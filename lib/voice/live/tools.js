  ...
  async function handleToolCall(toolCall) {
    ...
    for (const fc of calls) {
      ...
      const ctx = {
        ...
        // The last few completed assistant turns, oldest first, so the
        // write-order gate can find a read-back the caller has already heard
        // even when the most recent turn was an answer to a clarifying
        // question. Empty array rather than undefined: the gate falls back to
        // the single previous turn, which is what it did before the widening.
        // Copied here for the reason written above -- the producer and the
        // consumer each had tests and nothing proved the wire, which is how
        // LVX45 sat unreachable for a whole deployment.
        recentReplyTexts: Array.isArray(state.recentReplyTexts)
          ? state.recentReplyTexts.filter((t) => typeof t === "string" && t)
          : [],
