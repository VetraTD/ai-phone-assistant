...
export async function handleLiveSessionConnection(ws, _req, deps = {}) {
  ...
  function adjudicateUtterance(u) {
    ...
    if (u.transcriptAtMs === null) {
      bumpCounter("live_utterance_no_transcript");
  ...
  function noteUtteranceTranscript(text, atMs) {
    const u = pendingUtts.find((x) => x.transcriptAtMs === null) || openUtt;
    if (!u) return;
    u.transcriptAtMs = Math.round(atMs);
    u.transcriptChars += (text || "").length;
  }

  function utteranceFrame({ atMs, v, playing, forwarded, held, dropped }) {
  ...
  function onMediaFrame(mulawFrame, atMs) {
    ...
    const { forward, barge, held, dropped } = gate.push({
      frame: mulawFrame,
      playing,
    ...
    utteranceFrame({ atMs, v, playing, forwarded: forward.length, held, dropped });
