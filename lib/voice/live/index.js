...
export async function handleLiveSessionConnection(ws, _req, deps = {}) {
        ...
        turns: [
          {
            ...
            parts: [
              {
                text:
                  "(System: the caller has gone quiet. Say exactly this to them, " +
  ...
  function checkSilence(atMs, voiced, playing) {
    ...
    if (playing || silenceStage >= 3 || exitAfterTurn) return;

    const th = SILENCE_THRESHOLDS[state.step] || SILENCE_THRESHOLDS_DEFAULT;
    const due = [th.nudge1, th.nudge2, th.hangup][silenceStage];
