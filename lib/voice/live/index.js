 ...
 */
function liveLanguageCode(config) {
  const override = String(process.env.LIVE_LANGUAGE_CODE || "").trim();
...
export async function handleLiveSessionConnection(ws, _req, deps = {}) {
  ...
  async function retryPendingWrite() {
    ...
    sendTurnNote("write_retried", ok ? WRITE_RETRIED_NOTE : WRITE_RETRY_FAILED_NOTE);
  }
  ...
  function onServerContent(sc, atMs) {
    ...
    if (sc.inputTranscription?.text) {
      ...
      if (verdict.isEcho) {
        ...
        // The caller has just answered the spelling question. If a write was
        // refused waiting for exactly this, issue it now rather than hoping the
        // model will. Fire-and-forget: nothing in this handler may block on a
        // database round-trip, and the turn note carries the outcome.
  ...
  async function onStart(startData) {
    ...
    const config_ = {
      ...
      speechConfig: {
        ...
        languageCode: liveLanguageCode(config),
      },
    ...
    try {
      ...
      log.info("live_session_open", {
        callSid,
        // Which language the voice was actually pinned to, and where it came
        // from. "the accent changed" is unanswerable from a log that does not
        // record what was asked for.
        language_code: liveLanguageCode(state.config),
        language_source: process.env.LIVE_LANGUAGE_CODE
          ? "env"
          : state.config?.locale
            ? "tenant"
            : "default",
