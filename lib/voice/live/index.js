...
export async function handleLiveSessionConnection(ws, _req, deps = {}) {
      ...
      // "agreed" here would measure something the gate does not.
      {
        ...
        if (agreedNow && readBackThen) {
          lastAgreement = {
            ...
            // The TEXT as well as the key, for the same reason lastReadBackText
            // exists beside lastReadBackKey: a fingerprint can say the wording
            // moved and cannot say whether the PROPOSAL did. Supersession needs
            // to ask whether both read-backs named the same time, which is a
            // question only the words can answer.
            readBackText: lastReplyText,
            atMs: now(),
          };
  ...
  const turnState = () => ({
    ...
    // NOT measurement -- the write-order gate DECIDES on this one. Without it
    // agreementSuperseded falls back to fail-closed and refuses every
    // re-worded read-back, which is the six-attempt livelock of CA919b69.
    lastAgreementReadBackText: lastAgreement?.readBackText ?? null,
