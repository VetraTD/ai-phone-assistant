 ...
 *
 * BOTH ENTRIES ARE "Kore" ON PURPOSE, and that is not an oversight. An
 * unsupported voice name is not rejected by the vendor; it is silently ignored
 * and the session returns Kore anyway (backlog LVX13). So a British-sounding
 * name written here from memory would either be a no-op that looks like a fix,
 * or a change nobody has heard. The en-GB entry is filled in from
 * scripts/voice-compare.js, once the owner has picked by ear from candidates
 * the rig has proved are real -- roadmap phase 1, "a British voice you are
 * happy with". Until then this map exists, is consulted, and reports itself.
 */
...
export async function handleLiveSessionConnection(ws, _req, deps = {}) {
  ...
  async function retryPendingWrite() {
    ...
    sendTurnNote(
      ...
      // A FAILED retry is the one case that must be spoken: the caller has been
      // told about a booking that does not exist, and the model has to correct
      // that out loud. A successful one has nothing new to say.
      { requestReply: !ok }
    );
  }
