...
import { callerHasNameOnFile, nameSpokenIn } from "../../nameQuality.js";
...
import { ENGINE_OWNED_TOOLS } from "./guards.js";
import {
  matchClaimSlot,
  classifyClaimAction,
  slotParts,
...
import {
  ...
  DEFAULT_TIMEZONE,
} from "../../capabilities/datetime.js";
import { claimActionMap } from "../../../capabilities/index.js";
 ...
 */
/**
 * Every pack's claim vocabulary, resolved once per process. LVX114.
 *
 * Folded rather than looked up per tool because the claim guard runs on reply
 * text and has no function call to resolve an owner from. Frozen at import for
 * the same reason ACTION_TOOL_NAMES is: the registry is static, and rebuilding
 * it inside a per-turn handler would allocate on every caller sentence.
 */
const CLAIM_ACTIONS = claimActionMap();

/**
 * "today"/"tomorrow" for a naive-local slot key, or null.
 *
 * relativeDayLabel works on an INSTANT, and a verifiedSlots key is a naive
 * local wall clock with no zone attached (guards.js slotKey), so it has to be
 * anchored in the business timezone first. Doing that with `new Date(key)`
 * would read it as the SERVER's zone, which is UTC in production and
 * America/Los_Angeles under vitest -- a silent off-by-one on every call placed
 * near midnight, in the direction that makes "tomorrow" name the wrong day.
 *
 * @param {string} key - "YYYY-MM-DDTHH:MM", naive local
 * @param {object} config - normalised business config
 * @returns {"today"|"tomorrow"|null}
 */
function slotRelativeLabel(key, config) {
  const parts = slotParts(key);
  if (!parts) return null;
  const timezone = config?.timezone || DEFAULT_TIMEZONE;
  try {
    const ms = zonedComponentsToUtcMs(
      {
        year: parts.year,
        month: parts.month,
        day: parts.day,
    ...
    return relativeDayLabel(new Date(ms), timezone);
  } catch {
    // An invalid IANA zone throws rather than falling back, and a caller
    // mid-call must not lose a completion over a bad config value. Losing the
    // relative label only costs the "tomorrow" anchor; the weekday and the date
    // still match.
    return null;
  }
}
...
/**
 * Rung two of the claim ladder, spoken word for word. LVX114.
 *
 * "Before I finish" is doing real work: it frames the question as the last step
 * of something going fine, not as a retraction. Neither line mentions a
 * booking, a system, a problem or an apology -- there is nothing to apologise
 * for, and a caller who is told there is will believe it.
 */
const CLAIM_CONFIRM_TIME_LINE =
  "Before I finish — can I just confirm the time with you?";
const CLAIM_CONFIRM_NAME_LINE =
  "Before I finish — can I just take the name for the booking?";
...
export async function handleLiveSessionConnection(ws, _req, deps = {}) {
  ...
  /**
   * The end-of-call booking sweep, off without a deploy. LVX120.
   *
   * Every guard on this path has one, and this is the one that WRITES without
   * the model or the caller in the loop, so it needs the switch most.
   */
  const sweepOff = String(env.LIVE_BOOKING_SWEEP || "").trim().toLowerCase() === "off";
  ...
  function auditTurn() {
    ...
    const claimMatch = replyText ? S.completionClaimRe?.exec(replyText) : null;
    const claimedCompletion = Boolean(claimMatch);
    // THE SPAN, NOT THE TURN. Everything below that reads the sentence reads
    // this: the predicate bounds it, so a long turn cannot drag an unrelated
    // date or a second act into the answer. It is never logged or stored -- it
    // carries the caller's name and appointment time (LVX24).
    const claimSpan = claimMatch ? claimMatch[0] : "";
    // The SENTENCE the claim sits in, for the slot. The span above is the
    // predicate alone -- "we're all set" is the entire match on LVX114's turn --
    // so it carries no date and matches no slot. The sentence carries "for
    // Monday, September 14th, at 1 PM", and stopping at the full stop keeps a
    // different date in a neighbouring sentence out.
    const claimSentence = claimMatch ? sentenceAround(replyText, claimMatch.index) : "";

    // WHICH ACT, so a write can be matched to the claim it backs. LVX114.
    //
    // `claimed && !wroteAnything` in lib/postCallVerify.js let a successful
    // CANCEL vouch for a fabricated BOOKING, and the verdict came back `ok` on
    // the worst call of the round. A kind on the claim side is what that file's
    // own principle -- "an unrelated success must not vouch" -- needs in order
    // to reach `claimed`.
    //
    // The words are locale, the tools are the pack's, and the engine knows
    // neither: claimActionMap() folds every pack's declaration the same way
    // actionToolNames() does.
    let claimAction = claimedCompletion
      ? classifyClaimAction(claimSpan, S.claimActionProbes, CLAIM_ACTIONS)
      : "unspecified";

    // THE SLOT THE SENTENCE NAMED, and it does two jobs.
    //
    // On CAbdff67b2 the assistant said "we're all set for Monday, September
    // 14th, at 1 PM". No verb in it, so the probes above return `unspecified`
    // and a cancellation still vouches -- which is exactly how that call
    // reported clean. But it names a time a real check_appointment_availability
    // had returned minutes earlier, and naming a verified future slot in a
    // completion claim is what a booking confirmation IS. So it is
    // reclassified, and the same value is then the WHEN that the completion
    // below needs. One read, two jobs.
    //
    // Only ever PROMOTES `unspecified`. A claim that already said "cancelled"
    // is not turned into a booking because it happened to name a time.
    const claimedSlot =
      claimedCompletion && runner?.guards?.verifiedSlotKeys
        ? matchClaimSlot(claimSentence, runner.guards.verifiedSlotKeys(), {
            relativeLabelFor: (key) => slotRelativeLabel(key, state.config),
          })
        : null;
    if (claimedSlot && claimAction === "unspecified" && CLAIM_ACTIONS.booked) {
      claimAction = "booked";
      bumpCounter("live_claim_action_from_slot");
    }

    // A SENTENCE ABOUT WHAT THE CALLER ARRIVED WITH IS A REPORT. LVX120.
    //
    // On the call of 2026-09-11 the assistant opened with "I see you have an
    // appointment scheduled for Monday, September 14th at 4 30 PM" -- read
    // straight off get_caller_appointments_from_db, entirely true. The
    // predicate matched ("you have ... scheduled"), it was classified `booked`,
    // and the post-call verdict then demanded a book_appointment write that
    // this call had no reason to make.
    //
    // The verdict came back `claim_without_row`, which was the RIGHT answer for
    // that call -- but reached through the wrong sentence, because a real
    // fabrication three minutes later went undetected. Two errors cancelling is
    // not a working detector.
    //
    // The frozen arrival slots separate them: Monday was on the caller's record
    // before we said anything, so naming it claims nothing. A NEW verified slot
    // is a different matter.
    //
    // Demoted to `unspecified` rather than dropped: the sentence is still a
    // completion-shaped thing the model said, and the old any-write rule is the
    // right amount of suspicion for it.
    if (claimedSlot && callerSlotsAtStart.includes(claimedSlot) && claimAction === "booked") {
      claimAction = "unspecified";
      bumpCounter("live_claim_reported_existing");
    }

    // LVX29's claim ledger.
    //
    // Shape only: which turn, which step, whether a tool backed it, and now
    // which act it named plus the tools that could have made that act true.
    // Never the sentence. tests/livePostCall.test.js asserts the whole ledger
    // stringifies with no name and no time in it.
    if (claimedCompletion) {
      ...
      claimsThisCall.push({
        ...
        action: claimAction,
        satisfiedBy: CLAIM_ACTIONS[claimAction]?.satisfiedBy || [],
      });
    }
    ...
    if (claimedCompletion && !toolsRanThisTurn() && !toolRanPrevTurn) {
      ...
      // ALREADY TRUE? THEN SAY NOTHING. LVX115, from the call of 2026-09-11.
      //
      // book_appointment succeeded at 03:50:38. Twenty-one seconds and two
      // turns later the assistant said "We have your strategy call scheduled
      // for Tuesday, September 15th at 4 30pm" -- a TRUE sentence about a row
      // that existed. The guard above looks back exactly one turn (LVX93's
      // known hole), so it could not see the booking, fired, and the ladder
      // asked the caller "Before I finish, can I just confirm the time with
      // you?" about an appointment they already had. The note then fired on the
      // next claim and the assistant told the caller, out loud, "Just to
      // clarify, the appointment wasn't booked yet." It was.
      //
      // The write ledger is the authority the look-back is not: it spans the
      // whole call. This is the same kind-matching lib/postCallVerify.js does
      // after the call, and applying it there and not here is what made an
      // internal correction audible and FALSE -- LVX108's family, arriving
      // through a door I opened.
      //
      // THE SLOT IS PART OF THE TEST, not just the tool. A call that books
      // Monday and then fabricates a Tuesday booking must still be caught, so a
      // write only backs a claim when the times agree -- or when one of them is
      // unknown, where silence is the safer failure.
      //
      // The counter above still fires, deliberately: it measures the look-back
      // condition and that has not changed. What changed is what we DO about
      // it, and live_claim_backed_this_call is how often the difference matters.
      const backing = claimBackedThisCall(claimAction, claimedSlot);
      if (backing) {
        bumpCounter("live_claim_backed_this_call");
        log.info("live_claim_backed_this_call", { callSid, action: claimAction, tool: backing });
      }

      // MAKE IT TRUE BEFORE ARGUING WITH IT. LVX114.
      //
      // CLAIM_NOTE asks the model to correct itself, and asking is what this
      // front-end cannot rely on: there is no toolConfig that forces a call, so
      // every note, refusal and declaration is a REQUEST. It has worked -- on
      // call 7aef50 the model retracted and booked for real -- and it has
      // failed on three consecutive calls since, twice by producing an apology
      // the caller heard and once by making the model believe its own
      // fabrication and reach for the wrong tool.
      //
      // So the ladder runs first. If the booking can be completed from evidence
      // that did not come from the model, it is, and the note never fires
      // because there is no longer anything to correct. Everything the ladder
      // does not handle falls through to the note inside runClaimLadder, so the
      // guard that exists today is unchanged for every claim this cannot reach.
      //
      // Fire-and-forget with a catch, exactly like retryPendingWrite: this runs
      // from the turnComplete handler and applyTurn() must not wait on a tool
      // round.
      if (!claimGuardOff && !backing) {
        runClaimLadder({ action: claimAction, slot: claimedSlot, span: replyText }).catch((err) =>
          log.error("claim_ladder_error", { callSid, reason: err?.message, severity: "warn" })
        );
      }
    }
    ...
    // THE NAME, KEPT THE FIRST TIME IT IS SAID BACK. LVX121.
    //
    // The read-back happens early -- "Got it thanks, Nithin" -- and the sweep
    // runs at the end of the call, by which point the three-turn window has
    // moved past it. On 2026-09-11 the name was said at 06:23:18, the sweep ran
    // at 06:24:37, and it declined `name_unrecovered` on a call where the name
    // had been spoken, spelled, and read back.
    //
    // First wins: the earliest read-back is the one the caller heard and
    // corrected if it was wrong. In memory only, never logged.
    const readBackNow = replyText ? S.nameReadBackRe?.exec(replyText) : null;
    if (!nameReadBackThisCall && readBackNow?.[1]?.trim()) {
      nameReadBackThisCall = readBackNow[1].trim();
    }
  ...
  /**
   * Those same turns as one string, for the end-of-call sweep. LVX120.
   *
   * Reuses LVX78's window rather than keeping a second one. Three turns is the
   * right size here too and for the same reason: a slot named early and then
   * talked past is not what the caller left believing.
   *
   * `lastReplyText` is appended because recentReplies is filled at the START of
   * the NEXT turn -- so at finish() the sentence that ended the call, which is
   * exactly the one likely to carry the promise, is not in it yet.
   */
  const recentReplyText = () => [...recentReplies, lastReplyText].filter(Boolean).join(" ");

  /**
   * Did the caller say no, in so many words?
   *
   * Reads the TAIL of what they have said rather than the whole call: a "no
   * thanks" about something else twenty turns ago is not a refusal of this
   * booking.
   *
   * Fails OPEN by construction -- an empty or mangled transcript matches
   * nothing and does not block. Deliberate, and the reason is written on
   * bookingDeclineRe: losing the caller's words is routine on this front-end,
   * and refusing every booking whose caller was unintelligible is the commoner
   * harm.
   */
  function callerDeclinedRecently() {
    const said = typeof callerSaidThisCall === "string" ? callerSaidThisCall : "";
    if (!said) return false;
    return Boolean(getStrings(state.config).bookingDeclineRe?.test(said.slice(-300)));
  }
  ...
  /** True only while the engine is completing a claim it caught the model making. */
  let completingClaimWrite = false;
  /** Rung two of the claim ladder is asked at most once per call (LVX21). */
  let claimConfirmAsked = false;
  /** Client names on this caller's records when the call OPENED. See LVX119. */
  let callerNamesAtStart = [];
  /** The first name the assistant read back to this caller. See LVX121. */
  let nameReadBackThisCall = null;
  /**
   * Slot keys the caller ALREADY HAD when the call opened. LVX120.
   *
   * A sentence naming one of these is the assistant REPORTING a record, not
   * claiming this call did something. "I see you have an appointment scheduled
   * for Monday, September 14th at 4 30 PM" is true, is read straight off a
   * lookup, and was being counted as an unsatisfied booking claim -- which is
   * the false alarm LVX57 warns teaches readers to ignore the ledger.
   */
  let callerSlotsAtStart = [];
  ...
  const turnState = () => ({
    ...
    // Only ever true inside completeClaimedBooking. LVX114.
    //
    // Threaded the same way lastChance is, and copied into ctx in
    // lib/voice/live/tools.js -- a field produced here and not copied there is
    // how the hang-up gate sat unreachable for the life of a deployment, which
    // is why tests/liveToolContext.test.js asserts the wire rather than the two
    // ends separately.
    completingClaim: completingClaimWrite,
  });
  ...
  function recordToolOutput(out) {
    if (out.capabilityEffects?.length) {
      ...
      for (const e of out.capabilityEffects) {
        ...
        // The ROW ID travels with a booking now. LVX114: the pack's own onEffect
        // texts the caller a confirmation the moment this effect lands, and
        // without the id the post-call sender cannot tell which rows have
        // already been confirmed and sends a second message for the same
        // appointment.
        if (e.type === "booked") {
          writesThisCall.push({
            type: "booked",
            ...
            // WHICH TIME, normalised by the guards' own slotKey. LVX115: a
            // claim that arrives two turns after the booking it describes has
            // to be recognisable as being about THAT booking, and the only way
            // to tell it from a claim about a different one is the time.
            slot: runner?.guards?.normaliseSlot?.(e.data?.scheduled_at) || null,
          });
        }
  ...
  /**
   * The caller's name, recovered from something other than this sentence.
   *
   * ORDER MATTERS AND IT IS NOT ARBITRARY. `callerFacts.Name` is what the model
   * actually put in a tool argument earlier in the call -- it has been past the
   * spelling gate's attention and is the spelling the pack would have written.
   * The read-back capture is the fallback for the case LVX114 is: the model
   * never got as far as a tool call, so the only place the name exists is its
   * own sentence ("Thanks, John").
   *
   * @returns {{name: string, source: string}|null}
   */
  function recoverClaimedName(span) {
    const held = runner?.capabilityState?.appointments?.callerFacts?.Name;
    if (typeof held === "string" && held.trim()) {
      return { name: held.trim(), source: "caller_facts" };
    }
    const S = getStrings(state.config);
    const readBack = span ? S.nameReadBackRe?.exec(span) : null;
    const captured = readBack?.[1];
    if (typeof captured === "string" && captured.trim()) {
      return { name: captured.trim(), source: "read_back" };
    }
    // The earliest read-back on the call, which the window may have moved past.
    // Last because it is the oldest: anything in the span itself is more
    // current, and this is the fallback that stops a name said five turns ago
    // from being lost. LVX121.
    if (nameReadBackThisCall) {
      return { name: nameReadBackThisCall, source: "read_back_earlier" };
    }
    return null;
  }

  /**
   * Is this name the caller's, on evidence that did NOT come from the model?
   *
   * Two independent sources, and either is enough:
   *
   *   the caller's own transcript   nameSpokenIn over everything they have been
   *                                 transcribed saying this call -- LVX53's
   *                                 check, already built
   *   the caller's own records      a name already on an appointment for this
   *                                 number, which the database supplied at call
   *                                 start and the model never touched
   *
   * The second matters more than it looks. The transcript on this front-end is
   * a DEGRADED COPY -- 2,480 ms of speech has logged as zero characters -- so a
   * transcript-only test would refuse to complete bookings for callers who
   * plainly did say their name. On LVX114 the caller already had a real
   * appointment, so his name was on file whatever transcribed.
   *
   * Fails CLOSED. No evidence means no write, and the ladder asks him instead.
   */
  function nameIsTheCallers(name) {
    const said = typeof callerSaidThisCall === "string" ? callerSaidThisCall : "";
    if (said && nameSpokenIn(name, said)) return "transcript";
    const onFile = (state.callerContext?.upcomingAppointments || [])
      .map((a) => a?.client_name)
      .filter(Boolean);
    if (onFile.some((held) => nameSpokenIn(name, held))) return "on_file";
    // The records the caller ARRIVED with, which a cancellation cannot erase.
    //
    // Still a MATCH, never a source: the name being written comes from the
    // model's read-back, and this only asks whether the caller's own records
    // already carry it. LVX77's "Jane Doe" fails this exactly as it fails the
    // other two -- a placeholder the model invented matches no record.
    if (callerNamesAtStart.some((held) => nameSpokenIn(name, held))) return "on_file_at_start";
    return null;
  }

  /**
   * MAKE THE CLAIM TRUE. LVX114, and the point of this whole path.
   *
   * The assistant told the caller a booking was done. No tool did it. Rather
   * than argue with the sentence -- which is what CLAIM_NOTE does, by asking,
   * and asking has now failed on three consecutive calls -- recover what the
   * sentence promised, verify every piece of it against something that did not
   * come from the model, and perform the write. The caller hears nothing
   * different and the sentence stops being a lie.
   *
   * ---------------------------------------------------------------------------
   * WHY THIS IS NOT THE MODEL BOOKING WHATEVER IT LIKES
   * ---------------------------------------------------------------------------
   *
   * WHEN comes from `verifiedSlots`, which is filled ONLY from an availability
   * tool's own response. A time the model invented is not in that set, has no
   * candidate to match, and is unreachable -- and the write then goes through
   * `guards.before`, which refuses it a second time against the same set. The
   * rule is enforced twice, by construction, and neither enforcement is new
   * code written for this feature.
   *
   * WHO comes from the caller's own transcript or the caller's own records,
   * never from this sentence being believed about itself.
   *
   * Both must be present. Either one missing falls to the ladder: ask the
   * caller once, plainly, and then leave it to a person.
   *
   * ---------------------------------------------------------------------------
   * WHY IT GOES THROUGH handleToolCall
   * ---------------------------------------------------------------------------
   *
   * Identical to retryPendingWrite directly above: the availability invariant,
   * the spelling gate, checkRequirements, the pack's own no-name refusal and
   * the duplicate cache all still apply. The ONLY thing `completingClaim`
   * relaxes is the consent question, because the caller has already been told.
   *
   * NO sendToolResponse: there is no outstanding vendor tool call to answer.
   * And NO NOTE ON SUCCESS, which is the one place this differs from every
   * other guard on this path and is deliberate. The model already said the
   * right thing; telling it so invites it to say it again -- call 6 repeated a
   * sentence verbatim 43 ms after a note -- and there is now nothing to
   * correct. Silence IS the feature.
   */
  async function completeClaimedBooking({ action, slot, span }) {
    if (!runner || !session || closed) return { ok: false, reason: "closed" };
    const spec = CLAIM_ACTIONS[action];
    const tool = spec?.complete;
    // Declared-but-not-completable is the normal case, not an error: cancel and
    // reschedule move a row that already exists, and completing a false claim
    // about one would do real damage. See claimActions in the pack.
    if (!tool) return { ok: false, reason: "not_completable" };

    if (!slot) {
      bumpCounter("claim_completion_unrecoverable");
      log.error("claim_completion_unrecoverable", {
        callSid,
        action,
        reason: "slot_unverified",
        verified_slots: runner.guards?.verifiedCount?.() ?? 0,
        severity: "warn",
      });
      return { ok: false, reason: "slot_unverified" };
    }

    const recovered = recoverClaimedName(span);
    if (!recovered) {
      bumpCounter("claim_completion_unrecoverable");
      log.error("claim_completion_unrecoverable", {
        callSid,
        action,
        reason: "name_unrecovered",
        severity: "warn",
      });
      return { ok: false, reason: "name_unrecovered" };
    }

    // LVX77 IS THE STANDING WARNING HERE: a booking retry once wrote "Jane Doe",
    // a name the caller never said, into somebody's diary. A name identifies the
    // row, so this refuses rather than guesses.
    const provenance = nameIsTheCallers(recovered.name);
    if (!provenance) {
      bumpCounter("claim_completion_unrecoverable");
      log.error("claim_completion_unrecoverable", {
        callSid,
        action,
        reason: "name_provenance",
        name_source: recovered.source,
        severity: "warn",
      });
      return { ok: false, reason: "name_provenance" };
    }

    let out;
    pendingToolCalls += 1;
    completingClaimWrite = true;
    try {
      out = await runner.handleToolCall({
        functionCalls: [
          {
            id: "complete_" + tool,
            name: tool,
            args: { client_name: recovered.name, scheduled_at: slot },
          },
        ],
      });
    } catch (err) {
      log.error("claim_completion_error", { callSid, tool, reason: err?.message, severity: "warn" });
      return { ok: false, reason: "error" };
    } finally {
      // In a finally for the same reason onToolCall's is: a throw must not leave
      // the silence ladder suppressed, and must not leave the consent skip armed
      // for a write the MODEL makes on the next turn.
      completingClaimWrite = false;
      pendingToolCalls -= 1;
    }

    recordToolOutput(out);
    const ok = out.functionResponses?.[0]?.response?.success === true;
    bumpCounter(ok ? "claim_completed_in_code" : "claim_completion_refused");
    // Shape only. The slot and the name are the caller's data and neither is
    // logged -- which source vouched for the name is the part worth reading.
    log.error(ok ? "claim_completed_in_code" : "claim_completion_refused", {
      callSid,
      tool,
      action,
      name_source: recovered.source,
    ...
    return { ok, reason: ok ? null : "refused" };
  }

  /**
   * Has a write on THIS CALL already made this claim true? LVX115.
   *
   * @param {string} action
   * @param {string|null} slot - the slot the claim named, when it named one
   * @returns {string|null} the tool that backs it, or null
   */
  function claimBackedThisCall(action, slot) {
    const satisfiedBy = CLAIM_ACTIONS[action]?.satisfiedBy || [];
    if (satisfiedBy.length === 0) return null;
    for (const w of writesThisCall) {
      if (!w?.tool || !satisfiedBy.includes(w.tool)) continue;
      // Both slots known and different means this claim is about something
      // else, and something else is what the guard exists for.
      if (slot && w.slot && w.slot !== slot) continue;
      return w.tool;
    }
    return null;
  }
  ...
  /**
   * THE LADDER. Complete silently -> ask the caller once, plainly -> escalate.
   *
   * Rung one is completeClaimedBooking above, and when it works nobody hears
   * anything: the caller was told the booking existed, and now it does.
   *
   * RUNG TWO IS WHERE THIS PROJECT HAS REPEATEDLY GONE WRONG, so the rule is
   * written here rather than assumed. It must not say anything went wrong.
   * Three calls running, an internal control message reached the caller and
   * came out as "I'm sorry, the booking didn't go through" and "I'm not sure
   * why it's not updating, and I don't want to keep you on the phone while I
   * figure it out" -- and LVX105 deleted a line that asked for an apology for
   * exactly this reason. The caller has been told they are booked. They are
   * about to be, as soon as the one missing detail is in. Nothing has gone
   * wrong from where they are sitting, and telling them otherwise invents a
   * problem in order to apologise for it.
   *
   * WHY speakLine AND NOT sendTurnNote. Two reasons, both load-bearing:
   *
   *   1. speakLine hands over the words to say. CLAIM_NOTE describes a
   *      situation and lets the model choose its own sentence, which is how the
   *      apologies got out. This is the channel the silence ladder already uses
   *      on every call, and it has never produced a leak.
   *   2. sendTurnNote allows ONE note per model turn across every guard, and on
   *      the call that found LVX93 the spelling nudge had already spent it. A
   *      guard that is correct but silent is not a guard. speakLine is not
   *      rationed by that budget.
   *
   * THE QUESTION NAMES THE FIELD THAT ACTUALLY FAILED. Asking to confirm the
   * time when the NAME is what could not be verified is noise, and noise on
   * this path is what the caller experiences as the assistant being confused.
   *
   * ONCE PER CALL. A nudge that can repeat is the shape that made the leak
   * guard destroy a call (LVX21).
   *
   * RUNG THREE needs no code here and that is worth saying plainly: a claim
   * with no matching write already ends as verdict `claim_without_row`, which
   * writes a customer_requests row and notifies the business, in any mode
   * including `count` (lib/postCallVerify.js). The cases that can never be
   * completed after the fact -- the caller hangs up, the promised slot was
   * never checked, a transfer, anything about price -- all arrive there by
   * simply failing the rungs above, which is why none of them is special-cased.
   */
  async function runClaimLadder({ action, slot, span }) {
    const result = await completeClaimedBooking({ action, slot, span });
    if (result.ok) return true;

    // STRICTLY ADDITIVE: anything this path does not handle falls through to the
    // note that has always fired here, so no claim loses the guard it had.
    //
    // `not_completable` is the MAJORITY case, not an edge -- an unspecified
    // claim, a cancellation, a reschedule. Those keep CLAIM_NOTE exactly as
    // before, and nothing about what the model is told for them has moved.
    if (result.reason === "not_completable" || result.reason === "closed") {
      sendTurnNote("claim", CLAIM_NOTE);
      return false;
    }

    // Asked once already. The note is the fallback rather than a second
    // question: a nudge that can repeat is what made the leak guard destroy a
    // call (LVX21).
    if (claimConfirmAsked) {
      sendTurnNote("claim", CLAIM_NOTE);
      return false;
    }

    // A refusal is not a dead end: the commonest cause is a hard name the
    // caller has not spelled, and that path has already stashed a pendingWrite
    // which the existing spelling retry will re-issue. Asking about the name
    // is exactly the right question in that case too.
    const line =
      result.reason === "slot_unverified"
        ? CLAIM_CONFIRM_TIME_LINE
        : CLAIM_CONFIRM_NAME_LINE;

    if (!speakLine("claim_confirm", line)) {
      sendTurnNote("claim", CLAIM_NOTE);
      return false;
    }
    claimConfirmAsked = true;
    bumpCounter("claim_confirm_asked");
    log.error("claim_confirm_asked", { callSid, action, reason: result.reason, severity: "warn" });
    return false;
  }
  ...
  async function onToolCall(toolCall) {
    ...
    // ENGINE TOOLS DO NOT VOUCH FOR A CLAIM. LVX121.
    //
    // `end_call` and `set_call_intent` belong to no capability pack: one hangs
    // up the phone and the other classifies the conversation. Neither can make
    // a booking true, and both were counted here -- so on 2026-09-11 the final
    // "Your appointment is all set" was shielded from the claim guard because
    // end_call happened to succeed in the same turn.
    //
    // This is a MEASUREMENT change as well as a behaviour one:
    // live_claim_without_action will fire on turns it previously ignored, and
    // that is the guard seeing cases it was blind to rather than the model
    // getting worse. It is the same correction LVX93 made for lookups, applied
    // to the other class of tool that cannot back a claim.
    realToolCallsThisTurn += (toolCall.functionCalls || []).filter(
      (fc) => !ENGINE_OWNED_TOOLS.has(fc?.name)
    ).length;
  ...
  async function onStart(startData) {
    ...
    if (business && database.isEnabled()) {
      ...
      // THE NAMES THIS CALLER ARRIVED WITH, frozen. LVX119.
      //
      // `state.callerContext` is LIVE: onEffect rewrites it after every
      // successful book or cancel, which it must, or the model would keep
      // offering to cancel an appointment it just cancelled (LVX33).
      //
      // That makes it useless as evidence of WHO THE CALLER IS. On CA... of
      // 2026-09-11 the caller cancelled their only appointment at 05:10:24 and
      // then booked a new one; by the time the booking claim arrived, the
      // snapshot was empty and the name on it was gone. Cancelling destroyed
      // the record that proved the name was theirs.
      //
      // Captured once, never updated. Names only -- no ids, no times, no
      // numbers; this is read solely to answer "has this caller used this name
      // before", never to supply one.
      callerNamesAtStart = (extras.callerContext?.upcomingAppointments || [])
        .map((a) => a?.client_name)
        .filter(Boolean);
    ...
    // The TIMES the caller arrived with, normalised by the guards' own slotKey.
    //
    // AFTER the runner, and that is not cosmetic: this was written thirty lines
    // earlier, where `runner` is still undefined, so `runner?.guards
    // ?.normaliseSlot?.()` returned undefined for every row, the list came back
    // empty, and nothing said so. An optional call that swallows a missing wire
    // is a wire nothing can prove -- the same defect peekPendingWrite hit.
    //
    // Normalised through guards rather than re-derived here, so the comparison
    // cannot drift from the set it is compared against.
    callerSlotsAtStart = (extras.callerContext?.upcomingAppointments || [])
      .map((a) => runner.guards.normaliseSlot(a?.scheduled_at))
      .filter(Boolean);
  ...
  /**
   * BOOK WHAT THE CALL PROMISED AND NEVER WROTE. LVX120.
   *
   * ---------------------------------------------------------------------------
   * WHY THIS EXISTS WHEN completeClaimedBooking ALREADY DOES
   * ---------------------------------------------------------------------------
   *
   * The mid-call completion fires on a DETECTED CLAIM, and detecting a claim
   * means matching how the model chose to phrase it. That is an unbounded
   * question, and the model keeps winning it: `we're all set` (LVX97),
   * `I have you down for` (LVX94), `you have your strategy call scheduled`
   * (LVX119), and on 2026-09-11 `Your new appointment is on Tuesday, September
   * 15th at 4 30 PM` -- which matches nothing, because the determiner is not
   * adjacent to the noun and `is on` is locative rather than a completion verb.
   * The predicate has been widened four times in two weeks and a caller was
   * harmed each time it was found lacking.
   *
   * This asks a BOUNDED question instead. Not "how did it say it" but "which of
   * the times an availability call actually returned did it name?" There are a
   * fixed number of candidates and the sentence either identifies one or it does
   * not. A phrasing nobody has ever seen still names Tuesday 4:30.
   *
   * ---------------------------------------------------------------------------
   * THE FIVE CONDITIONS
   * ---------------------------------------------------------------------------
   *
   *   1  nothing was booked on this call          the write ledger -- a fact
   *   2  EXACTLY ONE verified slot was named      matchClaimSlot over the turns
   *   3  a name, recovered and provenance-checked same check as the completion
   *   4  every require.requiredFields argument    the tenant's own declaration
   *   5  the caller did not decline               their own words
   *
   * CONDITION 2 IS ALSO THE BROWSING GUARD, and that is not a side effect. A
   * caller who is offered "9 AM, 1 PM, or 4 30 PM" and hangs up to think has
   * THREE slots on the table; matchClaimSlot returns null on ambiguity and
   * nothing is written. Browsing ends with options open. A promise ends with one
   * time named. That distinction does the work a consent check cannot do here,
   * because the sentence that would carry consent is the one we have stopped
   * depending on.
   *
   * CONDITION 4 IS INERT FOR A TENANT THAT CONFIGURES NOTHING, and it is worth
   * saying so rather than implying a gate that is not there. `requiredFields` is
   * per-tenant; Digile Media's capability config is `{}`. It earns its place for
   * a clinic that sets `["client_dob"]`, and for everyone else conditions 2 and
   * 3 are the whole test.
   *
   * ---------------------------------------------------------------------------
   * WHY IT IS SAFE TO WRITE WITHOUT THE CALLER STILL ON THE LINE
   * ---------------------------------------------------------------------------
   *
   * The time can only be one an availability tool returned -- the invariant in
   * guards.js refuses anything else, on this path as on every other. The name
   * can only be one the caller's own transcript or their own records already
   * carry. So the engine cannot invent either half; it can only finish the one
   * combination that was already put to the caller.
   *
   * And the asymmetry that justifies the whole feature applies hardest here: a
   * wrongly-created appointment is visible in the diary and cancellable in
   * seconds, while a missing one is invisible until somebody does not arrive.
   *
   * Runs BEFORE the session closes and before postCallVerify reads the database
   * (see finish()). That ordering is load-bearing: reversed, the read would find
   * nothing, report `claim_without_row`, wake a human -- and then book it.
   */
  async function bookPromisedAppointment() {
    if (!runner || sweepOff) return;

    // (1) Nothing booked. A call that wrote a booking has nothing outstanding,
    // and the duplicate cache would refuse a second one anyway.
    if (writesThisCall.some((w) => w?.type === "booked")) return;

    const spec = CLAIM_ACTIONS.booked;
    const tool = spec?.complete;
    if (!tool) return;

    // (2a) A BOOKING THE CALL ACTUALLY ATTEMPTED, if there is one. LVX121.
    //
    // A write the gates refused is stashed with its own arguments, precisely so
    // it can be re-issued in code -- it is what retryPendingWrite replays when
    // the caller finally agrees. Those arguments carry the name and the time the
    // model meant, which is better evidence than anything recoverable from
    // prose.
    //
    // On 2026-09-11 the sweep went hunting for a name in recent sentences and
    // declined `name_unrecovered`, while `pendingWrite` held
    // `client_name: "Nithin Dodla"` and the Friday slot the whole time. The
    // caller was told they were booked and left with nothing.
    //
    // TAKEN, not peeked: a stash read twice books twice, and this is the last
    // thing that will ever read it.
    //
    // Nothing is relaxed by preferring it. The arguments go through
    // handleToolCall like any other, so the availability invariant still refuses
    // a time no availability call returned, and the name still has to survive
    // the provenance check below.
    const stashed = runner.takePendingWrite?.();
    if (stashed && stashed.name === tool) {
      const stashedSlot = runner.guards?.normaliseSlot?.(stashed.args?.scheduled_at);
      const stashedName = typeof stashed.args?.client_name === "string" ? stashed.args.client_name.trim() : "";
      if (stashedSlot && stashedName && !callerSlotsAtStart.includes(stashedSlot)) {
        if (callerDeclinedRecently()) {
          bumpCounter("sweep_declined_caller_said_no");
          log.error("sweep_declined", { callSid, reason: "caller_declined", severity: "warn" });
          return;
        }
        const stashedProvenance = nameIsTheCallers(stashedName);
        if (!stashedProvenance) {
          bumpCounter("sweep_declined_no_name");
          log.error("sweep_declined", {
            callSid,
            reason: "name_provenance",
            name_source: "pending_write",
            severity: "warn",
          });
          return;
        }
        await issueSweepBooking(tool, stashed.args, "pending_write", stashedProvenance);
        return;
      }
    }

    // (2) Exactly one verified slot named, across the assistant's recent turns.
    //
    // Recent, not the whole call: a slot named early and then talked past is not
    // what the caller left believing. The window is the same handful of turns
    // the claim guard looks at, and every candidate must still be a time an
    // availability call returned.
    const slot = matchClaimSlot(recentReplyText(), runner.guards?.verifiedSlotKeys?.() || [], {
      relativeLabelFor: (key) => slotRelativeLabel(key, state.config),
    });
    if (!slot) return;

    // A time the caller ALREADY had is not a promise this call made. LVX120's
    // other half -- the same reasoning that stops a report reading as a claim.
    if (callerSlotsAtStart.includes(slot)) return;

    // (3) A name, from somewhere other than this sentence, verified against
    // something other than the model.
    // (5) BEFORE the name, because a caller who said no has settled the question
    // and reporting "no name" for that call would describe the wrong thing.
    if (callerDeclinedRecently()) {
      bumpCounter("sweep_declined_caller_said_no");
      log.error("sweep_declined", { callSid, reason: "caller_declined", severity: "warn" });
      return;
    ...
    const recovered = recoverClaimedName(recentReplyText());
    if (!recovered) {
      bumpCounter("sweep_declined_no_name");
      log.error("sweep_declined", { callSid, reason: "name_unrecovered", severity: "warn" });
      return;
    }
    const provenance = nameIsTheCallers(recovered.name);
    if (!provenance) {
    ...
    // (4) is enforced inside the tool by checkRequirements, which is the only
    // place that knows the tenant's block -- so it is not re-implemented here.
    // A missing field returns success:false and the sweep reports a decline.
    await issueSweepBooking(tool, { client_name: recovered.name, scheduled_at: slot }, recovered.source, provenance);
  }

  /**
   * The write itself, shared by both of the sweep's routes.
   *
   * One place, so the stashed-arguments branch and the recovered-from-prose
   * branch cannot drift in what they set, what they count, or what they log.
   */
  async function issueSweepBooking(tool, args, nameSource, provenance) {
    let out;
    completingClaimWrite = true;
    try {
      out = await runner.handleToolCall({
        functionCalls: [{ id: "sweep_" + tool, name: tool, args }],
      });
    } catch (err) {
      bumpCounter("sweep_failed");
      log.error("sweep_failed", { callSid, tool, reason: err?.message, severity: "warn" });
      return;
    } finally {
      completingClaimWrite = false;
    }
    ...
    recordToolOutput(out);
    const ok = out?.functionResponses?.[0]?.response?.success === true;
    bumpCounter(ok ? "sweep_booked_at_close" : "sweep_refused");
    // Shape only: which tool, where the name came from, what vouched for it.
    // Never the slot and never the name.
    log.error(ok ? "sweep_booked_at_close" : "sweep_refused", {
      callSid,
      tool,
  ...
  function finish(reason) {
    ...
    // And an APPOINTMENT the caller was promised, for the same reason. LVX120.
    //
    // ORDER IS LOAD-BEARING: this must run before session.close() below and
    // before verify() reads the database further down. Reversed, the read finds
    // nothing, reports claim_without_row, wakes a human -- and then books it.
    // Both true at different instants and useless together.
    bookPromisedAppointment().catch((err) =>
      log.error("sweep_failed", { callSid, reason: err?.message, severity: "warn" })
    );
