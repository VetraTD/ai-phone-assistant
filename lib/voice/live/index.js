...
export async function handleLiveSessionConnection(ws, _req, deps = {}) {
   ...
   * announced when on, and the caller's own words are never touched --
   * inputTranscription is right there and is deliberately left alone.
   */
  function debugTranscript(replyText) {
    if (!debugTranscriptOn || !replyText) return;
  ...
  function auditTurn() {
    ...
    if (claimedCompletion) {
      claimsThisCall.push({
        ...
        kind: "claim",
        step: state.step,
        toolBacked: realToolCallsThisTurn > 0 || toolRanPrevTurn,
      });
    }

    if (claimedCompletion && !realToolCallsThisTurn && !toolRanPrevTurn) {
  ...
  function applyTurn() {
    ...
    toolRanPrevTurn = realToolCallsThisTurn > 0;
