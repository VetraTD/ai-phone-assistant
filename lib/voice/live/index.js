...
export async function handleLiveSessionConnection(ws, _req, deps = {}) {
  ...
  function onMediaFrame(mulawFrame, atMs) {
    ...
    // The strategy is driven by what GEMINI received, not by what arrived here.
    // The gate withholds inbound audio during our own playback and then drops
    // it, so running the strategy on those frames lets a manual arm bracket
    // activityStart/activityEnd around audio the model never got -- asking it
    // to answer silence, and cutting off the reply it was already giving.
  ...
  function onServerContent(sc, atMs) {
    ...
    if (sc.generationComplete || sc.turnComplete) {
      ...
      if (transfer && transferAllowed()) {
        ...
        armExit("transfer", state.config?.transferPhoneNumber || process.env.TRANSFER_NUMBER || null);
  ...
  async function redialForTransfer(transferNumber, trigger) {
    ...
    await client.calls(callSid).update({
      twiml: `<Response><Dial ringTone="uk"${callerIdAttr}>${escapeXml(transferNumber)}</Dial></Response>`,
    });
    log.info("live_transfer_outcome", { callSid, success: true, trigger });
    database.markCallTransferred?.(callSid)?.catch?.(() => {});
    finish("transferred");
  }
