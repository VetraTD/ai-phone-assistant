...
export async function handleLiveSessionConnection(ws, _req, deps = {}) {
  ...
  const turnState = () => ({ step, callerTurnCount, transferAllowed: true });
  ...
  async function onToolCall(toolCall) {
    ...
    if (out.intentArgs?.intent) {
      intent = out.intentArgs.intent;
      if (step === STEPS.GREETING) step = STEPS.GATHER_DETAILS;
    }
    if (out.toolResults.some((r) => r.success)) step = STEPS.CONFIRM;
    if (out.endCallArgs) endCallArmed = true;
  ...
  function onServerContent(sc, atMs) {
    ...
    if (sc.outputTranscription?.text) echoGuard.noteSpoken(sc.outputTranscription.text, atMs);
  ...
  async function onStart(startData) {
    ...
    // One round trip, and the call proceeds without it rather than waiting:
    // everything here improves the answer, none of it is needed to give one.
    if (business && database.isEnabled()) {
      database
        .withTenantSafe(
          ...
          { operation: "liveSessionContext", callSid, fallback: [{ status: "rejected" }, { status: "rejected" }, { status: "rejected" }] }
        )
        .then(([callRes, integrationsRes, callerRes]) => {
          if (callRes.status === "fulfilled") extras.callId = callRes.value;
          if (integrationsRes.status === "fulfilled") extras.integrations = integrationsRes.value || [];
          if (callerRes.status === "fulfilled") extras.callerContext = callerRes.value;
        })
        .catch(() => {});
    }
    ...
    runner = createToolRunner({ config, extras, turnState });
    ...
    const systemInstruction = buildSystemInstruction(step, intent, config, extras);
