...
export async function handleLiveSessionConnection(ws, _req, deps = {}) {
  ...
  function onServerContent(sc, atMs) {
    ...
    if (sc.generationComplete || sc.turnComplete) {
      ...
      // -----------------------------------------------------------------
      // LVX160, AND THE DEFECT THIS NEARLY SHIPPED WITH.
      //
      // `spellingSettled` releases the stash on its OWN, with no agreement
      // anywhere in the condition -- which is correct for a write the spelling
      // gate held, because that write was already consented to and was waiting
      // only on letters.
      //
      // An in_addition stash has NOT been consented to. It is created the
      // moment the pack refuses, before the caller has answered anything, and
      // it carries `in_addition_to_existing: true` -- a second appointment.
      // Released by the spelling gate settling, it books one the caller never
      // agreed to. The first run of tests/liveWriteRecovery.test.js wrote a
      // second row on a caller who said "No, move the first one instead."
      //
      // So this stash is released by agreement and by nothing else. That is
      // what the whitelist above is for, and this is the other half of it: a
      // reason may be releasable by a "yes" and still not releasable by a
      // spelling answer, and those are two different questions.
      // -----------------------------------------------------------------
      const spellingMayRelease = spellingSettled(state) && retryReason !== "in_addition";
      if (spellingMayRelease || heldForAgreement) {
