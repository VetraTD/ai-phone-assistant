 ...
 * within-turn case.
 *
 * The observed repeat restated a whole confirmation plus a closing question plus
 * a goodbye -- about 140 characters -- so the window has to be wide enough to
 * hold one of those and narrow enough that an ordinary turn does not overlap
 * itself. Turns on this path run 80-260 characters.
 */
const REPEAT_TAIL_CHARS = 140;
...
export async function handleLiveSessionConnection(ws, _req, deps = {}) {
  ...
  function auditTurn() {
    ...
    if (
      ...
      // Same switch, same reasoning. An offer of times nothing verified is the
      // same defect one step earlier in the call.
    ...
    if (replyText) {
      ...
      // Two question marks in one turn is a stacked question, exactly and for
      // free. The observed shape was "Can I start with your name, company, and
      // what industry you're in?" -- which is ONE question mark and three asks,
      // so this undercounts by construction. Recorded rather than widened: a
      // conjunction parser here would be the phrasing treadmill this file
      // already warns about, and the two-question-mark case is the one a
      // prospect most obviously hears as being interrogated.
      // TWO rules, and the second was added on 2026-09-05 because the first one
      // missed the turn that caused three consecutive repeats: "May I get your
      // full name, and what is the best number to reach you on?" is one question
      // mark and two questions. The counter read 2 for a call in which stacking
      // drove six repeats.
      const questionMarks = (replyText.match(/\?/g) || []).length;
      if (questionMarks > 1 || asksMoreThanOneThing(replyText)) {
        bumpCounter("live_stacked_questions");
        log.info("live_stacked_questions", { callSid, step: state.step, marks: questionMarks });
      }
  ...
  function inspectRepeat(fragment, atMs) {
    ...
    if (!acrossTurns && soFar.length > REPEAT_TAIL_CHARS * 2) {
      const head = soFar.slice(0, soFar.length - REPEAT_TAIL_CHARS);
      const tail = soFar.slice(-REPEAT_TAIL_CHARS);
      withinTurn = longestSharedRun(head, tail) >= REPEAT_RUN_WORDS;
    }
