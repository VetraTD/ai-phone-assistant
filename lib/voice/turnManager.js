...
export function createTurnManager({ vad, audioOut, echoGuard, onInterrupt, onTurnEnd, onVoiceActive, bargeInAllowed, now = () => performance.now() } = {}) {
   ...
   * @returns {boolean}
   */
  function sustainedVoice() {
    try {
      if (BARGE_MIN_VOICED_MS <= 0) return true;
      if (typeof vad?.voicedRunMs !== "function") return !!vad?.isActive?.(now());
      return vad.voicedRunMs(now()) >= BARGE_MIN_VOICED_MS;
    } catch {
      return false;
    }
  }
   ...
   * @returns {boolean}
   */
  function confidentEnough(confidence) {
    if (BARGE_MIN_CONFIDENCE <= 0) return true;
    if (typeof confidence !== "number" || !Number.isFinite(confidence)) return true;
    return confidence >= BARGE_MIN_CONFIDENCE;
  }
