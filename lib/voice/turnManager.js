 ...
 * }}
 */
export function createTurnManager({ vad, audioOut, onInterrupt, onTurnEnd, now = () => performance.now() } = {}) {
  ...
  function handleFinal(text) {
    try {
      ...
      const normalized = normalize(text);
      const pureBackchannel = normalized ? isPureBackchannel(normalized) : false;

      if (speaking && !pureBackchannel) {
        // Finals are trustworthy even without VAD confirmation: an echo of
        // the AI's own speech would come back as the AI's own words, not as
        // unrelated caller content — so we don't gate this on vad.isActive.
  ...
  function handleAudioFrame(mulawBuf) {
    try {
      return vad?.processFrame?.(mulawBuf, now()) ?? null;
    } catch (err) {
