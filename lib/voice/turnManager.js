  ...
  function handleFinal(text) {
    try {
      ...
      if (speaking && !pureBackchannel) {
        ...
        // A short cue-less final with no live mic energy is an STT phantom
        // (echo artifact, line noise), not a caller interjection. A genuine
        // one-word interjection arrives with VAD active and still interrupts.
        const strippedWords = stripped.split(/\s+/).filter(Boolean);
        if (
          !containsInterruptCue(normalized) &&
          strippedWords.length < 2 &&
          !vad?.isActive?.(now())
        ) {
