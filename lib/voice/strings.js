 ...
 */
/**
 * A bounded adverb slot, for the same reason READ_BACK_GAP exists. LVX143.
 *
 * CA239c7cd2, 2026-09-18: "Your appointment has been SUCCESSFULLY rescheduled
 * to Thursday, September 24 at 2:00 PM" on a call where the write was refused
 * and `changed_rows` was 0. `claim_audit` read `claimed: 0` -- not suppressed,
 * never detected -- so none of LVX140's work could reach it.
 *
 * The two slots below used to be the literal word `now` and nothing else, so
 * "has been rescheduled" matched and "has been successfully rescheduled" did
 * not. One ordinary English word defeated the whole predicate, which is
 * LVX107's finding arriving at a different regex.
 *
 * WHY A SLOT AND NOT MORE WORDS. There is already a wider claim predicate built
 * to catch phrasings the narrow one misses; measured across the 207 assistant
 * turns in call-corpus/ it fires on the same 26 turns and has never once caught
 * anything extra. A list encodes the phrasings of the model that was live when
 * it was written, and this model changed. A slot does not care which adverb
 * gets reached for.
 *
 * `-ly` keeps it tight, and the participle list is what makes the sentence a
 * claim at all -- so the gap cannot admit an unrelated sentence, only the same
 * claim with an adverb in it.
 *
 * The three named words are the temporal adverbs that sit in this slot and do
 * NOT end in -ly. `already` is the one that caught me out while writing this:
 * it reads like an -ly word and ends in "dy", so the test case for it failed
 * against a correct implementation. Worth the two seconds it costs to check.
 */
const CLAIM_ADVERB = String.raw`(?:(?:now|already|just|[a-z]+ly)\s+)?`;

/**
 * The connector list and the determiner list, named once.
 *
 * Extracted from claimNounSubject below when LVX151's open-head-noun branch
 * arrived needing the same two alternations. A second copy of a list like this
 * is how two predicates that are supposed to agree stop agreeing -- and the
 * LVX62 entry carries a worked example of what a stale duplicate costs: an
 * in-code comment citing the wrong call went on being used to argue for the
 * next change until someone re-read the logs.
 */
const CLAIM_CONNECTORS = String.raw`and|but|so|then|okay|ok|great|perfect|yes|now|also|plus|confirm`;
const CLAIM_DETERMINER = String.raw`(?:your|the|that|this|those|these|both)`;
const CLAIM_CLAUSE_BOUNDARY = String.raw`(?:^|["'“‘]|[.,;:!?—-]\s*|\b(?:${CLAIM_CONNECTORS})[,]?\s+)`;

const claimNounSubject = (verbs) =>
  String.raw`${CLAIM_CLAUSE_BOUNDARY}${CLAIM_DETERMINER}\s+(?:appointments?|bookings?|calls?|consultations?)\b[^.?!]{0,60}?\s+(?:is|are|'s|(?:has|have)\s+${CLAIM_ADVERB}been)\s+${CLAIM_ADVERB}(?:${verbs})\b`;
...
// ---------------------------------------------------------------------------
// LVX151. THE TWO SHAPES THE REGEX ABOVE STILL CANNOT SEE.
//
// Everything from here to claimsCompletion() was built and validated in
// scripts/corpus/score-claims.mjs against all 304 assistant turns in
// call-corpus/, 18 adversarial negatives and 7 positives, and lived there while
// the LVX150 refusal-wording A/B ran -- widening the deployed detector mid-arm
// would have made the model self-correct more often and the arm unreadable.
// That arm closed "not demonstrated" at 0 of 7, so this moves in.
//
// MOVED, NOT COPIED. score-claims.mjs now imports these, and its fixture lists
// stay where they are: selftest() runs on every invocation of that script and
// exits 1 on a dirty predicate, so the fixtures go on guarding this code from
// the file they were written in. Two copies of a predicate that must agree is
// the failure this codebase has paid for more than once.
//
// MEASURED BEFORE IT SHIPPED, over every assistant turn in the corpus: 52 turns
// matched the narrow predicate, 62 match with these, and all 10 of the newly
// matched turns are real completion claims. Nothing else moved. Two of them are
// the fabrications the deployed detector is structurally blind to -- CA03558d
// ("Yes, I have confirmed that your appointment is booked", booked_rows 0) and
// CA299f23ce ("That is all booked", eight seconds after a refused write).
// ---------------------------------------------------------------------------
 ...
 * "All-In-One Package", so the deployed predicate is silent on this business's
 * own booking nouns. Note that "call" IS in that closed list and still does not
 * help: in "your free strategy call" it is not adjacent to the determiner.
 *
 * THE OBJECT SIDE LOOSENS SO THE VERB SIDE TIGHTENS -- LVX107's trade, applied
 * to this branch. This one takes only participles that can mean nothing but a
 * booking. `set`, `sorted`, `done`, `updated` and `moved` are deliberately NOT
 * here, because with an open head noun "the paperwork is all set" and "your
 * file has been updated" are ordinary sentences and would both become
 * fabrication alerts.
 */
const CLAIM_BOOKED_ONLY = String.raw`booked|scheduled|rescheduled|cancell?ed|confirmed`;
/**
 ...
 * a subordinate clause and `that` sits exactly where a boundary has to be.
 *
 ...
 * by running candidates rather than by reasoning:
 *
 ...
 */
const CLAIM_NEG_LEFT = String.raw`(?<!\b(?:not|never|cannot|can't|don't|didn't|doesn't|haven't|hasn't|won't|couldn't)\s)(?<!\bn't\s)`;
const CLAIM_REPORTING_THAT = String.raw`\b${CLAIM_NEG_LEFT}(?:confirmed|checked|verified)\s+that\s+`;
const CLAIM_OPEN_BOUNDARY = String.raw`(?:${CLAIM_CLAUSE_BOUNDARY}|${CLAIM_REPORTING_THAT})`;
const CLAIM_MODIFIED_NOUN = new RegExp(
  CLAIM_OPEN_BOUNDARY +
    String.raw`${CLAIM_DETERMINER}\s+` +
    String.raw`(?:[a-z][a-z-]*\s+){0,3}` +
    String.raw`[a-z][a-z-]*\s+` +
    String.raw`(?:is|are|'s|(?:has|have)\s+${CLAIM_ADVERB}been)\s+${CLAIM_ADVERB}(?:${CLAIM_BOOKED_ONLY})\b`,
  "i"
);
 ...
 *
 * The subject list is closed, which is what makes the longer participle list
 * safe here where it is not safe above.
 */
const CLAIM_DONE_PARTICIPLES = String.raw`booked|scheduled|confirmed|cancell?ed|rescheduled|moved|updated|all\s+set|set|sorted|done|finali[sz]ed`;
const CLAIM_UNCONTRACTED_COPULAR = new RegExp(
 ...
 *
 * Required by ADDITION 1 and by nothing else: its head noun is open, so
 * *"The phone number the appointment is booked under?"* has exactly its shape
 * and is the assistant asking. The regexes above are protected from that
 * sentence by their closed noun list; this one has to be protected explicitly.
 *
 * Split per SENTENCE rather than per turn, so a real claim followed by "Is
 * there anything else I can help you with?" — which is most of them — still
 * counts.
 */
...
/**
 * Does this turn tell the caller something is done?
 *
 * The regex FIRST, then the two additions per assertive sentence. The regex
 * half is deliberately tested against the whole turn and not per sentence: it
 * has always matched claim-shaped text wherever it sits, including inside a
 * question, and narrowing that here would be an unmeasured change riding along
 * with a measured one.
 *
 * @param {RegExp} re - the locale's own completion-claim regex
 * @param {boolean} withAdditions - false for a locale with no evidence behind
 *   the English-only shapes above. A closed list written from imagination is
 *   this file's most repeated mistake; see deniedAvailabilityRe.
 */
const makeClaimsCompletion = (re, withAdditions) => (text) => {
  if (!text) return false;
  if (re?.test(text)) return true;
  if (!withAdditions) return false;
  return assertiveSentences(text).some(
    (s) => CLAIM_MODIFIED_NOUN.test(s) || CLAIM_UNCONTRACTED_COPULAR.test(s)
  );
};
 ...
 *
 * `proceed` and `confirm` added 2026-09-12, and this is the FOURTH round of
 * adding a missing word. On CAb4c8a9e6 the assistant said "Okay, I'm cancelling
 * your appointment for Marcus Bell on Monday, September fourteenth at four
 * thirty PM. Should I proceed?" -- a complete read-back -- the gate refused it,
 * the model re-asked, and the caller heard three near-identical sentences in 22
 * seconds before the cancellation went through.
 *
 * ---------------------------------------------------------------------------
 * AND THE LIST IS THE WRONG AXIS. Read this before adding a fifth word.
 * ---------------------------------------------------------------------------
 *
 * Measured against 824 real assistant turns from 65 production calls
 * (scripts/probes/readback-corpus.mjs). Two of the misses were this shape:
 *
 *   "Would you like to cancel the appointment on Wednesday at 10 00 AM?"
 *
 * which misses only because the alternation below requires "me to". The obvious
 * fix is to make `me` optional. IT CANNOT BE DONE, because this is the same
 * grammatical shape:
 *
 *   "Would you like to book a strategy call as well?"
 *
 * The first is a read-back and the second is an offer with nothing settled, and
 * no phrasing rule can separate them -- only the fact that one names a specific
 * time and the other does not. So every widening either keeps missing real
 * read-backs or starts admitting offers, which is the exact hole the paragraph
 * above is about.
 *
 * The replacement, when it is built: require the turn to name a time that
 * resolves to the pending write's target, and drop the vocabulary test. The
 * corpus harness exists so a candidate can be scored before it ships.
 */
/**
 * `move` is TRANSITIVE here and the particle is not this verb.
 *
 * A read-back moves something: "shall I move that to Thursday". "Shall we move
 * on?" is a receptionist changing the subject, and it became reachable the
 * moment the pronoun was widened to include `we` (2026-09-17) -- caught by the
 * negative it was added with, not by reading. The lookahead applies to the
 * singular too, where "shall I move on?" was always wrong and always matched.
 */
const READ_BACK_ACTION_VERB = String.raw`(?:go\s+ahead|book|cancel|move(?!\s+on\b)|reschedule|change|make|update|put|try|proceed|confirm)`;
...
export const STRINGS = {
  en: {
    ...
    /**
     * READING SOMEONE THEIR EXISTING BOOKING IS A REPORT, NOT A CLAIM.
     *
     * Rule 2 of the post-call judge's own prompt, in those words. The live claim
     * guard never had it, and measured over 30 days of production that cost
     * SEVEN of 34 firings -- the assistant answers "what have I got booked?" by
     * reading the row back, and is told it lied:
     *
     *   "I see you have an appointment scheduled for Thursday, September 17th"
     *   "I can see you have a strategy call scheduled for Monday at 3 00 PM"
     *   "It looks like you're scheduled for a consultation"
     *
     * Those follow get_caller_appointments_from_db, which is a READ, so nothing
     * is in the action look-back and the guard fires every time.
     *
     * TWO FRAMES, both deliberately narrow:
     *
     *   a PERCEPTION verb -- "I see", "I can see", "it looks like" -- followed
     *   by "you" in the SAME sentence. The [^.?!] run is what stops a bare
     *   acknowledgement ("I see. You're booked for Monday.") from reaching
     *   across the full stop into the claim behind it.
     *
     *   "you have <a|your|that> ... scheduled|booked", for the turns that skip
     *   the perception verb.
     *
     * What it must NOT match is an assertion wearing a report's clothes: "I've
     * cancelled your appointment for Monday" has no perception frame and says
     * "your appointment" rather than "you have". Asserted both ways in
     * tests/liveClaimGuard.test.js.
     */
    existingAppointmentReportRe: new RegExp(
      String.raw`\b(?:i\s+(?:can\s+)?see|it\s+looks\s+like|looks\s+like)\b[^.?!]{0,80}\byou\b` +
        String.raw`|\byou\s+(?:already\s+)?have\s+(?:an?|your|that)\b[^.?!]{0,70}\b(?:scheduled|booked)\b`,
      "i"
    ),
    ...
    // -----------------------------------------------------------------------
    // ONE WORD LEFT A CALLER ON AN OPEN LINE. CAaef5bd82, 2026-09-17.
    //
    // The model booked the appointment, said "Thanks for calling Digile Media,
    // and have a WONDERFUL day", and did not call end_call. This regex is the
    // only other thing that can close a line, its adjective list read
    // `(?:great|good|lovely)`, and so nothing ended the call. The caller sat
    // there for eight seconds until the silence ladder nudged with "I'm still
    // here whenever you're ready", then another six.
    //
    // Scored across every farewell in call-corpus/ -- nine real calls -- rather
    // than patched on the strength of one:
    //
    //     "have a WONDERFUL day"   x4    all missed
    //     "have a GREAT day"       x2    both matched
    //
    // Two thirds of this tenant's farewells used a word the detector had never
    // seen. The earlier four hid it because the model ALSO called end_call on
    // those, so something else closed the line and the miss cost nothing.
    //
    // THE SECOND ALTERNATIVE IS NOW OPEN ON THE ADJECTIVE, AND ANCHORED TO THE
    // END INSTEAD. A closed list breaks again on "fantastic", "lovely rest of
    // your", or whatever the next model says, and each break is a caller on a
    // dead line. But a false positive here HANGS UP ON SOMEONE, which is much
    // worse than a missed goodbye -- the silence ladder catches the miss, and
    // nothing catches the wrong hang-up.
    //
    // So: any adjective, but only when the farewell is the last thing said,
    // which is what a farewell is. "I'll have a full day free on Tuesday" does
    // not arm; "...and have a wonderful day." does.
    //
    // The three candidates -- closed list, open-anywhere, open-and-anchored --
    // score IDENTICALLY on the corpus (6 of 6, no false positives across 78
    // assistant turns), so the evidence cannot choose between them. This one is
    // picked for having the least dangerous failure mode, and that reasoning is
    // the record, not the score.
    // -----------------------------------------------------------------------
    signOffRe:
    ...
    //
    // THE PRONOUN IS A CLOSED CLASS TOO, and it was not one until 2026-09-17.
    //
    // The modal had been widened four times; the pronoun was hardcoded `i` the
    // whole while. On call CAb4c0eb the model answered this gate's own refusal
    // with "One second, I have that down for your free strategy call at 1 PM on
    // Friday, September 18th. Shall we book that in for you?" -- a complete
    // read-back, in the exact form the refusal asks for -- and was refused
    // again, because the pronoun was plural.
    //
    // The cost was not one refusal. `readBackMade=false` ALSO withholds the
    // pendingWrite stash in services/tools.js, so the caller's "Yes please"
    // seconds later had nothing to release, retryPendingWrite never ran, and
    // the model told the caller "Perfect, I've booked that in for you" over an
    // empty appointments table. One pronoun disabled the gate, the retry and
    // the ceiling budget together.
    //
    // `we` leans on the verb list rather than widening it, which is why this is
    // not the fifth round of the mistake the note above warns about: `get`,
    // `check`, `see` and `ask` are still absent, so "Can we get your full name?"
    // and "Shall we move on?" stay out. Certified both directions in
    // tests/confirmReadBackRe.test.js.
    //
    ...
    confirmReadBackRe: new RegExp(
      String.raw`\b(?:just\s+to\s+confirm|to\s+confirm\b|can\s+i\s+(?:just\s+)?confirm|let\s+me\s+confirm|confirming\s+that` +
        ...
        // "Are you sure you would like me to cancel your appointment?"
        //
        // CAb4427e, 2026-09-18. The caller said yes to that sentence and the
        // cancellation was refused, because this family had every other way of
        // asking -- "shall I", "would you like me to proceed", "are you happy
        // for me to" -- and not this one. Both of that call's OTHER
        // confirmations matched; this one phrasing is the whole gap, and it
        // cost the caller the thing they rang to do.
        //
        // BOUND TO AN ACTION VERB, exactly like its siblings, rather than
        // admitting a bare "are you sure". The verb is what keeps this a
        // confirmation OF A WRITE instead of any reassurance the model offers,
        // and it is the same discipline the rest of this alternation follows.
        //
        // Scored against every assistant turn on disk before being added: one
        // hit in 179 real turns, and it is the sentence above.
        String.raw`|would\s+that\s+be\s+right|(?:shall|should|may|can)\s+(?:i|we)\s+${READ_BACK_ACTION_VERB}|(?:would|do)\s+you\s+(?:like|want)\s+me\s+to\s+${READ_BACK_ACTION_VERB}|are\s+you\s+happy\s+for\s+me\s+to|are\s+you\s+sure\s+(?:you'?d\s+like|you\s+would\s+like|you\s+want)\s+(?:me\s+)?to\s+${READ_BACK_ACTION_VERB}|you'?d\s+like\s+(?:me\s+)?to\s+${READ_BACK_ACTION_VERB})\b`,
      "i"
    ),
    ...
    // -----------------------------------------------------------------------
    // LVX137. THE ASSISTANT TURNING A TIME DOWN.
    //
    // Half of a detector, and the weaker half on purpose. The other half asks
    // whether the time named in the sentence is one an availability response
    // ITSELF put on the record as open -- which is structural, needs no
    // phrasing list, and is what makes a hit meaningful. This only finds the
    // sentences worth asking that question about.
    //
    // FITTED TO ONE OBSERVED SENTENCE, on CA6df19e98:
    //   "I'm sorry, 2:00 PM is not available."
    // One instance in 153 assistant turns across fifteen real calls, which is
    // the entire corpus this could be scored against. So it WILL under-report,
    // and that is the right direction for the failure it feeds: it counts, it
    // does not refuse. A phrasing this misses is a number slightly too low; a
    // phrasing it over-matches would be a false accusation in a log that a
    // human reads as fact.
    //
    // English only, deliberately. There is a Spanish bundle below and no
    // Spanish evidence whatsoever, and this file has been bitten repeatedly by
    // closed lists built on a guess (signOffRe's adjective, three times). A
    // counter that is silent on a locale is honest; one that is wrong on a
    // locale is worse than absent.
    // -----------------------------------------------------------------------
    deniedAvailabilityRe:
      /\b(?:is|are|was|were)\s+(?:not|no\s+longer)\s+available\b|\b(?:isn'?t|aren'?t|wasn'?t|weren'?t)\s+available\b|\bunavailable\b|\b(?:already|fully)\s+booked\b|\b(?:is|are)\s+(?:taken|full)\b|\bdo(?:es)?\s*n(?:o|')t\s+have\s+(?:that|any)\b|\bno\s+(?:openings|availability|slots)\b/i,
  },
  ...
  es: {
    ...
    completionClaimWideRe:
      /\b(?:(?:ya\s+)?(?:he|hemos)\s+(?:reservado|agendado|cancelado|programado|anotado|enviado)|(?:est[aá]|queda|qued[oó])\s+(?:reservad[oa]|agendad[oa]|cancelad[oa]|confirmad[oa]|list[oa]))\b/i,
    // See the English existingAppointmentReportRe.
    //
    // Localized rather than copied, and for the opposite reason to the line
    // above: this one SUPPRESSES a note. An English-only pattern would leave
    // Spanish callers exactly where every caller was before -- the assistant
    // reads back the appointment they already have and is told it lied -- so
    // the miss costs conversation quality rather than write integrity.
    //
    // Same two frames as the English: a perception verb reaching no further
    // than the end of its own sentence, and a possession frame. "veo que tiene
    // una cita programada para el jueves", "parece que tiene una cita".
    //
    // NOT a translation of the English word list, which is the trap the
    // completionClaimWideRe comment above is about. These are the two
    // constructions Spanish uses for the same act; neither has been read on a
    // Spanish call yet, and this suppresses rather than gates, so the failure
    // direction is the status quo.
    existingAppointmentReportRe:
      /\b(?:veo|puedo\s+ver|parece)\s+que\b[^.?!¿¡]{0,80}\b(?:tiene|tienes)\b|\b(?:tiene|tienes)\s+(?:una|la|su)\b[^.?!¿¡]{0,70}\b(?:programad[ao]|agendad[ao]|reservad[ao])\b/i,
    ...
    // LVX137, and NULL ON PURPOSE rather than a guess.
    //
    // The English counterpart is fitted to one observed sentence out of 153
    // real assistant turns. There is not a single Spanish denial anywhere in
    // the corpus, and this file's own history is three rounds lost to closed
    // lists written from imagination — signOffRe's adjective list broke on
    // "wonderful" after breaking twice before.
    //
    // `null` keeps the key present (tests/strings.test.js requires parity, and
    // typeof null matches typeof a RegExp), leaves the counter silent on this
    // locale, and is visible to whoever next has Spanish evidence to fill it
    // with. A counter that is silent on a locale under-reports honestly; one
    // invented from a phrasebook puts a wrong number in a log a human reads as
    // fact.
    deniedAvailabilityRe: null,
  },
};

// ---------------------------------------------------------------------------
// LVX151. The claim predicates as FUNCTIONS, attached per locale.
//
// Attached in a loop rather than written into each table literal, so a locale
// added later cannot be given a claim regex and silently left without the
// sentence-level half -- the shape of defect this file keeps producing, where a
// value exists at the producer and never reaches the consumer.
//
// Each table gets the helper built from ITS OWN regexes. The superset property
// the two regexes have by construction therefore survives into the functions:
// same additions on both sides, narrow floor inside wide floor.
//
// ADDITIONS ARE ENGLISH-ONLY, and this is deliberate rather than an oversight.
// CLAIM_MODIFIED_NOUN and CLAIM_UNCONTRACTED_COPULAR are English determiners
// and English copulas; they would be inert against Spanish rather than wrong,
// but "inert" and "validated against nothing" read identically in a counter.
// Spanish keeps its regex and gets the key, exactly the trade
// `deniedAvailabilityRe: null` makes twenty lines above.
// ---------------------------------------------------------------------------
for (const [lang, table] of Object.entries(STRINGS)) {
  const withAdditions = lang === "en";
  table.claimsCompletion = makeClaimsCompletion(table.completionClaimRe, withAdditions);
  table.claimsCompletionWide = makeClaimsCompletion(
    table.completionClaimWideRe || table.completionClaimRe,
    withAdditions
  );
}
