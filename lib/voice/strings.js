 ...
 */
const READ_BACK_GAP = String.raw`(?:\s+(?!(?:name|names|spelling|number|address|email|phone|surname)\b)[a-z’'-]+){0,1}`;
...
export const STRINGS = {
  en: {
    ...
    // (READ_BACK_GAP) wherever this pattern assumed adjacency, plus the
    // clause-terminal guard (READ_BACK_END) that stops the gap from reaching
    // an attributive adjective. Certified 18 positives / 13 negatives.
    confirmReadBackRe: new RegExp(
      String.raw`\b(?:just\s+to\s+confirm|to\s+confirm\b|can\s+i\s+(?:just\s+)?confirm|let\s+me\s+confirm|confirming\s+that` +
        String.raw`|is\s+that${READ_BACK_GAP}\s+(?:right|correct|okay|ok)${READ_BACK_END}` +
        String.raw`|does\s+that${READ_BACK_GAP}\s+(?:sound|look)${READ_BACK_GAP}\s+(?:right|good|correct)${READ_BACK_END}` +
        String.raw`|(?:did|have)\s+i\s+(?:get|got)\s+(?:that|this|it|everything|them|the\s+details|those\s+details)${READ_BACK_GAP}\s+right${READ_BACK_END}` +
        String.raw`|(?:just\s+)?to\s+(?:make\s+sure|be\s+sure|double[\s-]?check)\s+(?:i|we)\s+(?:have|'?ve\s+got|got)\s+(?:that|this|it|everything|them|the\s+details|those\s+details)${READ_BACK_GAP}\s+(?:right|correct)${READ_BACK_END}` +
        String.raw`|would\s+that\s+be\s+right|shall\s+i\s+(?:go\s+ahead|book|cancel|move|reschedule)|should\s+i\s+(?:go\s+ahead|book|cancel|move|reschedule)|(?:would|do)\s+you\s+(?:like|want)\s+me\s+to\s+(?:go\s+ahead|book|cancel|move|reschedule)|are\s+you\s+happy\s+for\s+me\s+to|you'?d\s+like\s+(?:me\s+)?to\s+(?:book|cancel|move|reschedule|change))\b`,
      "i"
    ),
