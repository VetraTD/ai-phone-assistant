  ...
  en: {
    ...
    confirmReadBackRe: new RegExp(
      String.raw`\b(?:just\s+to\s+confirm|to\s+confirm\b|can\s+i\s+(?:just\s+)?confirm|let\s+me\s+confirm|confirming\s+that` +
        ...
        String.raw`|would\s+that\s+be\s+right|(?:shall|should|may|can)\s+(?:i|we)\s+${READ_BACK_ACTION_VERB}|(?:would|do)\s+you\s+(?:like|want)\s+me\s+to\s+${READ_BACK_ACTION_VERB}|are\s+you\s+happy\s+for\s+me\s+to|you'?d\s+like\s+(?:me\s+)?to\s+${READ_BACK_ACTION_VERB})\b`,
      "i"
    ),
