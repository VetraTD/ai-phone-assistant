...
export const STRINGS = {
  en: {
    ...
    // stays anchored to a QUESTION being asked, so the idiom cannot trip it.
    closingTicRe:
      /\b(?:is|was)\s+there\s+anything\s+else\b|\banything\s+else\s+(?:i|we)\s+(?:can|could)\b|\banything\s+else\s+(?:for\s+you|you\s+need|today)\b|\banything\s+else\s+(?:you'?d|you\s+would|you'?re\s+wondering)\b|\b(?:what|how)\s+else\s+(?:can|could|may)\s+(?:i|we)\b/i,
  },
