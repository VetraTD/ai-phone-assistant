...
const claimNounSubject = (verbs) =>
  String.raw`(?:^|["'“‘]|[.,;:!?—-]\s*|\b(?:and|but|so|then|okay|ok|great|perfect|yes|now|also|plus|confirm)[,]?\s+)(?:your|the|that|this|those|these|both)\s+(?:appointments?|bookings?|calls?|consultations?)\b[^.?!]{0,60}?\s+(?:is|are|'s|(?:has|have)\s+${CLAIM_ADVERB}been)\s+${CLAIM_ADVERB}(?:${verbs})\b`;
