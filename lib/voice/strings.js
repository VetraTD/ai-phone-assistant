...
export const STRINGS = {
  en: {
    ...
    // cannot match "your ".
    completionClaimRe:
      /(?:\b(?:(?:i|we)(?:'ve| have)\s+(?:got\s+)?(?:(?:you\s+)?(?:booked|scheduled|cancell?ed|rescheduled|moved|recorded|noted|sent|made a note|put you down|got you down|down)|(?:a|an|the|your|that|this)(?:\s+\w+){1,2}\s+(?:booked|scheduled|cancell?ed|rescheduled|moved|recorded|sent))|(?:that'?s|it'?s|you'?re|you are)\s+(?:booked|scheduled|cancell?ed|rescheduled|confirmed|updated|all set|sorted|done))\b|(?:^|["'“‘]|[.,;:!?—-]\s*|\b(?:and|but|so|then|okay|ok|great|perfect|yes|now|also|plus|confirm)[,]?\s+)(?:your|the|that|this|those|these|both)\s+(?:appointments?|bookings?|calls?|consultations?)\b[^.?!]{0,60}?\s+(?:is|are|'s|(?:has|have)\s+(?:now\s+)?been)\s+(?:now\s+)?(?:booked|scheduled|confirmed|cancell?ed|rescheduled|moved|updated|all\s+set|set|sorted|done)\b)/i,
    ...
    completionClaimWideRe:
      /(?:\b(?:(?:i|we)(?:'ve| have)\s+(?:got\s+)?(?:(?:you\s+)?(?:booked|scheduled|cancell?ed|rescheduled|moved|recorded|noted|sent|made a note|put you down|got you down|down)|(?:a|an|the|your|that|this)(?:\s+\w+){1,2}\s+(?:booked|scheduled|cancell?ed|rescheduled|moved|recorded|sent))|(?:that'?s|it'?s|you'?re|you are)\s+(?:booked|scheduled|cancell?ed|rescheduled|confirmed|updated|all set|sorted|done|finali[sz]ed|taken care of)|(?:we'?re|we are)\s+(?:all set|confirmed|finali[sz]ed))\b|(?:^|["'“‘]|[.,;:!?—-]\s*|\b(?:and|but|so|then|okay|ok|great|perfect|yes|now|also|plus|confirm)[,]?\s+)(?:your|the|that|this|those|these|both)\s+(?:appointments?|bookings?|calls?|consultations?)\b[^.?!]{0,60}?\s+(?:is|are|'s|(?:has|have)\s+(?:now\s+)?been)\s+(?:now\s+)?(?:booked|scheduled|confirmed|cancell?ed|rescheduled|moved|updated|all\s+set|set|sorted|done|finali[sz]ed)\b)/i,
