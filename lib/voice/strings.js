...
/**
 * ONE PREDICATE. The narrow/wide split is gone, and LVX114 is why.
 *
 * The split was LVX97's, made on 2026-09-09 so that one counting round stayed
 * comparable with the ten calls that produced LVX94-98: the wide phrasings fed
 * the post-call ledger while the NARROW one alone drove CLAIM_NOTE, so nothing
 * about what the model was told mid-call moved.
 *
 * That round ended. What the split left behind is a live guard that cannot see
 * the commonest fabrication phrasing there is. On call CAbdff67b2 the assistant
 * said "we're all set for Monday, September 14th, at 1 PM", never called
 * book_appointment, and was NEVER TOLD IT HAD LIED -- because `we're all set`
 * was wide-only and the note reads the narrow one. It carried the false booking
 * to the end of the call and the caller hung up believing in an appointment
 * that did not exist.
 *
 * So the ladder this project keeps returning to -- count first, act once the
 * counter has said how often it fires when nothing is wrong -- has been climbed
 * for these phrasings. They fired, on real calls, on genuine fabrications.
 *
 * WHAT IS NOT WIDENED, and it is the load-bearing half. The `we` subject keeps
 * a deliberately short predicate list of its own: only `all set` / `confirmed` /
 * `finalised` may follow it. "We're booked up on Wednesday" is availability and
 * "we're done for today" is an ordinary wrap-up, and a guard that nags a model
 * for saying either is worse than one that misses. `you`/`that`/`it` keep the
 * longer list they already had.
 *
 * The phrasings this is known to cover are in tests/completionClaimRe.test.js.
 * Anything not in that table is a guess -- that was true of both halves and is
 * true of the single one.
 */
function buildCompletionClaimRe() {
  const copular = String.raw`booked|scheduled|cancell?ed|rescheduled|confirmed|updated|all set|set|sorted|done|finali[sz]ed|taken care of`;
  const nounVerbs = String.raw`booked|scheduled|confirmed|cancell?ed|rescheduled|moved|updated|all\s+set|set|sorted|done|finali[sz]ed`;
  const wePlural = String.raw`|(?:we'?re|we are)\s+(?:all set|confirmed|finali[sz]ed)`;
  return new RegExp(
...
export const STRINGS = {
  en: {
    ...
    // cannot match "your ".
    // Assembled at the top of this file, where the collapse of the old
    // narrow/wide split is recorded. This drives CLAIM_NOTE, the counters and
    // the post-call ledger alike -- one predicate, one population.
    completionClaimRe: buildCompletionClaimRe(),
    // AN ALIAS, kept because callers still read it. LVX114, 2026-09-11.
    //
    // This was LVX97's wider predicate, fed to the post-call ledger while the
    // narrow one drove the turn note. The split is collapsed (see
    // buildCompletionClaimRe), so this now names the SAME RegExp as
    // completionClaimRe.
    //
    // Not deleted, because lib/voice/live/index.js reads it with a fallback to
    // the narrow one for locales that carry no wide variant -- Spanish has
    // always had two identical literals -- and removing the key would make that
    // fallback silently evaluate undefined and turn the post-call audit off for
    // those callers. A guard that protects only English callers is a defect this
    // file already names once.
    //
    // Safe to share one object: the pattern carries no /g, so it holds no
    // lastIndex and two readers cannot interfere.
    completionClaimWideRe: buildCompletionClaimRe(),
    // WHICH ACT was claimed, so a write can be matched to the claim it backs.
    // LVX114, 2026-09-11.
    //
    // `claimed && !wroteAnything` let ANY write vouch for ANY claim: on
    // CAbdff67b2 a successful cancel_appointment_db proved a fabricated BOOKING
    // was fine, and postcall_verify returned `ok` on a call where the caller
    // lost a real appointment and gained an imaginary one. lib/postCallVerify.js
    // already states the principle for the abandoned-write verdict one
    // paragraph above the hole -- "an unrelated success must not vouch for an
    // abandoned write" -- and this is what that principle needs to reach
    // `claimed`: a kind on the claim side to match the tool on the write side.
    //
    // THE WORDS ARE LOCALE. WHICH TOOL SATISFIES THEM IS THE PACK'S (see
    // claimActions in capabilities/appointments.js). That split is the same one
    // this file has always had -- strings knows English, packs know what a
    // booking is -- and it is why the engine can ask the question without
    // knowing any tool's name.
    //
    // RUN AGAINST THE MATCHED CLAIM SPAN, never the whole turn. The span is
    // bounded by the predicate above, so a turn that cancels one thing and
    // offers another cannot smear the two together.
    //
    // TWO MATCHES MEANS UNSPECIFIED, deliberately. "I've cancelled that and
    // booked you in for Monday" names two acts, and a claim recorded as one of
    // them would demand the wrong write and report a call that went fine. An
    // ambiguous claim falls back to the old any-write rule, which is the
    // behaviour every claim has today -- LVX57's standing warning is that a
    // false alarm teaches the reader to ignore the ledger.
    // "no, leave it" -- the caller declining to go ahead. LVX120.
    //
    // Read by the end-of-call sweep, which writes a booking the call promised
    // and never made. Everything else it checks is a fact about the call; this
    // is the one place it consults the caller's own words, and it is the only
    // thing that can stop it.
    //
    // DELIBERATELY A CLOSED LIST, and narrow. It FAILS OPEN: a transcript that
    // lost the caller's words does not block the booking, because on this
    // front-end losing them is routine -- 2,880 ms of speech has arrived as four
    // characters -- and refusing every booking whose caller was unintelligible
    // is the commoner harm. LVX112's lesson is that a gate reading silence as an
    // answer is worse than one that does not read silence at all.
    //
    // So this must only match an UNAMBIGUOUS refusal to proceed. "I'm not sure"
    // and "hold on" are not here: a caller thinking aloud has not declined, and
    // treating hesitation as a no is how the hesitation gate came to refuse
    // bookings on the commonest confirmation in English.
    bookingDeclineRe:
      /\b(?:no\s+thanks?|no\s+thank\s+you|don'?t\s+book|do\s+not\s+book|don'?t\s+bother|never\s*mind|forget\s+it|cancel\s+that|scratch\s+that|leave\s+it|not\s+right\s+now|not\s+today|maybe\s+(?:later|another\s+time)|another\s+time|i'?ll\s+(?:think|call\s+back|ring\s+back|do\s+it\s+later)|let\s+me\s+think)\b/i,
    claimActionProbes: [
      { action: "cancelled", re: /\bcancell?ed\b/i },
      { action: "rescheduled", re: /\b(?:rescheduled|moved\s+(?:it|that|you|your))\b/i },
      { action: "booked", re: /\b(?:booked|scheduled|put\s+you\s+down|got\s+you\s+down|down\s+for)\b/i },
      { action: "noted", re: /\b(?:noted|recorded|made\s+a\s+note)\b/i },
    ],
    ...
    confirmReadBackRe: new RegExp(
      String.raw`\b(?:just\s+to\s+confirm|to\s+confirm\b|can\s+i\s+(?:just\s+)?confirm|let\s+me\s+confirm|confirming\s+that` +
        ...
        String.raw`|would\s+that\s+be\s+right|(?:shall|should|may|can)\s+i\s+(?:go\s+ahead|book|cancel|move|reschedule)|(?:would|do)\s+you\s+(?:like|want)\s+me\s+to\s+(?:go\s+ahead|book|cancel|move|reschedule)|are\s+you\s+happy\s+for\s+me\s+to|you'?d\s+like\s+(?:me\s+)?to\s+(?:book|cancel|move|reschedule|change))\b`,
      "i"
    ),
  ...
  es: {
    ...
    completionClaimWideRe:
      /\b(?:(?:ya\s+)?(?:he|hemos)\s+(?:reservado|agendado|cancelado|programado|anotado|enviado)|(?:est[aá]|queda|qued[oó])\s+(?:reservad[oa]|agendad[oa]|cancelad[oa]|confirmad[oa]|list[oa]))\b/i,
    // See the English claimActionProbes. Present rather than absent for the same
    // reason as the key above: tests/strings.test.js demands key parity, and a
    // locale silently missing this one would make every Spanish claim
    // `unspecified` through an undefined lookup rather than through a decision.
    //
    // `listo` is deliberately NOT here. It is Spanish's "all set" -- a stative
    // wrap-up with no act named in it -- so it belongs in the predicate that
    // detects a claim, not in the probes that say WHICH act was claimed. Same
    // reasoning that keeps "all set" out of the English probe list.
    // See the English bookingDeclineRe. Present rather than absent because the
    // key parity check demands it, and because a sweep that could not hear a
    // Spanish caller's "no" would write bookings they refused -- the one thing
    // that predicate exists to stop. Same closed-list discipline: an outright
    // refusal only, never hesitation.
    bookingDeclineRe:
      /\b(?:no\s+gracias|no\s+reserve|no\s+lo\s+reserve|d[eé]jelo|olv[ií]delo|cancele\s+eso|ahora\s+no|hoy\s+no|en\s+otro\s+momento|otro\s+d[ií]a|lo\s+pensar[eé]|d[eé]jeme\s+pensar|le\s+llamo\s+luego)\b/i,
    claimActionProbes: [
      { action: "cancelled", re: /\bcancelad[oa]s?\b|\bhe(?:mos)?\s+cancelado\b/i },
      { action: "rescheduled", re: /\bcambiad[oa]s?\b|\bmovid[oa]s?\b|\bhe(?:mos)?\s+(?:cambiado|movido)\b/i },
      { action: "booked", re: /\breservad[oa]s?\b|\bagendad[oa]s?\b|\bprogramad[oa]s?\b|\bhe(?:mos)?\s+(?:reservado|agendado|programado)\b/i },
      { action: "noted", re: /\banotad[oa]s?\b|\bhe(?:mos)?\s+anotado\b/i },
    ],
