...
export const STRINGS = {
  en: {
    ...
    // cannot match "your ".
    completionClaimRe:
      /(?:\b(?:(?:i|we)(?:'ve| have)\s+(?:got\s+)?(?:you\s+)?(?:booked|scheduled|cancell?ed|rescheduled|moved|recorded|noted|sent|made a note|put you down|got you down|down)|(?:that'?s|it'?s|you'?re|you are)\s+(?:booked|scheduled|cancell?ed|rescheduled|confirmed|updated|all set|sorted|done))\b|(?:^|["'“‘]|[.,;:!?—-]\s*|\b(?:and|but|so|then|okay|ok|great|perfect|yes|now|also|plus|confirm)[,]?\s+)(?:your|the|that|this|those|these|both)\s+(?:appointments?|bookings?|calls?)\b[^.?!]{0,60}?\s+(?:is|are|'s|(?:has|have)\s+(?:now\s+)?been)\s+(?:now\s+)?(?:booked|scheduled|confirmed|cancell?ed|rescheduled|moved|updated|all\s+set|set|sorted|done)\b)/i,
    ...
    completionClaimWideRe:
      /(?:\b(?:(?:i|we)(?:'ve| have)\s+(?:got\s+)?(?:you\s+)?(?:booked|scheduled|cancell?ed|rescheduled|moved|recorded|noted|sent|made a note|put you down|got you down|down)|(?:that'?s|it'?s|you'?re|you are)\s+(?:booked|scheduled|cancell?ed|rescheduled|confirmed|updated|all set|sorted|done|finali[sz]ed|taken care of)|(?:we'?re|we are)\s+(?:all set|confirmed|finali[sz]ed))\b|(?:^|["'“‘]|[.,;:!?—-]\s*|\b(?:and|but|so|then|okay|ok|great|perfect|yes|now|also|plus|confirm)[,]?\s+)(?:your|the|that|this|those|these|both)\s+(?:appointments?|bookings?|calls?)\b[^.?!]{0,60}?\s+(?:is|are|'s|(?:has|have)\s+(?:now\s+)?been)\s+(?:now\s+)?(?:booked|scheduled|confirmed|cancell?ed|rescheduled|moved|updated|all\s+set|set|sorted|done|finali[sz]ed)\b)/i,
    ...
    signOffRe:
      /\b(?:thanks|thank you)\s+for\s+calling\b[^.!?]*\b(?:good\s*bye|bye|great\s+day|good\s+day|great\s+weekend|lovely\s+day|take\s+care)\b|\bhave\s+a\s+(?:great|good|lovely)\s+(?:day|weekend|evening)\b/i,
    ...
    confirmReadBackRe:
      /\b(?:just\s+to\s+confirm|to\s+confirm\b|can\s+i\s+(?:just\s+)?confirm|let\s+me\s+confirm|confirming\s+that|is\s+that\s+(?:right|correct|okay|ok)|does\s+that\s+(?:sound|look)\s+(?:right|good|correct)|did\s+i\s+get\s+that\s+right|have\s+i\s+got\s+that\s+right|would\s+that\s+be\s+right|shall\s+i\s+(?:go\s+ahead|book|cancel|move|reschedule)|should\s+i\s+(?:go\s+ahead|book|cancel|move|reschedule)|(?:would|do)\s+you\s+(?:like|want)\s+me\s+to\s+(?:go\s+ahead|book|cancel|move|reschedule)|are\s+you\s+happy\s+for\s+me\s+to|you'?d\s+like\s+(?:me\s+)?to\s+(?:book|cancel|move|reschedule|change))\b/i,
