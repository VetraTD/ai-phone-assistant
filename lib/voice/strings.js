...
export const STRINGS = {
  en: {
    ...
    // Several per kind, cycled within a call. The caller hears a tool round on
    // most turns of a booking, and the identical sentence three times running
    // is how a person notices they are talking to a machine.
    holdAvailability: [
      "Let me check the diary for you.",
      "Just having a look at what we have got.",
      "One second while I check the calendar.",
    ],
    holdLookup: [
      "Let me pull that up.",
      "Just finding that for you now.",
      "Let me have a look at your details.",
    ],
    holdWrite: [
      "Just getting that sorted for you.",
      "Let me get that written down.",
      "Putting that through now.",
    ],
  ...
  es: {
    ...
    holdAvailability: [
      ...
      "Un momento, estoy viendo qué tenemos.",
      "Permítame revisar el calendario.",
    ],
    holdLookup: ["Déjeme buscar eso.", "Ya se lo busco.", "Permítame ver sus datos."],
    holdWrite: ["Ya se lo estoy registrando.", "Permítame anotarlo.", "Lo estoy ingresando ahora."],
 ...
 *
 * Locale-independent: tool NAMES are the same in every language, and the line
 * each one maps to is looked up per-locale by the caller. Falls back to the
 * generic filler for anything unrecognised — including a business's own webhook
 * tools, where guessing what it does would be worse than "One moment."
 *
 * @param {string} toolName
 * @returns {"filler"|"holdAvailability"|"holdLookup"|"holdWrite"}
 */
export function holdKindForTool(toolName) {
  if (typeof toolName !== "string" || !toolName) return "filler";
  if (/availability|available_slots|find_slots/.test(toolName)) return "holdAvailability";
  if (/^(get|list|lookup|check)_/.test(toolName)) return "holdLookup";
  if (/^(book|cancel|reschedule|record|create|update)_/.test(toolName)) return "holdWrite";
  return "filler";
}
