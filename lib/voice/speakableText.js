...
// sometimes emits, groups phone numbers into hyphen-separated runs, expands
 ...
 */
// Hyphen, not space, and it is a measurement rather than a preference.
//
// A space-grouped number LOSES A DIGIT through Google Cloud TTS:
//
...
//
// Two independent recognisers agree, so it is the synthesizer and not the
// instrument. ElevenLabs prefers hyphens too (2 of 2 correct against 1 of 2
// with spaces), and the UK form reads correctly either way (en-GB, 2 of 2 both
// separators) — so one separator serves both lanes and neither is a guess.
//
// A dropped digit in a callback number is the worst single thing a receptionist
// can get wrong: the caller writes down a number that does not ring, and
// nothing downstream can detect it. Reproduce with `npm run tts:readback`.
const PHONE_GROUP_SEPARATOR = "-";

function groupPhoneDigits(digits, profile) {
  // UK numbers are not grouped in threes and never were. "+442079460958" was
  // being spoken "442 079 460 958" — the country code swallowed into the first
  // group and the rest chopped at meaningless boundaries. A British caller
  // hears "020 7946 0958".
  //
  // Deliberately approximate: this is the shape of the common ranges, not the
  // full national numbering plan. A TTS engine reading a group boundary one
  // digit off is still perfectly intelligible, and the plan is a great deal of
  // code and test surface for an inaudible improvement.
  if (profile?.phone?.style === "uk" && digits.startsWith("44")) {
    const national = `0${digits.slice(2)}`;
    if (/^07/.test(national) && national.length === 11) {
      return `${national.slice(0, 5)}${PHONE_GROUP_SEPARATOR}${national.slice(5)}`; // 07700-900123
    }
    if (/^0(20|23|24|28|29)/.test(national) && national.length === 11) {
      return [national.slice(0, 3), national.slice(3, 7), national.slice(7)].join(PHONE_GROUP_SEPARATOR); // 020-7946-0958
    }
    if (national.length === 11) {
      return [national.slice(0, 4), national.slice(4, 7), national.slice(7)].join(PHONE_GROUP_SEPARATOR); // 0161-496-0000
    }
    return national;
  }
  ...
  if (local.length === 10) {
    return [local.slice(0, 3), local.slice(3, 6), local.slice(6)].join(PHONE_GROUP_SEPARATOR);
  }
  const groups = [];
  for (let i = 0; i < local.length; i += 3) groups.push(local.slice(i, i + 3));
  return groups.join(PHONE_GROUP_SEPARATOR);
}

/**
 * Group 10+ digit runs (phone numbers); leave shorter digit runs alone.
 ...
 */
function spacePhoneNumbers(text, profile) {
  return text.replace(/\+?\d[\d\-.]*\d/g, (match) => {
    const digits = match.replace(/[^\d]/g, "");
    if (digits.length < 10) return match;
    return groupPhoneDigits(digits, profile);
  });
}

/**
 * Currency, & and % -> words. The decimal case must run before the whole-unit
 * case so "$5.50" isn't first collapsed by the simpler pattern.
 *
 * All three currencies unconditionally, with no profile lookup. A symbol only
 * ever appears because the model chose it, so gating on the business's own
 * locale would mishandle precisely the case that needs handling — and if a US
 * business's model emits a pound sign, expanding it correctly is strictly safer
 * than reading the glyph aloud.
 */
/**
 * "$150.50" -> "150 dollars and 50 cents".
 *
 * It used to be "150 dollars 50", which a synthesizer reads as "one hundred
 * fifty dollars fifty" — ambiguous — and "$150.00" came out as "150 dollars
 * 00", spoken "one hundred fifty dollars zero zero". A fee read wrong is a
 * wrong fee, not a wrong-sounding one.
 *
 * Three cases the naive form gets wrong, all handled here: no cents at all
 * rather than "zero zero"; no leading zero, because "05 cents" is spoken "zero
 * five cents"; and the singular, because "1 cents" is not a thing anybody says.
 *
 * @param {string} minor two digits, as written
 */
function spokenAmount(major, minor, unit, subUnit, subUnitSingular) {
  const cents = Number.parseInt(minor, 10);
  if (!cents) return `${major} ${unit}`;
  return `${major} ${unit} and ${cents} ${cents === 1 ? subUnitSingular : subUnit}`;
}

function expandSymbols(text) {
  return text
    .replace(/\$(\d+)\.(\d{2})\b/g, (_m, maj, min) => spokenAmount(maj, min, "dollars", "cents", "cent"))
    .replace(/\$(\d+)\b/g, "$1 dollars")
    .replace(/£(\d+)\.(\d{2})\b/g, (_m, maj, min) => spokenAmount(maj, min, "pounds", "pence", "penny"))
    .replace(/£(\d+)\b/g, "$1 pounds")
    .replace(/€(\d+)\.(\d{2})\b/g, (_m, maj, min) => spokenAmount(maj, min, "euros", "cents", "cent"))
    .replace(/€(\d+)\b/g, "$1 euros")
    .replace(/&/g, " and ")
    .replace(/%/g, " percent");
}
...
export function stripInternalTerms(text) {
  ...
  // What was stripped, never the sentence it was stripped from.
  //
  // This used to log `{ original: text.slice(0, 200) }`, and that text is the
  // assistant's own speech -- which carries the caller's name, number and
  // appointment time, because reading details back is one of the commonest
  // lines on these calls. Neither lib/logger.js nor tests/logPhiLint.test.js
  // caught it: both work from PHI_FIELD_NAMES, by field NAME, and this field
  // was called `original`.
  //
  // The vocabulary itself is OURS and is the thing anyone reading this line
  // actually wants -- which term got through, so the upstream fix can be
  // checked. The caller's sentence was never the signal.
  const terms = [
    ...new Set([
      ...(text.match(INTERNAL_TERM_RE) || []).map((t) => t.toLowerCase()),
      ...(text.match(UUID_RE) ? ["uuid_literal"] : []),
      ...(text.match(HTTP_STATUS_RE) ? ["http_status"] : []),
    ]),
  ];
  ...
  log.error("internal_term_stripped", { chars: text.length, terms, severity: "warn" });
  return out;
}
 ...
 *
 * The bracket must be ADJACENT to the identifier. The older form allowed
 * `\s*` in front of it, which made ordinary prose match: "the note said
 * {see reception}" was excised down to "the note on the when they arrive."
 * A real pseudo-call never has that space — `book_appointment{client_name:Ada}`,
 * `default_api:get_caller_appointments_from_db{}` — so requiring adjacency
 * costs the guard nothing and stops it eating English.
 */
const CALL_SHAPE_RE = /\b[A-Za-z][A-Za-z0-9_]*(?:\s*[.:]\s*[A-Za-z0-9_]+)?[{(][^{}()]*[})]/;
 ...
 * @param {string[]} [ctx.toolParamNames] - every PARAMETER name those
 *   declarations expose, for the whole-text pre-pass below.
...
export function sanitizeOutbound(text, ctx = {}) {
  ...
  // WHOLE-TEXT PRE-PASS, before the split below.
  //
  // Everything after this point judges one sentence at a time, so a bracketed
  // blob whose VALUE contains a full stop is never seen whole: the `{` lands in
  // one sentence and the `}` in the next, and JSON_RE needs both. That is
  // precisely how a caller came to hear
  // `{reason:Caller declined further assistance and said thank you. }`
  // on 2026-08-29 — the string matches JSON_RE as a whole and matches nothing
  // per-sentence.
  //
  // Excises only when EVERY key in the blob is a real parameter of a real
  // registered tool. Braces in prose ("the note said {see reception}") are not
  // evidence of anything.
  const paramSet = new Set(
    Array.isArray(ctx.toolParamNames) ? ctx.toolParamNames.filter(Boolean) : []
  );
  let source = text;
  let preExcised = false;
  if (paramSet.size) {
    source = source.replace(/[{[]([^{}[\]]*)[}\]]/g, (whole, blob) => {
      const keys = [...blob.matchAll(/(?:^|,)\s*["']?([A-Za-z_][A-Za-z0-9_]*)["']?\s*:/g)].map(
        (m) => m[1]
      );
  ...
  // Deliberately NOT seeded from preExcised: the loop below uses `changed` to
  // decide whether a short unspeakable sentence was excised down to nothing or
  // was simply short to begin with. Seeding it true would start dropping
  // two-word sentences ("Of course.") that nothing had touched.
  let changed = false;
  const kept = [];
  // WHICH rule tripped, for the log line below. Not the text it tripped on:
  // that text is the assistant's own speech and carries the caller's details.
  const rules = new Set();
  if (preExcised) rules.add("arg_blob");
  ...
  for (const sentence of sentences) {
    ...
    if (s !== sentence) {
      changed = true;
      rules.add("call_shape");
    }

    // Evaluated separately rather than short-circuited, so the log can say
    // whether a SHAPE gave it away or one of this tenant's own tool names did.
    // The two have different fixes -- one is the model inventing syntax, the
    // other is the model naming a real tool aloud -- and the counter they share
    // cannot tell them apart.
    const structural = HIGH_CONFIDENCE.some((re) => re.test(s));
    const registry = registryRes.some((re) => re.test(s));
    if (structural) rules.add("structural");
    if (registry) rules.add("registry");
    const tripped = structural || registry;
    ...
    if (!tripped && !speakable) { changed = true; rules.add("unspeakable"); continue; }
  ...
  if (!changed && !preExcised) return text;

  const out = kept.join(" ").replace(/\s+([,.;:!?])/g, "$1").replace(/\s{2,}/g, " ").trim();
  bumpCounter("internal_term_leaks");
  // See stripInternalTerms above for why this is a shape and not the text.
  log.error("outbound_sanitized", { chars: text.length, rules: [...rules], severity: "warn" });
...
export function toSpeakable(text, ctx) {
  try {
    ...
    out = spellUrls(out);
    out = spacePhoneNumbers(out, ctx?.profile);
