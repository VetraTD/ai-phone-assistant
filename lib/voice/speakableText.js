...
function groupPhoneDigits(digits, profile) {
  ...
  if (profile?.phone?.style === "uk" && digits.startsWith("44")) {
    ...
    if (/^07/.test(national) && national.length === 11) {
      return `${national.slice(0, 5)} ${national.slice(5)}`; // 07700 900123
    }
    if (/^0(20|23|24|28|29)/.test(national) && national.length === 11) {
      return `${national.slice(0, 3)} ${national.slice(3, 7)} ${national.slice(7)}`; // 020 7946 0958
    }
    if (national.length === 11) {
      return `${national.slice(0, 4)} ${national.slice(4, 7)} ${national.slice(7)}`; // 0161 496 0000
    }
    return national;
  }
...
function expandSymbols(text) {
  return text
    ...
    .replace(/£(\d+)\.(\d{2})\b/g, "$1 pounds $2")
    .replace(/£(\d+)\b/g, "$1 pounds")
    .replace(/€(\d+)\.(\d{2})\b/g, "$1 euros $2")
