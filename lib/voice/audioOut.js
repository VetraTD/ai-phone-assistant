 ...
 * @param {function(): void} [opts.onFirstFrameWire] - fired on the first media frame of each
 *   contiguous wire burst (see the paced-playout note below)
 * @returns {{
...
export function createAudioOut({
  ...
  // ---------------------------------------------------------------------
  // Queue-to-wire visibility.
  //
  // enqueue() is when audio becomes available; this is when Twilio actually
  // gets it. Pacing puts a deliberate gap between the two, and the turn
  // metrics stamp first_audio_sent at enqueue — so without a signal here the
  // pacing delay sits outside every latency number the server reports.
  //
  // Fires on the first frame of each contiguous burst rather than once per
  // turn, because this module has no concept of a turn. Consumers dedupe:
  // metrics.mark() keeps the first value per turn, so a mid-turn refire (TTS
  // arriving slower than realtime) is ignored while a new turn is measured.
  // ---------------------------------------------------------------------
  let wireIdle = true;

  function noteFrameOnWire() {
    if (!wireIdle) return;
    wireIdle = false;
    try {
      onFirstFrameWire?.();
    } catch (err) {
      log.error("audio_out_first_frame_cb_failed", { streamSid, reason: err?.message });
    }
  }
  ...
  function pump() {
    try {
      ...
      if (queue.length === 0) {
        stopPump();
        wireIdle = true; // burst over — the next frame starts a new one
      }
    }
  }
  ...
  function enqueue(mulawBuf, markName) {
    try {
      ...
      // Unpaced mode has no queue to drain, so one enqueue() call IS the
      // burst; re-arm here or the callback would fire once per call and
      // never again.
      if (!PACING_ENABLED) wireIdle = true;
  ...
  function clear({ fadeMs } = {}) {
    try {
      ...
      if (!wantFade) {
        ...
        wireIdle = true; // whatever plays next is a new burst
      ...
      outstandingMarks.clear();
      playingUntil = Math.max(now(), sentUntil);
      // After the ramp frames above, so the taper stays part of the burst it
      // is fading out rather than counting as a new one.
      wireIdle = true;
