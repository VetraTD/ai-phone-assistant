...
//
// 1200ms is derived, not guessed. docs/latency-and-tts-tests.md measured
// llm_ttfb_ms at p50 1836 / p95 2347 — a TIGHT distribution, p95 only 1.28x
// p50. VOICE_INTENT_MARKER then moved p50 to 940ms, which puts p95 at roughly
// 1200ms if the shape held. So the old 2000ms default sat ABOVE p95: it fired
// on almost nothing, which is why "noticeable pauses after callers finish
// speaking" reached the Digile Media owner uncovered. 1200 covers about the
// slowest 5% of turns and stays silent on the rest.
//
// The assumption worth re-checking against a fresh probe is that the p95/p50
// ratio survived the marker change. If the assistant starts saying "one
// moment" on turns it was about to answer anyway, this is why — raise it.
const SLOW_TIMEOUT_MS = envInt("VOICE_LLM_SLOW_MS", 1_200, { min: 200, max: 10_000 });
