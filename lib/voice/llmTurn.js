...
// How long the caller waits in silence before the session offers "One moment."
//
// This is PERCEIVED latency, not real latency, and worth being honest about:
// it does not make the model answer sooner, it stops the caller wondering
// whether the line dropped.
//
// 1200 was wrong and a live call proved it: "it keeps saying one moment before
// a lot of responses when it just does not need to". Two errors in that
// derivation, both worth keeping written down.
//
// First, the timer starts at `llm_request` (lib/voice/session.js), which is
// AFTER STT endpointing — so 1200 here is ~1750-1900ms of silence as the caller
// experiences it, against a measured true_v2v p50 of ~2600ms. It fired ahead of
// the answer on ordinary turns rather than covering the tail.
//
// Second, llm_ttfb_ms includes any tool round trip, so every booking (which
// checks availability) is far slower than the p50 the threshold was set from.
// The turns most likely to trip it were exactly the common ones.
//
// 2500 keeps it above the ordinary reply and leaves it covering genuine stalls.
// The duplicate-hold-line bug fixed alongside this (see shouldPlayHoldLine) was
// the larger half of the complaint; this is the timing half.
const SLOW_TIMEOUT_MS = envInt("VOICE_LLM_SLOW_MS", 2_500, { min: 200, max: 10_000 });
 ...
 * @yields {{type:"slow"}|{type:"stalled",sinceLastChunkMs:number}|{type:"delta",text:string}|{type:"toolCall",name:string,silent:boolean}|{type:"toolEffect",effect:object}|{type:"done",reply:object,[key:string]:*}}
 */
export async function* runLlmTurn({
  ...
  config,
  extras,
  firstChunkTimeoutMs = SLOW_TIMEOUT_MS,
  totalTimeoutMs = 8000,
  ...
  try {
    while (true) {
      ...
      if (chunk.delta) {
        ...
        //
        // `refused` rides along for the same reason: a call the gates turned
        // away before execution did no work either, and a hold line in front of
        // it describes something that is not happening. Kept as a flag rather
        // than filtered here, because the turn still needs the toolGrace and
        // the metrics — it is only the ANNOUNCEMENT that has to stay quiet.
        if (chunk.toolCall.name !== "set_call_intent") {
          yield {
            type: "toolCall",
            name: chunk.toolCall.name,
            silent: !!chunk.toolCall.silent,
          };
        }
      }
