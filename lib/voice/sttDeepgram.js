...
// ---------------------------------------------------------------------------
// Deepgram streaming STT — the UNCOVERED half of the STT seam.
//
// Reached through sttStream.js, which picks a provider by compliance tier.
// This one serves the UK lane and any `standard` deployment: GDPR has no
// covered-products restriction and an Art. 28 DPA suffices, so the UK keeps
// the vendor this pipeline's turn-taking was tuned against. It is NOT used in
// `hipaa` mode, where no BAA covers Deepgram and sttGoogle.js runs instead.
//
// Extracted from lib/voice/sttStream.js unchanged apart from its name; that
// file is now the selector. It differs from the older services/deepgram.js
// (deleted 2026-08-21) in two ways:
...
//
// Utterance handling follows the original services/deepgram.js: is_final fragments
...
/**
 * Which Deepgram region this process talks to.
 *
 * `api.eu.deepgram.com` is NOT an account flag — it is a different base URL,
 * and the same API keys work against it. That distinction cost a wrong entry
 * on the owner's task list: it is an engineering change, not something to ask
 * Deepgram for.
 *
 * Audio sent to the US endpoint is processed in the US, whatever the DPA says.
 * So the UK stack has to point here, and DEEPGRAM_REGION is the switch — set
 * per Cloud Run service in B4, alongside the region's other configuration.
 *
 * Deepgram guarantees processing stays within the EU but does not pin a
 * COUNTRY, and it may move. If a fixed country is ever required — a clinic
 * contract that names one — that is Deepgram Dedicated, not a config change.
 *
 * Whisper models are unavailable on the EU endpoint. Irrelevant here (nova-3),
 * and written down so a future model swap does not discover it in production.
 *
 * @returns {{base: string, agent: string, production: string}}
 */
export function deepgramEnvironment(env = process.env) {
  const region = (env.DEEPGRAM_REGION || "us").trim().toLowerCase();
  if (region === "eu") {
    return {
...
function buildConnectOptions(
  ...
  const options = {
    ...
    vad_events: true,
    // Opt OUT of Deepgram's Model Improvement Partnership on every single
    // request.
    //
    // Without it, caller audio is eligible to be retained and used to train
    // Deepgram's models. On a healthcare line that is a disclosure nobody
    // consented to and nobody can take back — and it was the default, silently,
    // for the entire life of this system until 2026-08-21.
    //
    // Set HERE rather than at a call site, because there is exactly one place
    // that builds these options and a per-call-site version would be one
    // forgotten argument away from opting a tenant back in. Deepgram has also
    // been asked to disable MIP at the ACCOUNT level, which is the version that
 ...
 *
 * Reached through sttStream.js, which chooses the provider by compliance tier.
 * The A6 refusal below stays here rather than moving to the selector: a guard
 * at the constructor cannot be routed around, and a guard at the selector can.
 *
 * @param {object} opts
 ...
 */
export async function createDeepgramSttStream({
  language = "en-US",
  endpointing,
  keyterms = [],
  callSid,
  ...
  // A6. Before the key is even read: in `hipaa` mode there should be no key to
  // read, and if there is one, the project isolation the whole two-region
  // design rests on has already been breached.
  assertVendorAllowed("deepgram");
  ...
  async function connectSocket() {
    const client = new DeepgramClient({ apiKey: DEEPGRAM_API_KEY, environment: deepgramEnvironment() });
