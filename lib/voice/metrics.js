...
const COUNTER_NAMES = [
  ...
  // A claim the one-turn look-back called unbacked, which a write EARLIER IN
  // THE SAME CALL had already made true. LVX115.
  //
  // On 2026-09-11 a booking succeeded and the assistant described it correctly
  // two turns later. The guard could not see that far back, so it fired, the
  // ladder asked the caller to confirm a time they already had, and the note
  // then made the assistant say "the appointment wasn't booked yet" about a row
  // that existed.
  //
  // Read against live_claim_without_action: the difference between them is how
  // often that guard is firing on sentences that were true all along, which is
  // a number LVX93 has wanted since it was filed.
  "live_claim_backed_this_call",
  // THE END-OF-CALL SWEEP. LVX120.
  //
  // The mid-call completion fires on a detected CLAIM, which means matching how
  // the model phrased it -- an unbounded question the model keeps winning. Four
  // phrasings in two weeks, a caller harmed each time. The sweep asks a bounded
  // one instead: which of the times an availability call actually returned did
  // the call name? Read against claim_completed_in_code: the difference is the
  // population the phrasing-based path cannot see.
  "sweep_booked_at_close",
  // The sweep ran and the tool layer said no -- a requiredField the tenant set,
  // the slot gone, a name the pack refused. The gates working, not the sweep
  // failing.
  "sweep_refused",
  // A slot was named and nothing was written, and no name could be recovered or
  // verified. THE ONE TO WATCH: if it climbs while sweep_booked_at_close stays
  // flat, the provenance check is refusing the callers this is meant to serve,
  // which is LVX119's shape and exactly what nameSpokenIn is documented as doing
  // four times in five.
  "sweep_declined_no_name",
  // The caller said no in so many words, so nothing was written. The only thing
  // that stops the sweep on the caller's own authority.
  "sweep_declined_caller_said_no",
  // The sweep threw. Never expected; the call is already over when it runs, so
  // this costs the booking and nothing else.
  "sweep_failed",
  // A sentence naming an appointment the caller ALREADY HAD, demoted from a
  // booking claim to an unspecified one. LVX120.
  //
  // "I see you have an appointment scheduled for Monday at 4 30 PM" is read
  // straight off a lookup and is entirely true. It was being counted as an
  // unsatisfied booking claim, which is the false alarm LVX57 warns teaches
  // readers to ignore the ledger -- and on 2026-09-11 it produced the RIGHT
  // verdict through the WRONG sentence while a real fabrication went unseen.
  "live_claim_reported_existing",
