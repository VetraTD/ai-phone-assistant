...
const COUNTER_NAMES = [
  ...
  // A read answered from the per-turn memo instead of re-running the tool.
  // Measured 2026-09-16 on two real calls, one per model, with the tool
  // returning in 4-101 ms every time: check_appointment_availability three
  // times in one call on gemini-3.8-live and five times on gemini-3.1. Both
  // models re-fire, so this is not the 3.8-only defect it was filed as.
  //
  // READ `availability_day_listed` AS A DIFFERENT NUMBER FROM THIS REVISION
  // ONWARDS. It accumulates per SLOT, and every re-fire was adding its whole
  // list again -- 38 slots and 73 slots on those two calls. Suppressing the
  // repeat suppresses the double count with it, so the series steps down at
  // this deploy for a reason that is not a change in caller behaviour.
  "live_guard_read_memo_hit",
  ...
  "live_tool_refusals",
  // ---------------------------------------------------------------------
  // THE HAMMERING BRAKE, 2026-09-17. An identical action call, already
  // refused, with nothing the gate reads having changed, answered from the
  // previous refusal instead of re-running the gate stack.
  //
  // It exists because nothing bounded a model retrying inside one caller turn:
  // refused writes are never cached, both refusal budgets decline to increment
  // within a turn, and MAX_TOOL_ROUNDS counts MODEL turns, which 3.8 ends a
  // median 17 ms after a tool call. CA03558d fired five book_appointment calls
  // in 2.3 seconds and was capped three times.
  //
  // Read it as "loop detected and absorbed". A large number on a call that
  // booked correctly means the model is arguing with a gate it has already
  // lost to; a large number beside a LOST booking means the first refusal was
  // the wrong one, and that is the one to go and read.
  // ---------------------------------------------------------------------
  "live_write_refusal_replayed",
  ...
  // The claim guard declined to speak because the turn was a SIGN-OFF
  // RESTATEMENT of something this call genuinely completed.
  //
  // CAbef3df47, 2026-09-12: a real booking at 22:42:06, restated at the goodbye
  // at 22:42:37, and the guard called it a lie because its look-back is one turn
  // and the only tool on that turn was end_call. Measured exposure at the time:
  // 9 of 64 production calls end on a claim-shaped turn.
  //
  // Counted rather than silently skipped, because a suppression nobody can see
  // is indistinguishable from a guard that never ran -- the same trap
  // recover_declined was added to close. Read it against
  // live_claim_unbacked_by_action: the difference is what the model was actually
  // told about.
  "live_claim_signoff_restatement",
  // The turn ALSO asked the caller to approve it, so it was a read-back rather
  // than an assertion. Nine of 34 production firings. "I have you down for one
  // p m on September seventeenth" is a phrasing LVX94 added to the claim
  // detector because it was a real fabrication -- and a read-back says the same
  // words and then asks a question, which the detector cannot see.
  "live_claim_readback_not_claim",
  // The turn described an appointment the caller ALREADY HAD. Seven of 34.
  // Rule 2 of the judge's own prompt, finally applied to the live guard.
  "live_claim_existing_report",
  ...
  "live_claim_detected",
  // The same, under the WIDER predicate that LVX97 forced.
  //
  // "We're all set for your free consultation on Wednesday, September ninth at
  // one in the afternoon" -- said on call 156fb2 with book_appointment never
  // called and no row before or after. `you're all set` was covered here and
  // `we're all set` was not, so a fabricated booking produced zero claim events
  // of any kind on a fourteen-turn call, and the post-call ledger was blind to
  // it for the same reason.
  //
  // Drives the ledger and the reconciliation. It does NOT drive the turn note:
  // widening what the model is told mid-call changes what every later call
  // measures, and this number is what decides whether that is safe.
  "live_claim_detected_wide",
  // The DIFFERENCE, which is the whole point of shipping two.
  //
  // Exactly the turns the extra alternations added. Read against
  // live_claim_detected: if this fires on ordinary turns where nothing is
  // wrong, that is the widening being too greedy, and it says so before any
  // caller has heard a note about it.
  "live_claim_wide_only",
  ...
  // THE AVAILABILITY VERDICT, which nothing has ever recorded.
  //
  // `respond()` in capabilities/appointments.js returns `success: true` from
  // every branch of check_appointment_availability -- including the one meaning
  // "that time is taken" -- so tool_duration's success flag says nothing about
  // whether the slot was open. There was no log line and no counter carrying
  // `available` anywhere in the tree, which makes any claim about which time a
  // past call verified unfalsifiable. Found while trying to establish what
  // CA422f58 had checked; it cannot be answered, for that call or any other.
  //
  // Three names rather than one, because they are three different facts:
  //
  //   _point_open    a check for ONE time came back open. The only evidence that
  //                  ties a verified slot to the time actually asked about.
  //   _point_taken   a check for one time came back TAKEN. The same response
  //                  carries up to three `alternatives` that the model is told
  //                  to offer, so this is the moment a call stops having one
  //                  candidate time and starts having several.
  //   _day_listed    a whole-day query returned a list. Counted per SLOT, so a
  //                  day with sixteen openings reads sixteen -- the difference
  //                  between a caller browsing and a caller agreeing.
  //
  // Reported, never gated on: the booking invariant still reads the combined
  // verifiedSlots set, so no write's outcome moves.
  "availability_point_open",
  "availability_point_taken",
  "availability_day_listed",
  // A write that landed on a slot nothing ever point-checked.
  //
  // The availability invariant accepts any slot an availability response put on
  // the record, including the three `alternatives` that accompany a "that time
  // is taken" verdict -- which is correct, because that is how a caller picks
  // from times they were offered. But it means the written time can be one the
  // caller was SHOWN rather than one they were ASKED about, and that is the shape
  // in which a wrong time reaches the database with every guard passing. Nothing
  // has ever measured how often it happens.
  "write_landed_on_listed_slot",
  // The consent probe. Measurement only -- no write's outcome depends on these.
  //
  // _skipped_silent_turn: an action tool reached the consent machinery on a turn
  // whose caller text was empty, so every consent check was skipped. The nesting
  // that causes it is what keeps the cascade byte-identical, and on Live it means
  // an unchecked write whenever the caller's turn transcribed as nothing. This is
  // the number that says how often that actually happens on real calls, which
  // nothing has ever measured.
  //
  // _token_present: a durable agreement record existed at the moment of the
  // attempt -- the caller had affirmed a read-back earlier in the call. Paired
  // with the first so that "skipped AND no agreement ever" can be told apart from
  // "skipped but they agreed a turn ago", which is the distinction that decides
  // whether the hole is dangerous or merely untidy.
  //
  // _agreement_recorded: bumped by the engine when it writes a token at all. A
  // zero here with non-zero attempts means the ledger is not wired, visible on
  // call one rather than call nine.
  "write_consent_skipped_silent_turn",
  "write_consent_token_present",
  // The consent latch releasing a write the turn-local checks would have
  // refused -- a booking CAf1d6447d34 would have lost. Counted only where it
  // changes the outcome, so it reads as "bookings saved", not "agreements".
  "write_consent_latch_held",
  // A "yes" that was still the caller's most recent word, but answered a
  // read-back the model has since replaced. Counted where it fires, because a
  // non-zero here is a write the old gate would have allowed.
  "write_consent_agreement_superseded",
  // ---------------------------------------------------------------------
  // AN AGREEMENT SPENT BY THE ACTION IT AUTHORISED, 2026-09-17. Two counters
  // because the token is read in two places and only one of them is the latch.
  //
  // _latch_spent: the consent latch declined to carry a token because an
  //   action tool had already completed on this call. Cancel-then-book is the
  //   shape: the cancellation's read-back is still standing, still names the
  //   time, and would otherwise have re-booked the slot the caller had just
  //   asked to be rid of.
  // _token_spent: the same rule at the silent-turn verdict, which matters more
  //   because the whole gate stack is nested inside "the caller said
  //   something" -- a surviving token there skips the write-order gate, the
  //   latch, the spelling gate and the provenance check together.
  //
  // Both read as "writes this rule refused". A non-zero pair on a call that
  // booked correctly is not a fault; a non-zero pair beside a lost booking is
  // where to look first.
  // ---------------------------------------------------------------------
  "write_consent_latch_spent",
  "write_consent_token_spent",
  "consent_agreement_recorded",
  // LVX117, and the only one of these that gates anything. Bumped when a write
  // was refused because the caller's turn carried no speech AND no agreement
  // token exists anywhere on the call -- CA422f58's shape, where a cancellation
  // committed against a question the caller had not yet answered.
  //
  // Read it against _skipped_silent_turn, never alone: this counter is the
  // subset of those attempts where the caller had agreed to NOTHING. The
  // complement (skipped, but a token existed) is the benign shape and is
  // deliberately still allowed through. And because a clean call bumps neither,
  // the probe carries silent_turn_verdict on every attempt so that "refused
  // nothing" and "this build never served the call" stay distinguishable.
  "write_refused_no_consent_silent_turn",
  // LVX120. A turn the CALL asked for (speakLine: a silence nudge, the silence
  // goodbye) completed, and lastReplyText was held at the real reply underneath
  // it instead of being overwritten with the canned line.
  //
  // The positive half of a fix whose absence was invisible: on CAfc89ebd3 the
  // caller said "Yes." three times to a read-back and consent_agreement_recorded
  // stayed 0, because each yes was paired with "I'm still here whenever you're
  // ready." Read this against consent_agreement_recorded -- a call with nudges
  // and still no agreement recorded is the bug not fixed.
  "reply_held_over_spoken_line",
  // THE RECOVERY, lib/postCallRecover.js. A booking the call agreed to and never
  // recorded, made after the call from the times the calendar had confirmed open.
  //
  // Read as a funnel, because every rung of it declining is a legitimate outcome
  // and only the last two are faults:
  //   _booking_owed        the reader says a booking was agreed and no row exists
  //   _select_chose        it picked one of the confirmed-open times
  //   _select_declined     it refused to pick: ambiguous, or not in the set
  //   _select_out_of_range it answered with an index that does not exist
  //   _booked              a row now exists
  //   _slot_unavailable    the time went between the call ending and this running
  //
  // _booked_without_name is not a fault either: the name is taken only from a
  // booking this caller previously completed and is otherwise left null, because
  // LVX77 wrote an invented name into a row and the phone number does the
  // identifying anyway.
  "recover_booking_owed",
  "recover_select_chose",
  "recover_select_declined",
  "recover_select_out_of_range",
  "recover_select_failed",
  "recover_no_selection",
  "recover_no_candidate_slots",
  "recover_skipped_no_slots",
  "recover_transcript_too_short",
  "recover_no_transcript",
  "recover_row_read_failed",
  "recover_skipped_row_exists",
  "recover_skipped_never_agreed",
  "recover_book_failed",
  "recover_slot_unavailable",
  "recover_booked",
  "recover_booked_without_name",
  // A booking was owed by the structural facts of the call, and whether a row
  // exists for it. Prose-free: a point availability check came back OPEN for a
  // time the caller was asked about, and the caller affirmed a read-back.
  //
  // The pair is the point. reconcile() returns ONE ordered verdict and cannot see
  // this case at all -- on call CA422f58 the cancellation satisfied "did this call
  // write anything", so a caller who agreed to a booking that never happened was
  // reported `ok`. _owed is the denominator; without it _owed_no_row reading zero
  // would mean "nothing was ever owed" and "the check is not wired" at once.
  "postcall_booking_owed",
  "postcall_booking_owed_no_row",
  // THE SHADOW JUDGE, lib/postCallJudge.js. An LLM reading the finished
  // transcript, asked the same question postcall_booking_owed_no_row answers
  // structurally. It acts on nothing; these exist so the two can be COMPARED on
  // real calls, which is the only way either is validated without hand-labelling.
  //
  // _ran is the denominator and is not optional: _booking_missing reading zero
  // would otherwise mean "no call owed a booking", "the judge is switched off"
  // and "the judge is broken" all at once -- the fault-only-counter mistake this
  // file has paid for before.
  //
  // Where it should beat the structural check: a caller who picks a time off a
  // whole-day list, which leaves no point-verified slot for the structural check
  // to stand on. On the 2026-09-11 verification call that was 16 listed slots
  // against 2 point-checked.
  "postcall_judge_ran",
  "postcall_judge_booking_agreed",
  "postcall_judge_booking_missing",
  "postcall_judge_failed",
  "postcall_judge_skipped",
  // A post-call confirmation withheld because the caller already had one from the
  // booking-time sender, and because a verdict impugned the row. Both restored
  // from 68f0585. Counted rather than silent: without these, POSTCALL_VERIFY=send
  // double-texting and send working correctly look identical from the outside, and
  // `sent: 1` meant the bug rather than the feature.
  "postcall_confirm_skipped_already_confirmed",
  "postcall_confirm_skipped_verdict",
  // The neutral message for a booking the call owed and never wrote. It goes to
  // the number that RANG US -- a narrow exception to this module's own rule about
  // that number, taken because a booking with no row has no client_phone to read.
  // _skipped_no_number is the denominator that says how often even that is
  // unavailable.
  "postcall_pending_sent",
  // ---------------------------------------------------------------------
  // LVX122, closed 2026-09-17. `sent` on a postcall_verify line counted
  // ATTEMPTS: sendCallerSms returned undefined whether it delivered or took
  // one of five early returns, and the array was pushed unconditionally. On
  // CA6b773e2a the caller had declined, the consent gate blocked all three
  // sends, and the line still read sent=2.
  //
  // These two are the other half of the ledger. sent + not_sent is the number
  // of sends attempted, which is what the field used to mean on its own.
  // ---------------------------------------------------------------------
  "postcall_confirm_not_sent",
  "postcall_pending_not_sent",
  // A CANDIDATE, COUNTED RATHER THAN ACTED ON. The caller agreed, the calendar
  // had LISTED open times but never point-checked one, and no row exists --
  // which is CA03558d's shape and reads as booking_owed=FALSE today. The rule
  // that excludes it says why (browsing looks identical), and that reason is
  // not overturned by one call, so this measures the rate instead of changing
  // the gate. See lib/postCallVerify.js.
  "postcall_booking_owed_listed_only",
  // An abandoned write with no row behind it, escalated. reconcile() returns
  // one ordered verdict and write_abandoned is tested BEFORE claim_without_row,
  // so CA03558d -- six refused booking attempts, zero rows, and a caller told
  // the appointment was confirmed -- raised nothing at all.
  // LVX144. The caller answered a read-back by naming the slot back instead of
  // saying yes, and that now counts as agreement. Bumped only when the older
  // word-level test would have said no, so this IS the population the change
  // added -- the number to watch if it starts writing things people were only
  // thinking about.
  "write_consent_restated_slot",
  "postcall_abandoned_no_row",
  // The same situation without the row conjunct, LVX142. `abandoned` already
  // excludes any tool that later succeeded, so the extra "and no rows exist"
  // test only ever suppressed -- on CAd978554 an unrelated booking vouched for
  // an abandoned reschedule and nobody was told. This is what the escalation
  // now acts on; the narrower counter above it stays, and the gap between the
  // two is exactly what the widening added.
  "postcall_abandoned_outstanding",
  // LVX147. The phrase matcher said no claim and the LLM judge said there was
  // one. Exactly the population consulting the judge added: zero forever means
  // it bought nothing, and a high rate means the matcher is further behind than
  // the three calls that prompted this suggested. Read against
  // postcall_judge_ran, which is the denominator.
  "postcall_claim_judge_only",
  // The verification gave up waiting for the judge and decided on the matcher
  // alone. Counted because a silently degraded second reader is indistinguishable
  // from one that agreed -- and if this ever runs hot, the wait is the wrong
  // length or the judge is the wrong thing to block on.
  "postcall_judge_wait_timeout",
  "postcall_pending_skipped_no_number",
  "postcall_pending_failed",
  // A CALLER TURN THAT READ AS THE WRONG LANGUAGE. Count only; nothing acts.
  //
  // live_unusable_transcript catches non-Latin script and structurally cannot
  // catch fluent French, which is what cost two calls on 2026-09-12: an English
  // caller asking to book arrived as "Je pense que c'est une strategie pas".
  //
  // There is no fix at the source. The Live API exposes no way to constrain or
  // read the language of caller transcription -- AudioTranscriptionConfig is an
  // empty interface in the SDK, and the session's only languageCode sits in
  // speechConfig, which governs the voice the model SPEAKS, not what it hears.
  // So `language_pinned: true` has never meant what it reads as, and this pair
  // measures the rate instead of guessing at it.
  //
  // _checked is the denominator and English-session turns only.
  "live_caller_turn_language_checked",
  "live_caller_turn_non_english",
  // A READ-BACK THAT ASKED MORE THAN ONE THING, at the moment a write depends on
  // the answer to it.
  //
  // live_stacked_questions already counts this per turn and says nothing to the
  // model, for reasons recorded where it fires: seven prompt instructions demand
  // one question per turn, the tenant's own config demands it in those words, and
  // it still happens twice a call. What that counter cannot say is how often the
  // stacking lands on the turn CARRYING THE CONSENT -- "Monday at two? And can I
  // take your email?" answered "Yeah" is an agreement attributable to neither
  // half, and the gate attributes it to the write. This pair is that rate.
  //
  // _checked counts read-backs examined, so a zero in _ambiguous cannot mean
  // "the check never ran". Measurement only; nothing is refused on it.
  "write_consent_readback_checked",
  "write_consent_readback_ambiguous",
  ...
  "live_close_abnormal",
  // -------------------------------------------------------------------------
  // LVX131. THE CALL THAT NEVER ENDED, AND THE WATCHDOG THAT NOW ENDS IT.
  //
  // CAdfeb9d's media socket died mid-sentence with no stop, no close and no
  // error, so finish() never ran: no summary, no postcall_verify, no
  // escalation, on a call with a refused booking attempt on it. Every timer in
  // the engine is driven by incoming media frames, so a severed socket freezes
  // the session rather than tearing it down.
  //
  // _armed is the positive twin and is not optional here. _timeout reads zero
  // for an estate where every call closed cleanly AND for one where the
  // watchdog was never wired at all -- and this repository has shipped a guard
  // in the second state before. The two together say which.
  // -------------------------------------------------------------------------
  "live_media_watchdog_armed",
  "live_media_timeout",
  // -------------------------------------------------------------------------
  // LVX137. A TIME THE SYSTEM LISTED AS OPEN, SPOKEN TO THE CALLER AS TAKEN.
  //
  // CA6df19e98: sixteen slots verified, one availability call, read_memo_hit 0,
  // and "I'm sorry, 2:00 PM is not available" on a Friday with zero scheduled
  // appointments in the diary.
  //
  // This is the mirror of every write gate here. Those stop the assistant
  // claiming a booking that does not exist; nothing watched it claiming a slot
  // is taken when it is free -- which leaves no row, no refusal and no counter,
  // so a caller pushed off their preferred time is invisible.
  //
  // _spoken is the denominator and the positive twin: how often the assistant
  // turns ANY time down. _verified_open is the subset where the time it turned
  // down is one an availability response itself put on the record as open.
  // Read as a ratio. _spoken staying 0 means the detector never fires at all,
  // which is a different fact from "it never got this wrong" -- and the
  // phrasing half is fitted to a single observed sentence, so it under-reports
  // by construction.
  // -------------------------------------------------------------------------
  "availability_denial_spoken",
  "availability_denied_verified_open",
  // -------------------------------------------------------------------------
  // THE ASSISTANT ANSWERED AN ENGLISH CALLER IN ANOTHER LANGUAGE.
  //
  // Three calls in eighteen by 2026-09-17 -- CAd48d7b in Spanish, CAfcd28789
  // reported as French, CAdc602f in Spanish again -- and every one was reported
  // by a human, because nothing here watched what the assistant SAID. The
  // sessions were opened with language_code en-US and output_language_pinned
  // true throughout: that pin governs the voice, not the words.
  //
  // live_caller_turn_non_english is NOT this counter's twin and cannot stand in
  // for it. That one reads the caller's turn and excludes Spanish on purpose,
  // because a Spanish-speaking caller is legitimate. Both decisions are right
  // and together they leave the real defect unobserved.
  //
  // _checked is the denominator: every assistant turn on an English-only
  // tenant. Read the fault against it -- a rate per turn is the number that
  // says whether this is a curiosity or the main thing wrong with the product.
  // -------------------------------------------------------------------------
  "live_assistant_turn_language_checked",
  "live_assistant_turn_non_english",
  // -------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  // THE TWO WAYS A CALL ENDED WRONG ON 2026-09-17, counted so the rate is
  // readable rather than inferred from a complaint.
  //
  // _held_question: the model called end_call and its own closing turn ended
  //   in a question to the caller, so the exit was held for a turn.
  //   CA2556d43d asked "Is there anything else I can help you with today?"
  //   and dropped the line 1.6 s later. Read this against
  //   live_exit_arm_checked: a high ratio means the model is routinely trying
  //   to hang up mid-question and the prompt is the thing to fix, not this.
  // _note_suppressed_exiting: the zero-text nudge declined to ask the model
  //   to speak because the call was already closing. On CAaef5bd82 the nudge
  //   and the exit landed 1 ms apart and the caller heard a sentence start
  //   and get cut off.
  // ---------------------------------------------------------------------
  "live_exit_held_question",
  // _held_in_turn_ask: LVX141, the seam between the two closing guards.
  //   CA94f2b4 asked "Is there anything else I can help you with today?" and
  //   said "have a great day" in the same breath, so LVX132's ask gate allowed
  //   the hang-up on the strength of the question and _held_question missed it
  //   because the reply ends with the farewell. This counts the hold that was
  //   added for that shape: the ask existed ONLY in the turn that was closing.
  //   Read against live_exit_held_question -- both are bumped when both apply,
  //   so the older series keeps its meaning and the difference is exactly what
  //   this added. Expect it against live_repeated_phrase too: a held turn is
  //   another chance for the model to sign off, which is LVX136's cost.
  "live_exit_held_in_turn_ask",
  // _held_unanswered_ask: LVX145, the third closing shape. CA06cadea6 asked
  //   "is there anything else?", waited six seconds, and hung up on a caller
  //   who was mid-syllable. LVX132's latch had already recorded that they were
  //   ASKED and is never reset, so nothing downstream knew the question was
  //   still outstanding. Counts the hold added for that: asked on an earlier
  //   turn, and the caller's transcript has not grown since.
  "live_exit_held_unanswered_ask",
  // _goodbye_exit_held_no_ask: LVX146, the same rule at the OTHER door. Every
  //   guard for closing a call sat on end_call; CA299f23ce never called it. It
  //   said "That is all booked. Thanks for calling Digile Media, and have a
  //   great day", the sign-off detector armed the exit directly, and the line
  //   dropped with booked_rows 0 and the caller never asked anything. Read this
  //   against live_goodbye_armed_exit: the ratio says how often the model tries
  //   to leave by the back door.
  "live_goodbye_exit_held_no_ask",
  "live_zero_text_note_suppressed_exiting",
  ...
  // -------------------------------------------------------------------------
  // LVX132. DO NOT HANG UP WITHOUT ASKING, and its two witnesses.
  //
  // Measured over ten calls of 2026-09-16/17 that reached end_call: 0 of 3
  // asked when end_call came before the confirm step, 4 of 6 when it came
  // after. The instruction to ask arrives at the END of the turn -- on
  // CAb08e4e, five seconds after the line had already dropped.
  //
  // _ask_check_ran is the positive twin and is bumped on every hang-up the
  // existing four gates would have allowed. _would_refuse_no_ask counts the
  // SITUATION whether or not the one-shot refusal is still available, so the
  // two diverging is the only way a second hang-up attempt on a call that was
  // never asked becomes visible.
  //
  // _refused_no_ask USED TO BE CAPPED AT 1 PER CALL and is not any more: LVX157
  // scopes the one-shot to a completed action rather than to the whole call.
  // Two refusals now mean two separate pieces of work, and a series compared
  // across 2026-09-19 has to know that.
  // -------------------------------------------------------------------------
  "end_call_ask_check_ran",
  "end_call_would_refuse_no_ask",
  "end_call_refused_no_ask",
  // -------------------------------------------------------------------------
  // LVX157. IT SAYS GOODBYE THE MOMENT THE JOB IS DONE.
  //
  // Twelve turns in call-corpus/ complete an action and sign off in one breath;
  // eleven never ask. The gates above could not see most of them: the ask latch
  // was satisfied by a question asked before the work existed (CA0ef8d221), or
  // the one-shot refusal had been spent earlier on unrelated work (CA84dc64).
  //
  // Read as a group. In the order they fire on a call:
  //
  //   _signoff_without_ask   THE DEFECT ITSELF, per turn, independent of every
  //     guard. It is bumped whether or not anything then refused, held or armed,
  //     so it stays comparable across changes to any of them. If a fix works
  //     this is the number that falls; if only the refusal counters move, the
  //     model is being caught rather than corrected.
  //   _ask_note_sent         Layer A went out with a completed write.
  //   _ask_note_honoured     ...and the model then actually asked. The positive
  //     twin, because an instruction that is read and ignored produces exactly
  //     the same _sent count as one that works, and this area has six failed
  //     wordings on record.
  //   _ask_latch_stale       a no_ask refusal that happened ONLY because the
  //     ask predated the write. Separates "the fix is catching CA0ef8d221" from
  //     "the model has started asking less often", which need opposite answers.
  //   _ask_rearmed_by_action a second no_ask refusal on a call, which can only
  //     happen when a write landed after the first. A gate that had simply
  //     stopped latching would show refusals WITHOUT this.
  // -------------------------------------------------------------------------
  // LVX161. A ceiling release that happened only because the proposal was read
  // back on an EARLIER turn than the one being refused.
  //
  // Reading "was a read-back made" per TURN inverted the gate: an apology in
  // reply to a refusal aged the read-back out, the flag went false, and the
  // ceiling ROSE from one to two. The model apologising bought the gate another
  // refusal against the caller. CAad88df4a took four before a cancellation went
  // through, with two "the system isn't working" sentences in between.
  //
  // Read against write_order_gate_ceiling: this is the subset of releases the
  // per-turn reading would have missed, and a zero here after the fix means
  // either the shape has stopped happening or the history is not being kept.
  "write_order_ceiling_kept_by_history",
  "live_completion_signoff_without_ask",
  "live_completion_ask_note_sent",
  "live_completion_ask_note_honoured",
  "live_ask_latch_stale_at_action",
  "end_call_ask_rearmed_by_action",
  ...
  "write_retry_refused",
  // A retry a GATE held, as opposed to one the operation could not satisfy.
  // Separate from write_retry_refused because the two ask different questions:
  // this one rising means the retry is firing at a moment the gate cannot
  // pass -- on CAa08fc3 it fired the instant a spelling settled, when the
  // caller had recited letters and agreed to nothing -- while
  // write_retry_refused rising means the diary genuinely could not take it.
  // Lumped together they read as one number that answers neither.
  "write_retry_gated",
