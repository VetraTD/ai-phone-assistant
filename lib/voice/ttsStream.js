...
export function createTtsTurn({
  ...
  // -------------------------------------------------------------------------
  // BILLING UNITS. Both vendors charge per CHARACTER SENT, not per character
  // heard, so these are counted at the send site and never derived from the
  // transcript afterwards. A barge-in cuts the audio, not the bill: text
  // already handed to ElevenLabs was synthesized and charged for whether or
  // not the caller ever heard it, and a transcript-derived count silently
  // under-reports exactly those turns.
  //
  // Reported by finishDone() as `tts_turn_chars` so scripts/call-report.js can
  // total a call without this module knowing anything about money.
  // -------------------------------------------------------------------------
  let elChars = 0;
  let googleChars = 0;
  ...
  function finishDone() {
    ...
    // Emitted before onDone so a throwing handler cannot swallow it.
    reportChars();
    try {
      onDone?.(payload);
    } catch (err) {
  ...
  // Idempotent: a turn can reach this from finishDone() or from abort(), and
  // on a barge-in racing completion it can reach it from both.
  let charsReported = false;
  function reportChars() {
    if (charsReported) return;
    charsReported = true;
    if (elChars === 0 && googleChars === 0) return;
    log.info("tts_turn_chars", { callSid, el_chars: elChars, google_chars: googleChars });
  }
  ...
  async function runFallback() {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      // Charged on the request, so counted here rather than after the
      // suppression check below — a barge-in that discards this buffer does
      // not refund it.
      googleChars += chunks[i].length;
  ...
  function abort() {
    ...
    ended = true;
    conn?.close();
    // A barge-in aborts the turn WITHOUT going through finishDone, and the
    // characters already handed to ElevenLabs were still synthesized and still
    // billed. Reporting only from finishDone would under-count precisely the
    // turns this accounting exists to make honest.
    reportChars();
  }
