...
export function createUtteranceCache({ synthesize } = {}) {
   ...
   *
   * The synthesis backend defaults to the one this instance was created with
   * (Google TTS, in the pipeline), but a per-call override can be passed so
   * the SAME shared LRU also holds a business's ElevenLabs-voiced
   * micro-utterances, keyed by that voice — an EL business warms `voiceKey =
   * its EL voiceId` with an EL synthesizer, a Google business warms its Google
   * voice with the default. get() reads whichever is keyed for the voice.
   * @param {string} voiceKey
   * @param {Array<{kind?: string, text: string}|string>} entries
   * @param {{synthesize?: function(string, string): Promise<Buffer>}} [opts]
   */
  async function warm(voiceKey, entries, opts = {}) {
    const synth = typeof opts.synthesize === "function" ? opts.synthesize : synthesize;
    if (typeof synth !== "function" || !Array.isArray(entries)) return;
    for (const entry of entries) {
      ...
      try {
        const buf = await synth(text, voiceKey);
        if (buf && buf.length) setEntry(voiceKey, text, buf);
      } catch (err) {
