...
export function createEchoGuard({
  ...
  // minTokens is published because it is a CONTRACT, not an internal detail:
  // it is the length below which classify() cannot answer at all, so every
  // caller that reasons about short transcripts has to know where that floor
  // sits. turnManager used to hardcode the same 4 and stay in step only by
  // comment, which meant raising VOICE_ECHO_MIN_TOKENS silently opened a band
  // of transcripts nothing was echo-checking.
  return { noteSpoken, noteAudioStopped, classify, isEcho, isShortEcho, reset, minTokens };
}
