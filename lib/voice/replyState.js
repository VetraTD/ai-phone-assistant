...
/**
 * How much voiced audio counts as the caller having ANSWERED.
 *
 * Chosen against the episodes on call CA9e3788, where the real answers ran
 * 1,920-3,800 ms and the noise ran 60-80 ms. Low enough to catch "yeah, how it
 * sounds"; high enough that a cough is not an answer.
 */
export function voicedAnswerMs() {
  const v = Number.parseInt(process.env.VOICE_ANSWER_MS, 10);
  return Number.isFinite(v) && v >= 0 && v <= 5000 ? v : 400;
}

/**
 * The caller answered the spelling question and the transcript could not carry
 * it. Spend one of their chances anyway.
 *
 * ---------------------------------------------------------------------------
 * WHY THIS EXISTS, AND WHY IT IS NOT KEYED ON TEXT
 * ---------------------------------------------------------------------------
 *
 * `applyCallerSpellingSignal` above opens with `if (!userText) return`. On call
 * CA9e3788 the caller spelled their name into 2,180 ms of audio that
 * transcribed to ZERO characters, and 2,900 ms that transcribed to four. That
 * early return meant the turn recorded neither the spelling nor a miss — so
 * `spellingSettled` could never become true, and `spellAskMisses`, the cap that
 * exists precisely to end this loop, could never advance. The gate was
 * unsatisfiable and unexhaustible at once, and the booking never happened.
 *
 * On this front-end the MODEL is the ASR and it heard the caller correctly.
 * `inputTranscription` is a degraded copy of what it already understood. So the
 * escape hatch asks the audio instead, which is the one signal that does not
 ...
 * ---------------------------------------------------------------------------
 *
 * It does not touch consent. Voiced audio is evidence that the caller ANSWERED;
 * it says nothing about WHAT they said. `isAffirmative` is untouched, because a
 * gate that read a cough as agreement would release writes on callers who never
 * agreed — far worse than the loop this closes. The write-order gate's
 * equivalent problem is bounded by a shared attempt budget instead, which ends
 * the re-asking without ever inventing a yes.
 *
 * Idempotent per question: it clears `spellAskPending`, so several utterances
 * inside one answer cost one miss. A transcript that arrives afterwards WITH
 * letters still sets `spellingCaptured`, which is better evidence and wins.
 *
 * @param {object} state - the call state carrying the spelling flags
 * @param {number} voicedMs - voiced audio in the caller's answer
 */
export function applyCallerVoicedAnswer(state, voicedMs) {
  if (!state || !state.spellAskPending) return;
  if (!Number.isFinite(voicedMs) || voicedMs < voicedAnswerMs()) return;
  state.spellAskMisses = (state.spellAskMisses || 0) + 1;
  state.spellAskPending = false;
}
