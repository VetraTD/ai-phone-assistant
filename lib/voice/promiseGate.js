...
/**
 * How long to wait, after swallowing the model's promise, before speaking the
 * engine's replacement.
 *
 * Not zero, which is what this was first written as. Two reasons, both found
 * from a live call. services/gemini.js yields `toolCall` AFTER the tool has
 * run, so an instantly-refused call would be announced as though it had
 * succeeded; and a deliberately-silent tool does no work at all, so the real
 * answer is moments away and a line in front of it is chatter.
 *
 * Whichever arrives first wins: the model's own text cancels this, and if
 * nothing arrives the line plays because by then there is a genuine wait.
 * 0 restores the speak-immediately behaviour.
 *
 * @returns {number}
 */
export function promiseSwapDelayMs() {
  const v = Number.parseInt(process.env.VOICE_PROMISE_SWAP_DELAY_MS, 10);
  return Number.isFinite(v) && v >= 0 && v <= 1_500 ? v : 400;
}
