...
import { effectiveTier } from "../compliance.js";
...
import { resolveGoogleVoice, resolveRingTone, resolveLocale, resolveSpeechLocale, resolveProfile } from "./voiceLocale.js";
import { getStrings, holdKindForTool, holdLineFor, holdLineVariants, HOLD_KINDS } from "./strings.js";
// The only hold line keyed to the CALLER's turn rather than to a tool. Same
// detector the spelling gate uses, so the two can never disagree about whether
// a spelling happened.
import { looksLikeSpelling } from "../spellingSignal.js";
import {
  isPromiseOnly,
  promiseGateMs,
  promiseSwapDelayMs,
...
import {
  ...
  ARBITRATED_RULES,
} from "./endpointArbiter.js";
import { VOICE_CATALOG } from "../../config/voices.js";
import { toSpeakable, sanitizeOutbound } from "./speakableText.js";
...
// selectPipelineHandler). A10 deleted the legacy lib/mediaStream.js and the
// PIPELINE_V2 opt-out that reached it. The behaviours listed below were ported
// from it and are now simply this file's (greeting, silence nudges, transfer,
// max-duration hangup,
...
// Backstop for the deferred micro-utterance warm. Normally the greeting's
// onSettled fires it; that callback is skipped when the caller barges the
// greeting, so this covers that case without ever racing pickup.
const UTTERANCE_WARM_BACKSTOP_MS = 8_000;
...
// Exported for the Live front-end, which runs the same ladder on its own
// clock. Shared rather than copied so a threshold changed here changes there.
...
export const SILENCE_THRESHOLDS_DEFAULT = { nudge1: 8_000, nudge2: 15_000, hangup: 24_000 };
...
/**
 * Throw away a half-prepared reply when the caller turns out not to be finished.
 *
 * Ships ON. The usual convention here is default-off for turn-taking changes,
 * and it is departed from deliberately: this path executes ONLY in a state that
 * today produces a guaranteed-wrong outcome (the caller's continuation answered
 * as a second, separate turn), and its worst case is one re-prepared reply that
 * nobody heard. `false` restores the old queueing behaviour without a deploy.
 */
const ABORT_ON_RESUME = process.env.VOICE_ABORT_ON_RESUME !== "false";

/**
 * How many times one caller utterance may cost us a discarded turn.
 *
 * Abort, restart, caller adds more, abort again — each cycle burns an LLM round
 * trip and the caller is never answered. Past this we queue instead, which
 * always terminates. 0 disables the whole path.
 */
const RESUME_ABORT_MAX = (() => {
  const v = Number.parseInt(process.env.VOICE_RESUME_ABORT_MAX, 10);
  return Number.isFinite(v) && v >= 0 && v <= 10 ? v : 2;
})();
...
//
// 1.5s, not the original 800ms: this window is also the caller's last chance
// to say "oh wait, one more thing". A caller who speaks inside it cancels the
// close entirely (see cancelPendingClose), so the window has to be long enough
// for someone to actually start talking — 800ms is barely a breath. The cost
// of being generous is a little dead air at the end of a call that is over
// anyway; the cost of being stingy is hanging up on someone mid-sentence.
const HANGUP_GRACE_MS = 1_500;

// The greeting is uninterruptible: barge-in is disarmed until it finishes.
//
// Everywhere else, letting the caller take the floor is right. Here it is not:
// a cough, a carrier echo or a speakerphone reflection cuts the business name
// off the front of the call, and the caller then has no idea who answered.
//
// Anything the caller says during the greeting is DISCARDED, not held. Holding
// it was tried first and is the wrong trade: the greeting gate sits ABOVE
// turnManager's word-count, sustained-voice and confidence checks, so held text
// is unfiltered — a cough, a television or a reflection too short for the echo
// classifier's 4-token bigram window all became the caller's first turn, when
// the identical audio one second later would have been rejected. The cost is
// that a caller who talks over the greeting says it again; the fix for that is
// a shorter greeting, not a laxer gate. Measured by greeting_speech_discarded.
//
// This flag is the rollback: VOICE_GREETING_UNINTERRUPTIBLE=false disarms the
// gate entirely, so nothing is ignored and nothing is discarded. That is why
// there is no second flag for the discard itself — a carry path that only runs
// when someone flips a knob under production pressure is the least-tested code
// on the most-stressed path.
const GREETING_UNINTERRUPTIBLE = process.env.VOICE_GREETING_UNINTERRUPTIBLE !== "false";

// The engine acknowledges a tool round: "Let me check the diary for you."
//
// This shipped ON, then OFF for a few hours on 2026-08-30, and is now ON again
// for a different reason than the first time. Worth writing down, because the
// first reason was wrong.
//
// It was originally a GAP-FILLER, fired 600ms into the wait. That cannot work
// here. The line takes ~1.5s to say, audio plays serially, and the gap it was
// covering measures 833ms p50 - so it arrived just before the answer and made
// those turns LONGER. A caller put it exactly: "it says the thing and then
// immediately comes back with the results, why did it even say that".
//
// It is now an ACKNOWLEDGEMENT, fired the INSTANT the tool starts. Nothing is
// being covered: the line simply is the pause, the way it is when a person says
// "let me check that for you" and then looks something up. Answering 200ms
// after those words is what sounds fake, because nobody checks that fast.
//
// Costs ~0.5s per tool turn. Set to exactly "false" to disable.
// engineFillerEnabled now lives in lib/voice/promiseGate.js — services/gemini.js
// needs the same answer to decide whether the model may stay silent, and the
// two halves must not be able to disagree.

// How long a tool may run before the caller is told to hold. Most answer well
// inside this, and "One moment." in front of an instant answer is chatter.
// ~400ms of G.711 mu-law silence (0xFF), 8kHz, one byte per sample. Enqueued
// after an acknowledgement so the answer that follows lands as a separate
// sentence rather than the same breath.
const HOLD_BEAT_SILENCE = Buffer.alloc(3200, 0xff);

// How often to look again while the model's own audio is still playing, and how
// many times before giving up. 150ms x 40 = 6s, comfortably inside the LLM hard
// timeout, so the retry can never outlive the turn it belongs to.
const HOLD_RETRY_MS = 150;
const HOLD_RETRY_MAX = 40;

// How long after the first acknowledgement a still-silent turn gets a second,
// different line. Covers the multi-tool-round shape (reschedule: look up, verify,
// answer) that used to acknowledge once and then go quiet for seconds. 0 disables.

// How long a tool round may run before the caller is told to hold. 0 = speak
// the moment the tool is known, which is the earliest a CORRECT line can exist.
//
// This has moved four times, and the fourth is a correction of the third.
//
//   600  debounced; fired on turns that did not need it
//   0    spoke on every single tool call — reported as chatter
//   1500 a threshold picked from llm_reply_after_tool_ms, 2026-08-30:
//          685 722 734 764 794 818 827 835 861 ... 3133
//        the reasoning being that a line takes ~1.5s to SAY and audio plays
//        serially, so firing at T only helps if the answer arrives after
//        T + 1.5s.
//   0    again — because the evidence for 1500 was already stale when it landed.
//
// The chatter that justified 1500 (eef138a) came from tools that had no
// business speaking at all: record_customer_request, end_call, request_transfer
// and set_call_intent all spoke a line then. 5f631fb landed AFTER eef138a and
// mapped every one of them to null (lib/voice/strings.js), and set_call_intent
// is filtered again upstream at lib/voice/llmTurn.js. The chatter was fixed at
// its source and the threshold was never re-derived against the smaller set of
// tools that remain.
//
// What 1500 cost, meanwhile, was reported live: the hold line arriving ~3s
// after the caller stopped speaking. The threshold is not the whole of that,
// but it is the half we control — and it is the WORST half, because the clock
// starts at the toolCall event, i.e. AFTER the model has already spent its
// round-trip. The model's thinking was being added to the wait instead of
// counted toward it.
//
// The accepted trade, made explicitly by the owner: a turn whose answer was
// ~800ms away now carries a line in front of it and ends up longer. In exchange
// the caller is never sitting in dead air, and the line always names the work
// that is actually happening. .env.example has argued for exactly this all
// along — "fired immediately it is simply the pause itself, the way it is when
// a person says 'let me check' and then looks something up".
//
// The env var stays, so the number can move again without a deploy — and now
// there is a hold_line_ms span to move it FROM rather than a stopwatch.
function toolHoldDelayMs() {
  const v = Number.parseInt(process.env.VOICE_TOOL_HOLD_DELAY_MS, 10);
  return Number.isFinite(v) && v >= 0 && v <= 5_000 ? v : 0;
}

// How much silence to put in front of the greeting.
//
// Reported live: the first few words of the greeting are not heard. Nothing in
// this pipeline can truncate that audio — onInterrupt is unreachable while
// greetingActive, and audioOut.clear() has no other caller that can fire before
// turn 1 — so the loss is not ours to cut. What IS true is that we hand Twilio
// the first byte of SPEECH as the first byte of audio on the call: TwiML
// returns a bare <Connect><Stream> with no <Say>, no <Pause>, and no priming
// frames, so the greeting starts the instant ElevenLabs produces sound, which
// can be before the carrier has finished bringing the caller's audio path up.
//
// Padding with silence is the standard mitigation and costs nothing that
// matters: it overlaps the ElevenLabs connect + TTFB that the call is waiting
// on anyway, so it is mostly free wall-clock rather than added latency.
//
// The cause is NOT proven — that is what greeting_audio_timing (below) is for.
// If the numbers say first-frame-to-wire was never the problem, this comes back
// out. 0 disables.
//
// Read at call time, not module load, so a Railway change takes effect on the
// next call rather than the next deploy (the convention lib/nameQuality.js
// spellPolicy() documents).
function greetingPrerollMs() {
  const v = Number.parseInt(process.env.VOICE_GREETING_PREROLL_MS, 10);
  return Number.isFinite(v) && v >= 0 && v <= 2_000 ? v : 500;
}

// G.711 mu-law at 8kHz is one byte per sample, so 8 bytes = 1ms. Same
// arithmetic as HOLD_BEAT_SILENCE above (3200 bytes = 400ms), named here
// because the preroll length is computed rather than fixed.
const MULAW_BYTES_PER_MS = 8;

// Backstop for the above. A greeting whose completion mark never arrives (a
// Twilio hiccup, a TTS path that never reports done) would otherwise leave
// barge-in disarmed for the entire call — the one failure of this design a
// caller cannot recover from. Comfortably longer than any real greeting: a long
// custom greeting plus the recording disclosure is ~10s.
const GREETING_GUARD_MAX_MS = (() => {
  const v = Number.parseInt(process.env.VOICE_GREETING_GUARD_MAX_MS, 10);
  return Number.isFinite(v) && v >= 1_000 && v <= 60_000 ? v : 12_000;
})();
...
function buildUtteranceWarmEntries(config) {
  ...
  const entries = [
    ...
    // Warmed, not synthesized on demand: these play to COVER a pause, so
    // synthesizing one at the moment it is needed would add latency to the
    // exact turn that already had too much of it.
    // Every variant of every hold kind, so the clip is ready the instant a tool
    // starts. HOLD_KINDS is derived from the tool map itself, so a capability
    // added later cannot silently ship an unwarmed line.
    ...HOLD_KINDS.flatMap((k) => holdLineVariants(S, k).map((text) => ({ kind: k, text }))),
...
/**
 * May this call be transferred to a human right now?
 *
 * EXPORTED for the speech-to-speech front-end (lib/voice/live/), which gates
 * `request_transfer` on the same answer. A second copy would be a second
 * reading of `transferPolicy`, and the failure mode is silent: the model is
 * allowed to offer a transfer the business never permitted, tells the caller
 * "putting you through now", and nothing happens.
 *
 * @param {object} config
 * @returns {boolean}
 */
export function resolveTransferAllowed(config) {
 ...
 */
/**
 * Does this tool WRITE something the caller would notice?
 *
 * Each pack declares its caller-visible writes as `actionTools` — the same list
 * checkRequirements gates on. Everything else (availability checks, appointment
 * lookups) is a read, and discarding a turn after one costs nothing.
 *
 * Unknown tool name => treated as a write. Failing safe here means a missed
 * recovery, not a duplicate booking.
 *
 * @param {string} name
 * @returns {boolean}
 */
export function isWriteTool(name) {
  ...
  try {
    ...
    if (!pack) return true;
    return (pack.actionTools || []).includes(name);
  } catch {
    return true;
  }
}
...
/**
 * May we throw away the in-flight turn because the caller started talking again?
 *
 * The caller paused mid-thought, the pipeline read the pause as end-of-turn and
 * began preparing a reply, and now they are carrying on. Nothing has been heard
 * yet, so the cheapest correct move is to discard the half-formed answer and
 * answer the whole sentence instead of queueing the remainder as a second turn.
 *
 * Every `false` here is a case where discarding would be worse than colliding.
 *
 * @param {object} opts
 * @param {boolean} opts.enabled - VOICE_ABORT_ON_RESUME
 * @param {boolean} opts.processingTurn - a turn is actually in flight
 * @param {boolean} opts.audioStarted - the caller has already heard some of it
 * @param {boolean} opts.textStarted - the model has begun emitting words
 * @param {boolean} opts.toolStarted - a tool round has begun this turn
 * @param {number} opts.resumeAborts - aborts already spent on this utterance
 * @param {number} opts.maxAborts - VOICE_RESUME_ABORT_MAX
 * @returns {boolean}
 */
export function shouldAbortForResume({
  enabled,
  processingTurn,
  audioStarted,
  textStarted,
  ...
  // The caller has heard part of the reply, so this is an interruption, not a
  // continuation — turnManager's barge-in path owns it, and two subsystems
  // tearing down the same turn is its own class of bug.
  if (audioStarted) return false;
  // The model is already emitting words, so TTS is synthesizing and audio is
  // ~95ms away. "The caller resumed while it was still THINKING" has stopped
  // being true, and discarding now throws away work about to be heard — the
  // barge path handles this case properly, including the history accounting.
  if (textStarted) return false;
  // THE MONEY CASE. The turn may already have called book_appointment.
  // Aborting unwinds the generator, but a tool completing after that is
  // written and never salvaged into the merged turn's history — so the merged
  // turn books again. Queue instead, exactly as before.
  if (toolStarted) return false;
  // Anti-starvation. Each abort costs a full LLM round trip; without a bound,
  // a caller who keeps adding words is never answered at all.
  if (!(maxAborts > 0)) return false;
  if (resumeAborts >= maxAborts) return false;
  return true;
}

/**
 * May the engine speak a hold line ("One moment.") right now?
 *
 * Shared by BOTH paths that can play one, because they drifted apart and the
 * caller paid for it. The `slow` branch checked only `producedText`; the
 * `toolCall` branch beside it checked three things. So on a booking — where
 * capabilities/appointments.js explicitly tells the model to say "one moment
 * while I check that" before the availability call — the model said it and the
 * engine then said it again on top. Reported from a live call as "it keeps
 * saying one moment when it just does not need to".
 *
 * @param {object} opts
 * @param {boolean} opts.producedText - has the model already spoken this turn
 * @param {boolean} opts.holdLinePlayed - has a hold line already played this turn
 * @param {string} opts.spokenThisTurn - what the assistant has said so far this turn
 * @param {RegExp} [opts.promiseRe] - locale's "I am about to do something" shape
 * @returns {boolean}
 */
export function shouldPlayHoldLine({ audioPlaying, holdLinePlayed, spokenThisTurn, promiseRe }) {
  if (holdLinePlayed) return false;
  // Gated on AUDIO, not on text.
  //
  // This used to open with `if (producedText) return false` — any text at all
  // this turn, and the hold line was dead. Under VOICE_INTENT_MARKER the model
  // writes the intent line and usually a sentence alongside its tool call, so
  // that was true on almost every turn: the line never fired, including on the
  // turn that needed it most (a reschedule that says "just to confirm…" and
  // then grinds for three seconds).
  //
  // The question that actually matters is "is the caller hearing something
  // right now", and isPlaying() answers exactly that — the same reasoning the
  // stalled branch already uses and documents. Text produced and finished is
  // precisely when a hold line earns its place.
  if (audioPlaying) return false;
  // The model volunteered its own "one moment" — saying it again is worse than
  // not saying it at all. Absent pattern means "no evidence of a promise", so
  // a locale without one keeps its hold lines rather than losing them all.
  //
  // THIS is the precise version of the check above, and it was unreachable
  // behind it for as long as producedText was a blanket veto.
  if (promiseRe && spokenThisTurn && promiseRe.test(spokenThisTurn)) return false;
  return true;
}

export function mapLanguage(config, { callerNumber } = {}) {
  ...
  //
  // "en" resolves through resolveLocale rather than hardcoding en-US. A UK
  // business is configured as ["en"] like every other English one, so it was
  // running Deepgram on a US model: British vowels through an American
  // acoustic model raise the word-error rate, and every word error is a chance
  // to invent a transcript that trips barge-in. That is a direct mechanical
  // reason false cutoffs were markedly worse on the +44 number than the US one.
  //
  // resolveSpeechLocale, NOT resolveLocale. The difference is the whole point:
  // resolveLocale checks the operator's chosen VOICE accent before the
  // timezone, which is right for text-to-speech and exactly wrong here. It let
  // a London business whose operator picked an American voice run en-US
  // recognition on British callers — the persona deciding how the caller was
  // heard. Speech recognition follows the CALLER: their number first, then the
  // business's number, then its timezone.
  if (first === "en") {
    return resolveSpeechLocale(config, { callerNumber }) === "en-GB" ? "en-GB" : "en-US";
  }
  const NOVA3_LANGUAGE_MAP = { es: "es", fr: "fr" };
 ...
 * loaded config (see services/db.js loadConfig -> voiceProvider/
...
function buildGreeting(config) {
  // Delegates to services/gemini.js greetingTextFor, which is now the single
  // composer for both front-ends. The Live path has no TTS leg and hands the
  // same text to the model instead of speaking it, and two copies of "does the
  // recording disclosure lead?" is two places for it to go missing.
  return greetingTextFor(config);
}

/** Stage-2 silence nudge text (ported from mediaStream.js), localized. */
export function buildSilenceNudge(stage, step, intent, config) {
  const S = getStrings(config);
  if (stage === 1) {
    return S.nudge1;
  }
...
export function buildSilenceGoodbye(cfg) {
 ...
 * The only pipeline: A10 removed the PIPELINE_V2 opt-out and the legacy
 * handler it selected.
 ...
 * @param {object} [opts]
 * @param {function(): number} [opts.now] - injectable clock (ms), defaults to
 *   performance.now(). Threaded into audioOut, echoGuard and turnManager so all
 *   four agree on one clock.
 *
 *   This exists for sim/cutoffSim.sim.js. That harness runs the caller script
 *   and every timer on vitest's virtual clock, but turnManager/echoGuard/
 *   audioOut used to read performance.now() directly — which vitest does not
 *   fake — so their VAD and echo windows were judged against WALL CLOCK while
 *   the audio around them moved in virtual time. Identical inputs disagreed
 *   about which utterance was cut off across runs, and the sim documented
 *   itself as not quotable because of it.
 */
export function handleVoiceSessionConnection(ws, _req, { now = () => performance.now() } = {}) {
  let callSid = null;
  let state = null;
  ...
  // Greeting phase. `greetingActive` closes the barge-in gate; anything the
  // caller says meanwhile is counted and dropped (see GREETING_UNINTERRUPTIBLE
  // for why it is not kept). The timer is the backstop for a completion mark
  // that never arrives — see GREETING_GUARD_MAX_MS.
  let greetingActive = false;
  let greetingGuardTimer = null;
  // { startAtMs, firstWireMs?, ttsFirstByteMs?, logged? } — see
  // greetingPrerollMs(). The greeting is spoken with isTurn=false, so none of
  // the per-turn latency marks are recorded for it and its own timing has
  // never appeared in any number we report.
  let greetingTiming = null;

  /**
   * Emit greeting_audio_timing once BOTH halves are known.
   *
   * Deliberately not emitted from whichever fires first. With a preroll the
   * wire burst leads; with VOICE_GREETING_PREROLL_MS=0 — the control arm the
   * .env instructions tell the operator to compare against — nothing is
   * enqueued until ElevenLabs produces audio, so the TTS callback runs BEFORE
   * that audio has been paced onto the wire and firstWireMs would log as null.
   * That is the one configuration where the number decides whether the preroll
   * is worth keeping, so it is the one that must not be missing. Found in
   * review.
   */
  function logGreetingTiming() {
    if (!greetingTiming || greetingTiming.logged) return;
    const { startAtMs, firstWireMs, ttsFirstByteMs } = greetingTiming;
    if (firstWireMs === undefined || ttsFirstByteMs === undefined) return;
    greetingTiming.logged = true;
    log.info("greeting_audio_timing", {
      callSid,
      prerollMs: greetingTiming.prerollMs ?? null,
      firstWireMs,
      ttsFirstByteMs,
      greetingChars: greetingTiming.greetingChars ?? null,
      // How much of the preroll was covering dead air rather than adding to
      // it: positive means the pad ran out before the greeting was ready.
      gapMs: ttsFirstByteMs - firstWireMs,
      startedAt: startAtMs,
    });
  }

  // Leak-guard context for this call: the live tool vocabulary plus the
  // localized line to speak in place of a sentence that cannot be repaired.
  // Built lazily and memoized — config is not known until the start event, and
  // rebuilding the declarations per sentence would put tool assembly on the
  // hot path.
  let leakCtxCache = null;
  function leakCtx() {
    const cfg = state?.config;
    if (!cfg) return undefined;
    if (!leakCtxCache) {
      leakCtxCache = {
        toolNames: geminiService.callToolNames(cfg, { integrations: state.integrations || [] }),
        toolParamNames: geminiService.callToolParamNames(cfg, {
          integrations: state.integrations || [],
        }),
        fallback: getStrings(cfg).actionNotCompleted,
        // Carried on the same context so every speech site gets locale-correct
        // phone grouping for free: "+442079460958" is spoken "020 7946 0958"
        // rather than "442 079 460 958".
  ...
  // Which hold the semantic arbiter's answer belongs to. Bumped per hold, so a
  // verdict that arrives after the caller has said something else cannot act
  // on a floor it no longer owns.
  let holdSeq = 0;
  ...
  // Discarded turns spent on the CURRENT caller utterance. Reset when the
  // caller actually hears audio — that is the only honest "we got somewhere"
  // signal, so it is the only honest place to refill the budget.
  let resumeAborts = 0;

  // Background context (knowledge / integrations / callerContext) fetch.
  let contextLoaded = false;

  // Lazily-synthesized hold-line buffers, cached per call and keyed by kind
  // ("filler", "holdAvailability", ...). Only reached on the Google path; the
  // ElevenLabs path writes the line into the turn's already-open stream.
  const holdBufs = new Map();
  // Which variant of each hold line to use next, PER ACTION.
  //
  // One shared counter looked tidier and read worse: on a cancel call the
  // lookup consumed variant 0, so the cancellation itself led with its SECOND
  // phrasing ("Taking care of that now") instead of its clearest one
  // ("Canceling that for you now"). The first thing a caller hears about an
  // action should be the version that names it most plainly.
  const holdLineNo = new Map();
   ...
   */
  // onFirstAudioExtra fires on the first synthesized byte regardless of isTurn.
  // The per-turn marks below are deliberately turn-only, which left the
  // GREETING — the one utterance a caller judges the product by — with no
  // timing at all. This is how greeting_audio_timing gets its TTS number
  // without pretending the greeting is a turn.
  function beginSpeaking(markName, isTurn, onSettled, onFirstAudioExtra) {
    ...
    const tts = createTtsTurn({
      ...
      onAudioChunk: (chunk) => {
        if (isTurn && !firstFrame) {
          ...
          // The caller is now hearing an answer, so the abort budget refills.
          // "We produced audio" is the only honest definition of progress
          // here: refilling on anything earlier would let a caller who keeps
          // adding words cycle forever without ever being answered.
          resumeAborts = 0;
          // Past this point a resumption is an interruption, not a
          // continuation, and belongs to turnManager's barge-in path.
          if (inFlightTurn) inFlightTurn.audioStarted = true;
        }
      ...
      onDone: (payload = {}) => {
        ...
        // Counted before the sticky check, which only fires on the FIRST such
        // turn — the question this answers is "how much of the call did the
        // caller not hear ElevenLabs for", so every affected turn counts.
        if (isTurn && usedFallback && !forceFallback) {
          bumpCounter("tts_fallback_turns");
        }
  ...
  function noteSpokenText(text) {
    echoGuard?.noteSpoken(text, now());
  }

  /**
   * Speak a fixed string (greeting / nudge / error / goodbye).
   *
   * Runs the leak guard but NOT the full toSpeakable pipeline. The difference
   * matters: this path carries the business's own greeting, and applying the
   * date/phone/currency transforms to it would change the most
   * identity-defining line of the call — and speakTextCacheable would then
   * apply them a second time. The guard alone costs nothing on clean copy and
   * closes the one genuinely exposed path here, fallbackFlow, which reads
   * caller-supplied text back verbatim.
   */
  function speakText(text, markName, onSettled, onFirstAudio) {
    const safe = sanitizeOutbound(text, leakCtx());
    const tts = beginSpeaking(markName, false, onSettled, onFirstAudio);
    activeTts = tts;
    noteSpokenText(safe);
    tts.write(safe);
    tts.end();
  ...
  function speakTextCacheable(text, markName) {
    const speakable = text ? toSpeakable(text, leakCtx()) : text;
   ...
   *
   * `final` marks a close the caller cannot talk their way out of — the call
   * has hit its duration ceiling, or STT is dead and we can no longer hear
   * them. Everything else is conversational: if the caller speaks before the
   * socket actually closes, startTurn cancels it and answers them instead.
   * @param {string} text
   * @param {string} markName
   * @param {{final?: boolean}} [opts]
   */
  function speakThenClose(text, markName, { final = false } = {}) {
    state.step = STEPS.ENDING;
    state.closeAfterMark = markName;
    state.closeIsFinal = final;
    speakTextCacheable(text, markName);
    armCloseFallback();
  }

  /**
   * Walk back an armed close because the caller is still talking.
   *
   * Clears both the mark the close was waiting on and the backstop timer —
   * missing either one leaves the call closing a few seconds later anyway,
   * which reads to the caller as a random disconnect mid-answer.
   * @param {string} reason
   */
  function cancelPendingClose(reason) {
    if (!state) return;
    state.closeAfterMark = null;
    state.closeIsFinal = false;
    clearTimeout(closeFallbackTimer);
    closeFallbackTimer = null;
    state.step = STEPS.GATHER_DETAILS;
    log.info("close_cancelled", { callSid, reason });
  }
   ...
   */
  async function playHoldLine(kind = "filler", { trigger = "tool", tool = null } = {}) {
    const epoch = state.speakEpoch;
    const n = holdLineNo.get(kind) || 0;
    holdLineNo.set(kind, n + 1);
    const text = holdLineFor(getStrings(state.config), kind, n);
    if (!text) return;

    // Stamped at the moment the line is handed to the audio path, and NOT
    // earlier: everything above this point can still bail out (an empty line, a
    // superseded epoch), and a mark for a line nobody heard would make
    // hold_line_ms a measure of intent rather than of the caller's experience.
    //
    // `trigger` is which of the four mechanisms fired — tool / promise_swap /
    // slow / stalled. Worth carrying because they mean different things: `tool`
    // is the engine naming work in progress, `slow` and `stalled` are watchdogs,
    // and `promise_swap` is the model having volunteered a wait line the engine
    // then corrected. Reading "the line was wrong" without knowing which of the
    // four produced it is how the previous two attempts at this went.
    let announced = false;
    const announce = () => {
      if (announced) return;
      announced = true;
      state.turnMetrics?.mark("hold_line");
      bumpCounter("hold_line_played");
      // `trigger` first, because it is the field that identifies WHICH of the
      // four mechanisms spoke and the console renderer shows the leading one.
      //
      // It was omitted on the first pass, and reading a live call on 2026-09-01
      // then cost a chain of inference: the spelling line was distinguishable
      // from the `slow` watchdog only because both were absent a tool name and
      // one of them had fired in the same millisecond as hold_flushed. A field
      // that exists in the code and not in the log is not instrumentation.
      log.debug("hold_line_played", { callSid, trigger, kind, tool, text });
    };
    if (!currentVoice().forceFallback) {
      if (state.speakEpoch !== epoch) return;
      // PRE-RENDERED, straight onto the audio queue.
      //
      // Writing the line as TEXT into the live turn stream gave the caller
      // non-deterministic timing: that socket runs auto_mode=true, so
      // ElevenLabs decides when to generate and can hold a short fragment until
      // more text arrives -- which is the reply. A caller heard the two voiced
      // back to back at the END of the wait, after several seconds of silence.
      //
      // audioOut is a serial queue and the reply's own chunks go through it
      // (see beginSpeaking onAudioChunk), so a clip enqueued now plays first
      // and the reply follows. DELIBERATELY does not touch activeTts: the
      // reply's TTS turn is live and owns it, unlike playCachedBuffer which
      // nulls it for standalone utterances.
      const speakable = toSpeakable(text, leakCtx());
      const cached = speakable ? utteranceCache.get(currentVoice().voiceId, null, speakable) : null;
      if (cached) {
        announce();
        noteSpokenText(speakable);
        audioOut?.enqueue(cached);
        // A beat, so the answer does not tailgate the acknowledgement. Without
        // it the two run together as one breath, which is the robotic reading.
        if (HOLD_BEAT_SILENCE) audioOut?.enqueue(HOLD_BEAT_SILENCE);
        return;
      }
      // Warm miss (cache not populated yet, or a line not in the warm set).
      // Fall back to the live stream rather than staying silent.
      if (!activeTts) return;
      try {
        announce();
        noteSpokenText(text);
    ...
    try {
      ...
      // Memoized per KIND, not just the one "One moment." line: there are now
      // four hold lines and a call can use more than one of them.
      const buf = cached
        || holdBufs.get(text)
        || await googleTts.synthesizeMulaw(text, googleVoice, callSid);
      if (!cached) holdBufs.set(text, buf);
      if (state.speakEpoch !== epoch) return;
      announce();
      noteSpokenText(text);
      audioOut?.enqueue(buf);
  ...
  // ------------------------------------------------------------------
  // Tenant scope for per-turn writes.
  //
  // Migration 029 puts FORCE row-level security on every tenant table, so a
  // write with no `app.business_id` set is REFUSED, not merely unscoped. The
  // pickup context load was already wrapped; these per-turn writes were not,
  // and a real call showed what that costs:
  //
  ...
  //
  // Every call site swallows its error into a log line and carries on — right
  // for a phone call, and the reason nobody noticed the transcript was gone.
  //
  // A helper rather than a wrap at each site, so the next writer added here
  // inherits the scope instead of having to remember it.
  // ------------------------------------------------------------------
  function scopedWrite(operation, fn) {
    return db.withTenantSafe(state?.businessId ?? null, fn, { operation, callSid });
  }
  ...
  function logCallerTranscript(text) {
    ...
    scopedWrite("addTranscriptEntry", () =>
      db.addTranscriptEntry(state.dbCallId, "caller", text, seq)
    ).catch((err) =>
  ...
  function logAiTranscript(text, callerSeq) {
    ...
    scopedWrite("addTranscriptEntry", () =>
      db.addTranscriptEntry(state.dbCallId, "ai", text, callerSeq + 1)
    ).catch((err) =>
  ...
  function buildExtras(config) {
    return {
      ...
      // Call-scoped end_call gating signals — see services/tools.js.
      callerTurnCount: state.callerTurnCount || 0,
      completedActionThisCall: !!state.completedActionThisCall,
      // Is the spelling question closed for this call? Derived from BOTH sides
      // of the exchange — what the assistant asked and what the caller
      // actually said back — rather than trusted to the prompt, and surfaced
      // here so the dynamic tail can state it as a fact instead of asking the
      // model to remember.
   ...
   *
   * OUTPUT tokens are forwarded too, and their absence was a real gap rather
   * than a decision: services/gemini.js buildUsage() has always returned
   * `outputTokens` and `thoughtsTokens` from the SDK's own usageMetadata — what
   * was BILLED — and this function dropped both on the floor. Input tokens
   * alone cannot price a turn, and output tokens are the dearer half per token,
   * so every cost figure derived from these logs was half a number.
   *
   * Kept omit-when-absent, like the two above: getLatencyStats skips turns
   * without a count, and a present-but-null field would be counted as a zero.
   *
   * @returns {{cached_tokens?: number, prompt_tokens?: number, output_tokens?: number, thoughts_tokens?: number}}
   */
  function usageExtras() {
    ...
    return {
      ...
      ...(usage.thoughtsTokens != null ? { thoughts_tokens: usage.thoughtsTokens } : {}),
    };
  }

  /**
   * Reopen the barge-in gate. Whatever the caller said over the greeting is
   * already gone — it was counted and dropped as it arrived.
   *
   * Idempotent: the completion mark and the watchdog can both reach this, and
   * on a fast call they race.
   *
   * The armSilenceTimer call is load-bearing on the WATCHDOG path and only
   * there. When the greeting-done mark arrives normally, the mark handler's own
   * armSilenceTimer(true) runs immediately after this and supersedes it. When
   * the mark never arrives, this is the only arm there is — and while speech
   * was being held, the drop branch's arm was the thing keeping the ladder
   * alive. Without this line, "mark lost AND the caller talked over the
   * greeting" is a call that goes silent for its whole duration. Arming twice
   * is free: armSilenceTimer retry-arms while audio is playing or a turn is in
   * flight.
   */
  function endGreetingPhase() {
    if (!greetingActive) return;
    greetingActive = false;
    clearTimeout(greetingGuardTimer);
    greetingGuardTimer = null;
    armSilenceTimer(false);
  }

  /**
   * The caller was not finished. Throw away the reply we had started preparing.
   *
   * Deliberately NOT onInterrupt. The differences are the whole point:
   *
   *  - No recordInterruptedTurn. That writes a system note saying "the caller
   *    interrupted you here — you had only said X". Nothing was audible, so
   *    that would be a false statement injected into the next prompt, and it
   *    would push the partial text as a user turn that the merged turn then
   *    duplicates.
   *  - No audioOut.clear() and no echoGuard.noteAudioStopped(). Nothing played.
   *  - No noteBargeForLoopBreaker(). Its remedy is "stay silent until a clean
   *    final arrives", which is precisely wrong here — we are already silent,
   *    and three hesitations in six seconds is a normal caller, not a runaway.
   *    Feeding these in would make the loop breaker fire on exactly the callers
   *    this exists to help.
   *  - callerTurnCount is given back. startTurn incremented it for the turn
   *    being discarded and will increment again for the merged one; that
   *    counter gates end_call, so double-counting makes the call look further
   *    along than it is.
   *
   * finishTurn IS called, with its own key. mark() is first-write-wins per
   * turn, so without it the merged turn inherits this one's llm_request and
   * reports a fabricated llm_ttfb_ms on every hesitant caller — which would
   * then be read as an LLM regression.
   */
  function abortTurnForResume() {
    const turn = inFlightTurn;
    state.speakEpoch++; // suppress in-flight TTS chunks + stale done marks
    if (activeTts) {
    ...
    // No need to clear the tool hold timer: it is turn-local to startTurn and
    // its callback already bails on a speakEpoch change, which the bump above
    // guarantees. Reaching for it from out here is what a ReferenceError is
    // made of.
    state.turnMetrics?.finishTurn({ aborted_before_audio: true });
    state.bargedTurnId = state.turnId;
    state.processingTurn = false;
    state.callerTurnCount = Math.max(0, (state.callerTurnCount || 1) - 1);
    inFlightTurn = null;
    resumeAborts += 1;
    bumpCounter("resume_aborts");
    clearSilenceTimer();

    // Fold everything back together, oldest first, so the merged turn sees one
    // chronological sentence. queuedText may hold a final that arrived while
    // this turn was running and was parked by the processingTurn branch.
    const parts = [turn?.userText, queuedText, heldText].filter((p) => (p || "").trim());
    queuedText = "";
    heldText = parts.join(" ");
    log.info("turn_aborted_on_resume", turnCtx({ aborts: resumeAborts, chars: heldText.length }));
    turnManager?.reset();
  }
  ...
  function handleCallerFinal(text) {
    if ((text || "").trim()) {
      const firstFinal = !state.sawCallerFinal;
      state.sawCallerFinal = true;
      // A LATCH, not a per-turn write: only the false->true transition is
      // published, so this costs one store round trip per call, not one per
      // turn. It is what stops `/twilio/status` on another instance tagging a
      // short real call as spam because the transcript insert had not landed.
      if (firstFinal) callState.writeShared(callSid, { sawCallerFinal: true });
    ...
    // The caller was still going. Discard the half-prepared reply and fold what
    // it was answering back into heldText, so the join below sees the whole
    // sentence.
    //
    // Hooked HERE rather than at the processingTurn branch in startTurn: this
    // is above the join, so cleanTranscript and extractFinalIntent each run
    // exactly once over the complete utterance. Merging further down would run
    // them per half, and a self-correction in the continuation ("actually,
    // make it Thursday") would be trimmed against the wrong preamble.
    if (
      shouldAbortForResume({
        enabled: ABORT_ON_RESUME,
        processingTurn: !!state.processingTurn,
        audioStarted: !!inFlightTurn?.audioStarted,
        textStarted: !!inFlightTurn?.textStarted,
        toolStarted: !!inFlightTurn?.toolStarted,
        resumeAborts,
        maxAborts: RESUME_ABORT_MAX,
      })
    ) {
      abortTurnForResume();
    } else if (state.processingTurn) {
      bumpCounter("resume_aborts_declined");
    }
    ...
    if (echoGuard?.isEcho(combined, now())) {
    ...
    // classifyHold used to be gated behind isIncomplete(), which made its own
    // `no_terminal_punctuation` branch unreachable for the case it was written
    // for. Deepgram runs smart_format + punctuate, so a final that lands in the
    // MIDDLE of a sentence usually still arrives punctuated — it reads as
    // "complete", the gate returns 0, and the assistant starts talking while
    // the caller is still going.
    //
    // Probe D measured 108 of 120 turns as "complete" with zero
    // no_terminal_punctuation and concluded there was nothing to tune. That
    // conclusion is measurement-limited: the probe's caller audio is clean
    // pre-rendered TTS with no hesitation, so it cannot produce the mid-sentence
    // final this branch exists to catch. Real callers hesitate.
    //
    // Ungating costs nothing when the caller really has finished: a hold that
    // catches a continuation is cancelled by it (see the holdTimer clear at the
    // top of this function) and adds no latency at all. It only spends time when
    // the caller was in fact done — which is why the no-punctuation wait is
    // retuned down in lib/transcriptUtils.js to suit the 150ms endpointing
    // window this now fires far more often under.
    // `combined` (raw) is passed alongside `clean` so the terminal-punctuation
    // branch can see the full stop that cleanTranscript/stripFillers removes.
    const { holdMs: wantedHoldMs, rule: classifiedRule } = classifyHold(clean, combined);
    ...
    if (holdMs > 0) {
      ...
      // Ask the model, alongside the timer that is already running. Never
      // awaited: the whole design is that the hold is unchanged unless an
      // answer arrives inside it. Off by default.
      //
      // Caught here as well as inside, because this is an optional accuracy
      // improvement bolted onto the one path every caller goes through. An
      // unhandled rejection from it must never be able to reach the call — and
      // this is not hypothetical: the first version threw synchronously on a
      // missing export and the only symptom was that the feature silently did
      // nothing.
      arbitrateHold({ fragment: clean, rule, holdMs, seq: ++holdSeq }).catch((err) => {
        log.debug("semantic_endpoint_error", { callSid, reason: err?.message });
      });
  ...
  /**
   * Semantic end-of-turn detection, running BESIDE the heuristic hold rather
   * than in front of it. See lib/voice/endpointArbiter.js.
   *
   * Everything about this is arranged so that a slow, failed, or absent answer
   * changes nothing: the hold timer is already ticking when this is called, it
   * is never awaited, and a verdict is only acted on while that same hold is
   * still the one in charge. Asking first and deciding after would put a
   * model round trip inside a 500-800ms budget, which is strictly worse than
   * the timer we have.
   *
   * @param {{fragment: string, rule: string, holdMs: number, seq: number}} o
   */
  async function arbitrateHold({ fragment, rule, holdMs, seq }) {
    if (!semanticEndpointEnabled()) return;
    // Only the genuinely ambiguous tiers. A fluent caller's finals come back
    // terminal_punctuation and never reach this, which is what keeps the
    // per-call cost proportional to how hesitant the caller actually is.
    if (!ARBITRATED_RULES.has(rule)) return;

    const verdict = await judgeTurnComplete(
      {
        fragment,
        recentTurns: state.history || [],
        deadlineMs: holdMs,
      },
      { getClient: geminiService.getClient },
    ).catch(() => ({ complete: null }));

    // No opinion, or the answer came back after the hold it belonged to had
    // already resolved. Either way the heuristic has decided and this must not
    // reopen it.
    if (!verdict || verdict.complete === null) return;
    if (seq !== holdSeq) return; // a newer final owns the floor
    if (holdTimer === null) return; // already flushed or extended past us
    if (!heldText) return; // nothing left to release

    if (verdict.complete) {
      // The caller has finished. Stop waiting out the rest of the timer — but
      // go through onHoldExpired rather than flushing outright, so its VAD
      // check still gets to overrule us if the caller is audibly still making
      // sound. Grammar is evidence; a voice on the line is better evidence.
      clearTimeout(holdTimer);
      holdTimer = null;
      onHoldExpired();
      // Counted only if it actually flushed. onHoldExpired may re-arm on its
      // VAD check, and a counter documented as "a hold cut short because the
      // model judged the caller finished" must not count holds that were not
      // cut short. Found in review.
      if (holdTimer === null) bumpCounter("semantic_endpoint_flushed");
      return;
    }

    // Mid-sentence. DOUBLE the original wait, not the whole remaining budget.
    //
    // The first version handed over everything up to the chain ceiling, so a
    // verdict arriving 150ms into an 800ms hold turned that turn into a 3s
    // wait — a single false INCOMPLETE costing the caller ~2.2s on top of a
    // measured 3,062ms p50. That is the runaway onHoldExpired already refuses
    // to allow: it grants HOLD_VAD_EXTENSION_MS per expiry precisely because a
    // live call once marched to the ceiling on one stale signal and cost 6.7s
    // voice-to-voice. Each extension has to earn itself here too.
    // Measured from the START of the hold, so the total budget is 2x what the
    // heuristic chose rather than 2x whatever is left — granting `holdMs` from
    // *now* would land within a few ms of the timer already running and make
    // the whole extension a no-op.
    const elapsed = holdStartedAt === null ? 0 : Date.now() - holdStartedAt;
    const remaining = Math.min(holdMs * 2, holdChainCeiling()) - elapsed;
    if (remaining <= 0) return;
    // ...and move the watermark, or onHoldExpired's "spoke since I last
    // looked" check would still be measuring from the start of the original
    // hold and could extend again on the same old sound.
    lastHoldCheckAtMs = Date.now();
    bumpCounter("semantic_endpoint_extended");
    log.debug("semantic_endpoint_extended", {
      callSid,
      rule,
      extensionMs: Math.round(remaining),
      tail: String(fragment).slice(-40),
    });
    startHoldTimer(remaining);
  }
  ...
  function recordInterruptedTurn(playingUntilMs) {
    ...
    const unheardMs = Math.max(0, (playingUntilMs ?? 0) - now());
  ...
  function onInterrupt(_text) {
    ...
    // A barge over the goodbye must not still hang up on them.
    //
    // closeAfterMark is matched by plain string compare, and after clear()
    // Twilio immediately re-echoes marks that were queued but never played
    // (see audioOut's docstring). So interrupting the goodbye made Twilio hand
    // back the very mark the close was waiting on, arming the hangup — while
    // the caller was mid-sentence. Waiting for their transcript to arrive and
    // cancel it is too slow: endpointing plus LLM round-trip routinely exceeds
    // the grace window.
    //
    // Barging IS the caller speaking, which is the same signal startTurn acts
    // on, so act on it at once and let the turn proceed normally.
    if (state?.closeAfterMark && !state.closeIsFinal) {
      cancelPendingClose("caller_barged_goodbye");
    }
    ...
    echoGuard?.noteAudioStopped(now());
  ...
  async function redialForTransfer(transferNumber, trigger) {
    try {
      ...
      scopedWrite("markCallTransferred", () => db.markCallTransferred(callSid)).catch((err) => {
  ...
  async function completeFallbackFlow({ callerName, callbackNumber, message }) {
    ...
    if (state.businessId) {
      try {
        const id = await scopedWrite("createCustomerRequest", () =>
  ...
  function failFallbackFlow() {
    ...
    speakThenClose(getStrings(state.config).fallbackFail, "fallback-fail-goodbye-done", { final: true });
  }
  ...
  async function startTurn(userText) {
    ...
    if (Date.now() - state.startedAt > CALL_MAX_DURATION_MS) {
      ...
      speakThenClose(maxDurationMsg, "maxdur-goodbye-done", { final: true });
      return;
    }

    // Already ending — but the caller just said something, so hanging up on
    // them is the wrong reflex.
    //
    // This used to be an unconditional `closeWs(); return;`: an instant, silent
    // disconnect on the caller's next word. It is how a caller who added "oh
    // wait, one more thing" during the goodbye got cut off mid-sentence, and
    // how a transfer that could not be placed left the call armed to drop dead
    // the moment they spoke again.
    //
    // Speaking is the clearest possible signal that the caller is not done, so
    // it CANCELS a conversational close and the turn proceeds normally. Only
    // two cases still hang up: a transfer already committed to a redial (the
    // call is leaving this socket regardless), and a close we cannot walk back
    // — max duration reached, or STT dead so we can no longer hear them.
    if (state.step === STEPS.ENDING) {
      if (state.transferAfterMark || state.pendingTransferNumber) {
        // The redial fires on its own mark; do not start a turn, do not close.
        return;
      }
      if (state.closeIsFinal) {
        closeWs();
        return;
      }
      cancelPendingClose("caller_spoke");
    }

    state.processingTurn = true;
    // Created HERE, not after ensureContext() below, because the abort-on-resume
    // path reads it. On the first turn ensureContext() awaits a network fetch,
    // and a caller resuming inside that window would otherwise find it null and
    // have their words dropped on the floor. callerSeq is filled in once the
    // caller row is persisted; toolStarted latches when a tool round begins and
    // is what forbids the abort (see shouldAbortForResume).
    inFlightTurn = { userText, callerSeq: null, spokenText: "", toolStarted: false };
    // Counts caller utterances actually handed to the LLM — not raw finals, so
    // echo, backchannels and STT phantoms never inflate it. end_call's gate
    // uses it to tell a real back-and-forth from the opening moments of a call
    // (services/tools.js).
    state.callerTurnCount = (state.callerTurnCount || 0) + 1;
    ...
    // A leading sentence that is nothing but "hold on", held back until we know
    // whether a tool call follows it. See lib/voice/promiseGate.js.
    // { spoken, timer } while open, null otherwise.
    //
    // Declared OUT here rather than beside the rest of the streaming state,
    // because the `finally` below has to be able to dispose of it and a `let`
    // inside `try` is not in scope in `finally`. The first version put it
    // there anyway: the cleanup threw a ReferenceError on every turn, which
    // showed up as the fallback-flow tests concatenating three separate caller
    // finals into one.
    let promiseGate = null;

    try {
      // Lazily resolve knowledge/integrations/callerContext on the first turn.
      //
      // Bracketed by marks because this await is INSIDE the window stt_tail_ms
      // reports: speech_end is stamped below from state.lastFinalAtMs (the real
      // moment speech ended) while stt_final is stamped at "now", i.e. after
      // this returns. So a slow call-start prefetch has always been charged to
      // the speech-to-text tail, where nobody would think to look for it — and
      // any future widening of the prefetch would make that worse and stay
      // just as invisible.
      //
      // Marked only when the wait is real. ensureContext() short-circuits on
      // contextLoaded, so marking unconditionally would stamp a ~0ms delta on
      // every turn after the first and drown the one measurement that matters
      // in zeros — finishTurn writes null for a delta whose marks are absent,
      // and getLatencyStats filters non-numbers, so skipping the marks is what
      // makes this a first-turn-only statistic.
      const contextWasPending = !contextLoaded;
      const contextWaitStartedAt = contextWasPending ? performance.now() : null;
      await ensureContext();
      const contextWaitEndedAt = contextWasPending ? performance.now() : null;
      if (state.speakEpoch !== myEpoch) return; // barged during context load

      // Did the caller just spell something, or decline to? Read HERE, before
      // buildExtras, because the prompt block and the write gate are both
      // decided from extras — reading it after the reply (where it used to
      // live) made every signal a turn late, so the caller spelled their name
      // and the same turn still told the model to ask for a spelling.
      applyCallerSpellingSignal(state, userText, getStrings(state.config));
      ...
      //
      // Filled in rather than replaced: the object was created before
      // ensureContext() so the resume path could see it, and replacing it here
      // would drop a `toolStarted` latch set in between.
      if (inFlightTurn) inFlightTurn.callerSeq = callerSeq;

      // Escape-trigger transfer (before invoking the LLM), same as legacy.
      if (TRANSFER_TRIGGERS.test(userText)) {
        state.processingTurn = false;
        // Paired with processingTurn: the turn is over, so nothing may later
        // mistake it for one still in flight and try to abort it.
        inFlightTurn = null;
      ...
      // Stamped HERE, not where the wait actually happened, for the reason the
      // comment above gives for speech_end and stt_final: a turn that escapes
      // to transfer never reaches finishTurn(), and createTurnMetrics only
      // clears its marks there — so marking earlier leaked this pair into the
      // NEXT real turn's payload, which then reported a context wait it never
      // incurred, with negative raw offsets. Found in review. The timestamps
      // are captured at the true moment and replayed here.
      if (contextWaitStartedAt !== null) {
        metrics?.mark("context_wait_start", contextWaitStartedAt);
        metrics?.mark("context_wait_end", contextWaitEndedAt);
      }
      ...
      let sentenceBuf = "";
      // Engine-owned hold line for a slow tool round (see the toolCall branch).
      let toolHoldTimer = null;
      let holdLinePlayed = false;
      ...
      let spokenThisTurn = "";
      // The hold kind of the first tool this turn that has one, kept because
      // the model's promise and its function call can arrive in either order.
      let pendingHoldKind = null;
      // ...and the tool it came from, for the hold_line_played log only. Never
      // read to decide anything — the kind is the decision.
      let pendingHoldTool = null;
      // The caller just spelled something. Say so NOW.
      //
      // Every other hold line in this file waits on a tool, and a tool cannot
      // exist until Gemini has finished a round trip — measured 2026-08-31 at
      // llm_tool_call_ms p50 1244ms, on top of ~570ms of Deepgram. That is the
      // floor for a tool-keyed line and there is no way under it.
      //
      // This turn is the exception, because the trigger is not a prediction
      // about what the model will do next. It is a fact about what the CALLER
      // just said, and lib/spellingSignal.js already reads it — the spelling
      // gate has consulted it since round 4. Nothing here is guessed.
      //
      // Reported live, and it earned its own fix by being specific: "after a
      // caller spells their name, there is a long silence. Honestly, that is
      // the only time I hear a long silence." It is also the turn LEAST likely
      // to call a tool at all — the model acknowledges and reads the details
      // back — so nothing keyed to a tool was ever going to cover it.
      //
      // Deliberately AFTER the transcript hold rather than at endpointing.
      // Spelling arrives in fragments and callers pause between letters; the
      // hold is what protects them from being cut off mid-name, and speaking
      // into that pause would talk over the very thing being spelled. Turn
      // start is early enough — it is ahead of the model, which is where the
      // second and a quarter actually goes.
      //
      // Takes the turn's single hold-line budget. If a tool follows, its line
      // is suppressed — which is correct: "writing that down" is true of a
      // booking write too, and two lines in one turn is the bug the budget
      // exists to prevent.
      if (engineFillerEnabled() && looksLikeSpelling(userText, getStrings(state.config))) {
        holdLinePlayed = true;
        playHoldLine("holdSpelling", { trigger: "spelling" });
      }

      // ...and whether ANY tool ran, which is the question the promise gate
      // actually needs: a promise made over a silent tool is still a promise
      // about work nobody can hear happening.
      let sawToolThisTurn = false;

      /**
       * Hand one batch of ready text to TTS, exactly as the ungated path does.
       *
       * Guarded because the promise gate can call this from a bare setTimeout
       * callback, where a throw has nowhere to go. playHoldLine wraps its own
       * write for the same reason: an aborted TTS turn throws on write, and
       * the turn's error path can abort activeTts while a gate timer is still
       * armed.
       */
      const speakSentence = (spoken) => {
        try {
        // Space-joined, not concatenated. toSpeakable trims each batch, so
        // appending bare glued sentences together — "Great.May I have your
        // full name?" — in the transcript, the dashboard, and the model's
        // own history. The audio was always fine: each batch is a separate
        // tts.write. Everything that reads the text afterwards was not.
          spokenThisTurn = spokenThisTurn ? `${spokenThisTurn} ${spoken}` : spoken;
          if (inFlightTurn) inFlightTurn.spokenText = spokenThisTurn;
          noteSpokenText(spoken);
          tts.write(spoken);
        } catch (err) {
          log.debug("promise_gate_write_failed", { callSid, reason: err?.message });
        }
      };

      /** No tool came. Speak the model's promise after all, slightly late. */
      const flushPromiseGate = ({ counts = true } = {}) => {
        if (!promiseGate) return;
        clearTimeout(promiseGate.timer);
        const { spoken } = promiseGate;
        promiseGate = null;
        // counts=false on the "more text arrived" path. metrics.js documents
        // _kept as "a promise that turned out to be true and was spoken as
        // written", and _swapped as the rate the prompt half should drive
        // down — so counting a gate that merely released text early inflates
        // the denominator and makes the ratio unreadable. Found in review.
        if (counts) bumpCounter("hold_line_promise_kept");
        speakSentence(spoken);
      };

      /**
       * A tool DID come. Throw the model's guess away and say what is actually
       * happening.
       *
       * Called from both orderings — a promise sitting in the gate when the
       * tool call arrives, and a tool call already seen when the promise
       * arrives — so it must not assume the gate is open. In the second case
       * there is nothing to cancel and the sentence is simply never spoken.
       *
       * Plays immediately rather than arming the usual 1500ms tool-hold timer:
       * we have just swallowed the only thing the caller was going to hear, so
       * waiting would trade a wrong line for a second of dead air. The
       * shouldPlayHoldLine predicate is deliberately not consulted for the
       * same reason — its job is to avoid stacking a line on top of the
       * model's, and the model's is gone.
       */
      const swapPromiseForHoldLine = (holdKind, holdTool = null) => {
        if (promiseGate) clearTimeout(promiseGate.timer);
        promiseGate = null;
        clearTimeout(toolHoldTimer);
        bumpCounter("hold_line_promise_swapped");
        // ARMED, not spoken on the spot.
        //
        // The first version played immediately, reasoning that we had just
        // swallowed the caller's only line and waiting would trade a wrong
        // line for dead air. Two things make that wrong.
        //
        // First, services/gemini.js yields `toolCall` AFTER the tool has
        // already run, so an instantly-REFUSED call — the spelling gate
        // declining a booking, say — would be announced as "Getting that
        // scheduled now" a moment before the model turns round and asks for a
        // spelling. Second, a silent tool like set_call_intent does no work at
        // all, so the real answer is arriving in a few hundred ms and any line
        // in front of it is chatter.
        //
        // A short timer fixes both without reintroducing the bug: whichever
        // comes first wins. Real text arrives → the delta branch clears this
        // and the caller simply hears the answer. Nothing arrives → the line
        // plays, because by then there IS a wait worth covering. Worst case is
        // ~750ms (gate + this) before a hold line, still half the 1500ms a
        // caller waits when the model says nothing at all.
        //
        // holdKind may be null — a tool mapped deliberately silent. In that
        // case the fallback is the neutral "One moment.", never the model's own
        // sentence: reported live as "it said 'Checking that now' and I am not
        // sure what it was checking". Vague is survivable; a specific claim
        // about work that is not happening is not.
        toolHoldTimer = setTimeout(() => {
          toolHoldTimer = null;
          if (state.speakEpoch !== myEpoch || holdLinePlayed) return;
          if (audioOut?.isPlaying()) return;
          holdLinePlayed = true;
          playHoldLine(holdKind || "filler", { trigger: "promise_swap", tool: holdTool });
        }, promiseSwapDelayMs());
        toolHoldTimer.unref?.();
      };

      /**
       * Would this sentence be safe to hold back? Only the FIRST thing said
       * this turn, only when the engine still owns the announcement, and only
       * when the sentence is a promise and nothing else.
       */
      const canGatePromise = (spoken) =>
        promiseGateMs() > 0 &&
        engineFillerEnabled() &&
        !spokenThisTurn &&
        !holdLinePlayed &&
        isPromiseOnly(spoken, getStrings(state.config).promiseRe);
      ...
      for await (const ev of gen) {
        ...
        if (ev.type === "delta") {
          ...
          producedText = true;
          // The model has started producing words, so TTS is already
          // synthesizing and audio is ~95ms behind. From here a resumption is
          // effectively a barge, and discarding the turn would throw away work
          // the caller is about to hear. See shouldAbortForResume.
          if (inFlightTurn) inFlightTurn.textStarted = true;
          // Text is arriving, so the caller is no longer waiting on silence.
          clearTimeout(toolHoldTimer);
          toolHoldTimer = null;
          sentenceBuf += ev.text;
          ...
          sentenceBuf = rest;
          if (ready) {
            const spoken = toSpeakable(ready, leakCtx());
            if (promiseGate) {
              // A gated promise followed by more words was never a bare
              // promise. Speak both, in order — and do not score it as a
              // promise we kept, because it was never on trial.
              flushPromiseGate({ counts: false });
              speakSentence(spoken);
            } else if (
              holdLinePlayed &&
              !spokenThisTurn &&
              ...
              // A hold line is already out, and the model has opened with a
              // bare promise anyway. Drop it.
              //
              // canGatePromise cannot reach this case: it requires
              // !holdLinePlayed, precisely because a line having played is what
              // normally means the gap is covered. That left a hole — the
              // engine speaks, and then the model's own "Checking that now"
              // follows it unchallenged, which is both the doubling this area
              // exists to prevent AND a claim that may be false.
              //
              // Found by the spelling line taking the turn's budget on a turn
              // whose existing test asserts that exact false sentence never
              // reaches the caller. It was reachable before that too, on any
              // turn where a tool line beat the model's first sentence.
              bumpCounter("hold_line_promise_swapped");
            } else if (canGatePromise(spoken)) {
              if (sawToolThisTurn) {
                // A tool already ran this turn, so there is nothing to wait
                // for: drop this sentence and let the engine answer for it.
                // Keyed on "a tool ran" rather than "a tool with a line ran",
                // because a promise made over a SILENT tool still needs
                // correcting — that is the "Checking that now" case.
                swapPromiseForHoldLine(pendingHoldKind, pendingHoldTool);
              } else {
                promiseGate = {
                  spoken,
                  timer: setTimeout(() => {
                    // A superseded turn must not speak: onInterrupt has
                    // already cleared the audio and bumped the epoch.
          ...
          // Same predicate the toolCall branch uses. Previously this checked
          // only producedText — any text at all this turn — which is why
          // `slow` won its race), so the check never rejected anything and a
          // second hold line played over the model's own promise.
          if (
            shouldPlayHoldLine({
              audioPlaying: !!audioOut?.isPlaying(),
              holdLinePlayed,
              spokenThisTurn,
              promiseRe: getStrings(state.config).promiseRe,
            })
          ) {
            holdLinePlayed = true;
            await playHoldLine("filler", { trigger: "slow" });
          }
          ...
          // ONE line per turn, whichever mechanism gets there first.
          //
          // This branch did not consult holdLinePlayed, so a turn could be
          // acknowledged by the tool path and then told "still working on
          // that" a couple of seconds later. On a reschedule the first line was
          // being suppressed and this one fired alone at 2500ms, which is what
          // was reported as the line coming "after too long" — it was a
          // different line.
          if (!holdLinePlayed && !audioOut?.isPlaying()) {
            holdLinePlayed = true;
            bumpCounter("llm_stalls");
            log.info("llm_stalled", turnCtx({ sinceLastChunkMs: ev.sinceLastChunkMs }));
            await playHoldLine("stillWorking", { trigger: "stalled" });
          }
        } else if (ev.type === "toolCall") {
          // A tool has STARTED. The prompt no longer asks the model to
          // announce a lookup — it satisfied that instruction with words and
          // no call, three turns running, and left the caller in silence. The
          // engine now owns the announcement, so it can only happen because
          // something is actually running.
          //
          // Spoken only once the wait has ALREADY run long — see
          // toolHoldDelayMs for why 1500 and not 0. Cancelled by text arriving,
          // which is the one event meaning the caller is about to hear the
          // answer, so a turn that answers quickly stays silent.
          metrics?.mark("llm_first_tool_call");
          // Only a WRITE blocks the resume path.
          //
          // This originally latched on any tool at all, which was too broad and
          // a live call showed the cost: on a booking, check_appointment_availability
          // fires early, so the guard was set for the rest of the turn and the
          // caller's continuation was queued and answered as a SECOND reply
          // instead of being folded in. Reported as "it said the whole response
          // to the first sentence and then immediately gave the response for the
          // second thing".
          //
          // The hazard the guard exists for — a tool completing after the abort,
          // never salvaged, so the merged turn books twice — only applies to
          // tools that write. A read is idempotent: discarding a turn after an
          // availability check costs nothing but the check itself.
          if (inFlightTurn && isWriteTool(ev.name)) inFlightTurn.toolStarted = true;
          // Remembered for the promise gate, which may see the model's "let me
          // check..." either before or after this event.
          //
          // This used to be first-kind-wins, on the reasoning that a turn which
          // looks up and then books should announce the lookup, "which is what
          // is happening when the caller is waiting". That reasoning is only
          // true while round one is still running. A caller reported the
          // opposite case: they gave their name, the turn checked availability
          // and THEN wrote, and they heard "Checking the calendar now." — a
          // description of work that had already finished, while they waited on
          // the write. The event is yielded after execution (services/gemini.js),
          // so by the time we read this, round one is over by definition.
          //
          // So: the most recent tool that has a line, for as long as nothing has
          // been said. Once a line is out, the turn's announcement is settled
          // and later rounds must not retroactively change what it meant.
          //
          // A silent tool must not ERASE a real kind — null here means "this one
          // has nothing to say", not "forget the round that did".
          //
          // A REFUSED call contributes nothing here for the same reason a
          // silent tool does not: the gates turned it away before it ran, so
          // there is no work to name.
          if (!holdLinePlayed && !ev.silent) {
            const k = holdKindForTool(ev.name);
            if (k) {
              pendingHoldKind = k;
              pendingHoldTool = ev.name;
            }
          }
          if (ev.silent) bumpCounter("hold_line_suppressed");
          sawToolThisTurn = true;
          // The model already said "let me check something" and we held it
          // back. Now we know what it is actually doing, so say that instead.
          // Note there is no `&& holdKindForTool(ev.name)` here any more. A
          // silent tool still has to answer for a promise we swallowed: the
          // caller was told something was being checked, and if we drop that
          // sentence and say nothing at all they get neither the claim nor a
          // correction. swapPromiseForHoldLine falls back to "One moment."
          //
          // A refused call passes `null` rather than its kind, so the swap
          // falls back to the neutral "One moment." The promise still has to be
          // swallowed — the model claimed work that the gates just declined —
          // but replacing it with an equally specific claim would only trade
          // one false statement for another. And the correction is usually
          // never spoken at all: a refusal did no work, so the model's real
          // reply ("could you spell that?") lands inside the swap delay and the
          // delta branch cancels the timer.
          if (engineFillerEnabled() && promiseGate) {
            swapPromiseForHoldLine(ev.silent ? null : holdKindForTool(ev.name), ev.name);
          } else if (
            engineFillerEnabled() &&
            !ev.silent &&
            holdKindForTool(ev.name) &&
            !toolHoldTimer &&
            !holdLinePlayed
          ) {
            // Which line, decided from the tool that actually started. "One
            // moment." is accurate for everything and says nothing; a
            // receptionist checking the diary says she is checking the diary.
            // Captured now because ev is gone by the time the timer fires.
            // null = a tool that is deliberately silent (set_call_intent, which
            // fires on EVERY turn under the intent marker; end_call; the quick
            // message and quote writes). Do not even arm a timer for those.
            const holdKind = holdKindForTool(ev.name);
            const holdTool = ev.name;
            // RE-CHECKS rather than giving up.
            //
            // It used to fire once and abandon the line if the model's own
            // audio happened to still be playing at that instant. On a
            // reschedule that is exactly what happens: the model says "just to
            // confirm..." and calls a tool in the same breath, so the one shot
            // lands while the caller is still hearing that sentence, returns
            // false, and the three seconds after it are silent. Reported as the
            // line arriving "after too long rather than ASAP" -- what was
            // actually heard that late was the separate stall watchdog.
            //
            // So while the ONLY thing standing in the way is audio still
            // playing, it looks again shortly. Cancelled by text arriving (the
            // delta branch), by the turn ending, and bounded by a retry cap so
            // it can never spin.
            let holdRetries = 0;
            const armToolHold = (delay) => {
              toolHoldTimer = setTimeout(() => {
                toolHoldTimer = null;
                if (state.speakEpoch !== myEpoch || holdLinePlayed) return;
                if (audioOut?.isPlaying()) {
                  if (holdRetries++ < HOLD_RETRY_MAX) armToolHold(HOLD_RETRY_MS);
                  return;
                }
                if (
                  !shouldPlayHoldLine({
                ...
                holdLinePlayed = true;
                playHoldLine(holdKind, { trigger: "tool", tool: holdTool });
              }, delay);
              toolHoldTimer.unref?.();
            };
            armToolHold(toolHoldDelayMs());
          }
        } else if (ev.type === "toolEffect") {
          // DELIBERATELY does not cancel the hold timer any more.
          //
          // It used to, on the reasoning that a finished tool meant the answer
          // was imminent. Measured on staging 2026-08-30, that is backwards:
          // tool_exec_ms p50 is ZERO — the database answers instantly — and the
          // caller then waits ~2s for the SECOND model round-trip to write the
          // reply. So cancelling here cancelled the cover for a pause that had
          // not started yet, and a live call reported 4-5 seconds of silence on
          // every tool turn.
          //
          // Only text arriving means the caller is about to hear something, and
          // the delta branch above already cancels on exactly that. A fast tool
          // still earns no gratuitous "one moment", because a fast tool is
          // followed by a fast delta.
          //
      ...
      if (state.speakEpoch !== myEpoch) {
        // A barged turn speaks nothing further, so a gated promise is dropped
        // rather than flushed — its timer checks the epoch too, but clearing
        // here means it cannot fire against a turn that is already gone.
        if (promiseGate) clearTimeout(promiseGate.timer);
        promiseGate = null;
      ...
      // The turn ended with a promise still held back and no tool call to
      // replace it, so the model was right and we were wrong: speak it. This
      // has to happen before tts.end() below, and before the sentence buffer,
      // or the caller loses the line entirely and hears nothing at all.
      flushPromiseGate();

      // Flush whatever's left in the sentence buffer (a final partial
      // sentence with no trailing punctuation, e.g. "...call us back!").
      if (sentenceBuf) {
        const spoken = toSpeakable(sentenceBuf, leakCtx());
        spokenThisTurn = spokenThisTurn ? `${spokenThisTurn} ${spoken}` : spoken;
        if (inFlightTurn) inFlightTurn.spokenText = spokenThisTurn;
        noteSpokenText(spoken);
        tts.write(spoken);
      ...
      applyReply(userText, reply, callerSeq, markName, producedText, spokenThisTurn);
    } catch (err) {
      handleTurnError(err, myEpoch, turnEffects);
    } finally {
      // A gated promise must never outlive the turn that produced it.
      //
      // The normal and barged paths both dispose of it, but the ERROR path did
      // not: handleTurnError aborts activeTts and may start a fallback flow or
      // an apology, and 350ms later the gate timer would fire and write the
      // swallowed sentence into a dead stream — splicing "Let me check our
      // calendar" into the apology, and calling noteSpokenText on audio nobody
      // heard, which corrupts echo detection. Found in review. Cleared here,
      // in the one block that runs however the turn ended.
      if (promiseGate) clearTimeout(promiseGate.timer);
      promiseGate = null;
  ...
  function dispatchCapabilityEffects(effects) {
    ...
    return dispatchEffects(effects, {
      ...
      // The call-start caller snapshot is stale the moment this call changes
      // anything. A pack may not reach into session state, so it computes the
      // next value and hands it back here — the same shape as
      // setCapabilityState. Without this, a caller who books at 10:00 and then
      // asks for 11:00 later in the SAME call is not recognised as already
      // having one, and CALLER CONTEXT keeps asserting an appointment the
      // caller just cancelled.
  ...
  function recordDurableEffect(effect, turnEffects) {
    ...
    // Latch "this call has actually done something for the caller", so a
    // goodbye arriving on a LATER turn than the action can still end the call.
    // Recorded here rather than in applyReply because the write has already
    // happened by this point — a barge over the confirmation sentence does not
    // un-book the appointment, and must not un-earn the right to hang up.
    if (effect.success && ACTION_TOOL_NAMES.includes(effect.name)) {
      state.completedActionThisCall = true;
    }
  }
   ...
   */
  function applyReply(userText, reply, callerSeq, markName, producedText, spokenText = "") {
    const {
      text: replyText,
      endCallArgs,
      toolResults,
    ...
    // wins over it.
    // History carries what was SPOKEN, for the same reason the transcript does:
    // a model that reads its own leaked implementation vocabulary in its own
    // history keeps writing in that register. Only the text is swapped — every
    // other channel on the reply (tool results, effects, end-call args) is
    // untouched, and the reducer stays pure.
    const historySafe = spokenText.trim() ? { ...reply, text: spokenText.trim() } : reply;
    const { intentSet, ended } = applyReplyState(
      state,
      { userText, reply: historySafe },
      {
        STEPS,
        mergeCapabilityState,
        dispatchEffects: dispatchCapabilityEffects,
    ...
    // What the caller actually HEARD, not what the model produced. The two
    // differ exactly when the leak guard fired, and that is the case where the
    // raw text is least worth keeping: storing it re-fed the model its own
    // implementation vocabulary on the next turn, which is how one leaked token
    // turned into eleven turns of the caller asking what software we run.
    // The raw text is not lost — it goes to the outbound_sanitized log line.
    logAiTranscript(spokenText.trim() || replyText, callerSeq);
  ...
  function startStt(language, endpointing, keyterms = [], tier = "standard") {
    createSttStream({
      // Which vendor may hear this caller. The STRICTER of the deployment and
      // the tenant, resolved once at pickup — not a per-turn decision, and not
      // an environment variable. See lib/voice/sttStream.js.
      tier,
      language,
      ...
      callSid,
      onFinal: (text, meta) => {
        // meta.confidence is Deepgram's own certainty about this transcript,
        // forwarded so a low-confidence scrap cannot interrupt the assistant.
        // It was produced by sttStream and dropped here for the whole life of
        // the pipeline — no threshold existed anywhere in the repo.
        const decision = turnManager?.handleFinal(text, meta);
        ...
        // "echo_short" is the same situation seen by a different gate: a 1-3
        // word final wholly contained in what the AI just said, which the main
        // classifier structurally cannot judge (it needs 4+ tokens for bigram
        // similarity to mean anything). It must re-arm the ladder identically,
        // or a line that echoes single words goes permanently quiet.
        if (decision?.action === "ignore" && (decision.reason === "echo" || decision.reason === "echo_short")) {
          bumpCounter("echo_suppressed_final");
          armSilenceTimer(false);
        }
        // Spoken over the greeting. Dropped, not kept: this gate sits above
        // turnManager's word-count/voiced/confidence checks, so anything held
        // here would be unfiltered — see GREETING_UNINTERRUPTIBLE.
        //
        // The arm is unconditional, matching the two echo branches above it: an
        // empty greeting-gated final still has to re-arm the ladder or a line
        // that produces them goes permanently quiet. Only the counter is gated
        // on there being words, because the counter measures what a caller
        // actually lost.
        if (decision?.action === "ignore" && decision.reason === "greeting") {
          const clean = (text || "").trim();
          if (clean) {
            log.info("greeting_speech_discarded", { callSid, chars: clean.length });
  ...
  function handleSttError(err) {
    ...
    speakThenClose(getStrings(state?.config).sttFailGoodbye, "stt-error-goodbye-done", { final: true });
  }
  ...
  ws.on("message", async (raw) => {
    ...
    switch (msg.event) {
      ...
      case "start": {
        ...
        // Wall clock for greeting_audio_timing. Taken at the top of the handler
        // rather than beside the greeting, because everything between the two —
        // the business lookup on the socket path, config load, audioOut/VAD/STT
        // construction — is time the caller is on an answered line hearing
        // nothing, and is exactly what a "the greeting starts late/clipped"
        // report is describing.
        greetingTiming = { startAtMs: Date.now(), wireLogged: false };

        // P10, second half. The upgrade proved the caller holds a token minted
        // by /twilio/voice; this proves they are using it for the call it was
        // minted for. Without it one valid token authorises a session for any
        // OTHER call SID the sender cares to name — and businessPhone arrives
        // in customParameters right below, so that would be a session against
        // a tenant of their choosing.
        //
        // null when enforcement is off (TWILIO_VALIDATE_SIGNATURE=false), which
        // is the same switch that turns off signature validation.
        if (ws.authorizedCallSid && ws.authorizedCallSid !== callSid) {
          log.error("media_stream_callsid_mismatch", {
        ...
        // No callerPhone. It is the caller's number, and callSid resolves to
        // it through `calls` for anyone with a reason to look.
        log.info("media_stream_start", { callSid, streamSid, businessPhone, pipeline: "v2" });
        ...
        // /twilio/voice already resolved the business before returning the
        // <Connect><Stream> TwiML (it has to — an unroutable number is answered
        // by a transfer or voicemail and never reaches this socket at all).
        // Reusing that row removes a Supabase round trip from the pickup path,
        // which runs before the caller hears a single byte of the greeting.
        //
        // The re-query is kept for the cases where the handoff cannot carry:
        // more than one server instance (callState is an in-process Map), or a
        // socket that arrived without going through the webhook.
        let business = state.business ?? null;
        if (!business && db.isEnabled()) {
          business = await db.lookupBusinessByPhone(businessPhone);
          if (!business) log.error("no_business_found", { callSid, businessPhone, stage: "media_stream", severity: "warn" });
        }
        ...
        state.contextPromise = (async () => {
          ...
          // All four in ONE round trip's worth of wall clock, not two.
          // createCall used to be awaited BEFORE the other three even started,
          // and startTurn awaits this whole promise (see ensureContext) — so
          // the first caller turn, the one they judge the product on, paid two
          // serial Supabase round trips. Nothing here consumes dbId.
          //
          // allSettled, not all: a failed createCall must not cost the
          // knowledge base and integrations, which matter far more to whether
          // turn one is any good. Each result is applied on its own merits.
          //
          // Scoped to this tenant for the duration of the four queries. The
          // business was resolved above, by a bootstrap lookup that runs before
          // any tenant is known; from here on everything this call touches is
          // one clinic's data, and row-level security enforces it rather than
          // each function remembering.
          //
          // A UNIT OF WORK, not the call: this holds one pooled connection and
          // an open transaction, and it closes in milliseconds when these four
          // settle. Wrapping the whole call would pin a connection for minutes.
          const [callRes, knowledgeRes, integrationsRes, callerRes] = await db.withTenantSafe(
            business.id,
            () =>
              Promise.allSettled([
                db.createCall(business.id, callSid, callerPhone, businessPhone),
            ...
            {
              operation: "sessionPickupContext",
              callSid,
              // The shape allSettled would have produced, so the four
              // destructured results below are unchanged even if the scope
              // itself fails. Losing the knowledge base must not also throw.
              fallback: [
                { status: "rejected" },
                { status: "rejected" },
                { status: "rejected" },
          ...
          if (callRes.status === "fulfilled" && callRes.value) {
            ...
            // Call BOUNDARY write, once, at pickup. `/twilio/status` runs on
            // whichever instance the load balancer picks, which is usually not
            // this one, and without these two it has no call row to summarise
            // and no tenant to notify.
            callState.writeShared(callSid, state);
          }
          state.knowledge = knowledgeRes.status === "fulfilled" ? knowledgeRes.value || [] : [];
          state.integrations = integrationsRes.status === "fulfilled" ? integrationsRes.value || [] : [];
          state.callerContext = callerRes.status === "fulfilled" ? callerRes.value : null;
          ...
          for (const [what, res] of [
            ...
            if (res.status === "rejected") {
              log.error("context_load_failed", { callSid, part: what, reason: res.reason?.message });
            }
          }
        })();
        ...
        // ---- Background: build this call's Gemini context cache ----
        // Chained off contextPromise, NOT fired at pickup: the knowledge base
        // and the integrations list are both part of the cached prefix, so
        // warming before they land would hash a different key and build a cache
        // this call can never read — paying storage for nothing while every
        // turn still misses.
        //
        // Never awaited, rejection swallowed: the same contract as
        // state.warmUtterances below. Creating a cache is a 200-500ms round
        // trip, and the whole point of doing it here is that it happens behind
        // the greeting and the caller's first utterance instead of underneath
        // turn 1. If it hasn't finished by the time they speak, turn 1 runs
        // uncached exactly as it does today — warming can only ever save money,
        // never cost latency.
        if (business) {
          state.contextPromise
            .then(() => geminiService.warmPromptCache(config, buildExtras(config)))
            .catch(() => {});
        }
        ...
        audioOut = createAudioOut({
          ...
          onFirstFrameWire: () => {
            state.turnMetrics?.mark("first_frame_wire");
            // First audio of the CALL reaching Twilio, logged once. With a
            // preroll this is the silence, which is the point: it measures how
            // long the caller sat on an answered line before this process put
            // anything on the wire at all.
            if (greetingTiming && greetingTiming.firstWireMs === undefined) {
              greetingTiming.firstWireMs = Date.now() - greetingTiming.startAtMs;
              logGreetingTiming();
            }
          },
        });

        // ---- Prime the audio path before the greeting ----
        // See greetingPrerollMs(). Enqueued before anything else so it is the
        // first thing in the pacing queue and cannot get behind the greeting.
        const prerollMs = greetingPrerollMs();
        if (prerollMs > 0) {
          audioOut.enqueue(Buffer.alloc(prerollMs * MULAW_BYTES_PER_MS, 0xff));
        }
        vad = createVad();
        ...
        turnManager = createTurnManager({
          ...
          // May the caller take the floor? Closed only for the greeting.
          //
          // Three ways out, because a gate that sticks shut deafens the call
          // for its whole duration — the one outcome worse than a clipped
          // greeting. The mark is the normal one; the elapsed and turn checks
          // hold even if the flag or its timer are somehow left behind.
          bargeInAllowed: () => {
            if (!greetingActive) return true;
            if (state?.turnId > 0) return true;
            const startedAt = state?.callStartTime;
            if (startedAt && Date.now() - startedAt > GREETING_GUARD_MAX_MS) return true;
            return false;
          },
        });
        ...
        // detect language.
        const sttLanguage = mapLanguage(config, { callerNumber: callerPhone });
        ...
        startStt(
          sttLanguage,
          sttLanguage === "multi" ? 100 : undefined,
          keytermsFromConfig(config),
        ...
        if (GREETING_UNINTERRUPTIBLE) {
          greetingActive = true;
          greetingGuardTimer = setTimeout(() => {
            if (!greetingActive) return;
            log.error("greeting_guard_expired", { callSid, severity: "warn" });
            bumpCounter("greeting_guard_expired");
            endGreetingPhase();
          }, GREETING_GUARD_MAX_MS);
          greetingGuardTimer.unref?.();
        }
        ...
        speakText(
          ...
          (payload) => {
            ...
            // The greeting has finished playing, so the vendor is free — warm the
            // micro-utterance cache now rather than racing pickup for it.
            state.warmUtterances?.();
          },
          // The greeting's own timing, which nothing has ever reported.
          //
          // firstWireMs is start-event -> first audio frame on the socket (the
          // preroll, when one is configured). ttsFirstByteMs is start-event ->
          // ElevenLabs' first byte. The gap between them says how much of the
          // preroll was covering dead air rather than adding to it: if the pad
          // reliably runs out before the greeting is ready, it covered nothing
          // and should come down. logGreetingTiming emits once both are in.
          () => {
            if (!greetingTiming) return;
            greetingTiming.ttsFirstByteMs = Date.now() - greetingTiming.startAtMs;
            greetingTiming.prerollMs = prerollMs;
            greetingTiming.greetingChars = greetingText.length;
            logGreetingTiming();
          },
        );
        ...
        // DEFERRED off the pickup path. warm() is a SEQUENTIAL for-await over
        // ~9 entries (lib/voice/utteranceCache.js), and for an ElevenLabs
        // business every one of them is a one-shot EL synthesis against the
        // same account and rate limit the greeting and first turn are using.
        // Firing them at "start" put nine requests in front of the two that the
        // caller is actually waiting on.
        //
        // Nothing is lost by waiting: the LRU is process-wide and shared across
        // calls, the warmed lines are fillers/nudges/goodbye that cannot occur
        // during the greeting anyway, and a miss falls through to live
        // synthesis exactly as it does today. Warmup failures stay swallowed
        // per-entry inside warm() and never break the call.
        state.warmUtterances = () => {
          state.warmUtterances = null; // once per call
        ...
        // The greeting's onSettled fires only when the greeting was NOT barged,
        // so a caller who talks over it would otherwise leave the cache cold for
        // the rest of the call. Self-nulling makes this idempotent with that
        // path — whichever runs first wins.
        // Held in a call-scoped handle so cleanup() can cancel it. Untracked,
        // it still fired ~8s after a call had already ended and kicked off a
        // sequential run of ElevenLabs syntheses for a caller who was no longer
        // on the line — billed characters for audio nobody could hear.
        warmBackstopTimer = setTimeout(() => state.warmUtterances?.(), UTTERANCE_WARM_BACKSTOP_MS);
        warmBackstopTimer.unref?.();

        // ---- Hard call-duration limit ----
        callDurationTimer = setTimeout(() => {
          if (state.step === STEPS.ENDING) return;
          speakThenClose(getStrings(state.config).maxDuration, "maxdur-goodbye-done", { final: true });
      ...
      case "mark": {
        ...
        // Before the generic "-done" re-arm below, so the ordering is stated
        // rather than incidental. endGreetingPhase arms the ladder for the
        // benefit of the watchdog path; on THIS path the generic arm below
        // re-arms with reset=true a moment later and wins. Anything the caller
        // said over the greeting was already dropped as it arrived.
        if (markName === "greeting-done") {
          endGreetingPhase();
        }
  ...
  function cleanup() {
    ...
    clearTimeout(closeFallbackTimer);
    clearTimeout(transferFallbackTimer);
    clearTimeout(warmBackstopTimer);
    clearTimeout(greetingGuardTimer);
    greetingActive = false;
    heldText = "";
    holdStartedAt = null;
