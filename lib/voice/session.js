...
// The engine's tool-round hold line. OFF by default since 2026-08-30.
//
// It was on, because the prompt's version of the same idea failed badly: the
// model obeyed "say one moment" and dropped the tool call on three consecutive
// turns, leaving the caller in silence. Emitted here it could not be a false
// promise — it fired only because a tool actually started.
//
// What that reasoning never accounted for is that THE LINE TAKES TIME TO SAY.
// Audio plays serially, so if the answer arrives while the line is still
// speaking, the line has ADDED latency rather than hidden it. Firing at time T
// only helps if the answer comes after T + ~1.5s (the length of the sentence).
//
// Measured on staging 2026-08-30, the gap it was covering is:
//
//   llm_reply_after_tool_ms   p50 833ms   p95 896ms
//
// which is comfortably shorter than the sentence itself. There is no threshold
// that wins: the wait is essentially always shorter than the words meant to
// cover it. A caller reported exactly this — "it says the thing and then
// immediately comes back with the results, why did it even say that" — and the
// honest reading is that the filler was making those turns LONGER.
//
// Set to "true" to bring it back. It only becomes defensible if real waits turn
// out to be several seconds, and even then a reactive timer cannot cover the
// start of a wait it does not yet know is happening.
// Read at CALL time, not module load — the convention lib/transcriptUtils.js
// documents for its own env reads. A module-level read freezes at whatever the
// environment held on first import, which makes the flag untestable and means a
// Railway change needs a deploy rather than a restart.
function engineFillerEnabled() {
  return process.env.VOICE_ENGINE_FILLER === "true";
}
