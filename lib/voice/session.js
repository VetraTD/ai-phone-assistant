...
export function handleVoiceSessionConnection(ws, _req, { now = () => performance.now() } = {}) {
  ...
  // Which variant of a hold line to use next. Per CALL, not per turn, so a
  // caller hearing three tool rounds hears three different sentences.
  let holdLineNo = 0;
  ...
  async function playHoldLine(kind = "filler") {
    const epoch = state.speakEpoch;
    const text = holdLineFor(getStrings(state.config), kind, holdLineNo++);
    if (!text) return;
