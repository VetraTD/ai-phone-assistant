...
const TOOL_HOLD_DELAY_MS = (() => {
  const v = Number.parseInt(process.env.VOICE_TOOL_HOLD_DELAY_MS, 10);
  // 0 = the instant the tool starts. This is an ACKNOWLEDGEMENT, not a
  // gap-filler: it is the line's own ~1.5s of speech that covers the wait, so
  // delaying it only produces the thing a caller notices - a line that lands
  // just before the answer it was supposed to precede.
    ...
    try {
      ...
      for await (const ev of gen) {
        ...
        if (ev.type === "delta") {
          ...
          //
          // Spoken IMMEDIATELY (VOICE_TOOL_HOLD_DELAY_MS defaults to 0). The
          // line is an acknowledgement, not a gap-filler: its own ~1.5s of
          // speech is the pause. Cancelled only by text arriving, which is the
          // one event meaning the caller is about to hear the answer.
