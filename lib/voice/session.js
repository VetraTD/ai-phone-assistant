...
import { isPromiseOnly, promiseGateMs, engineFillerEnabled } from "./promiseGate.js";
import {
  judgeTurnComplete,
...
export function handleVoiceSessionConnection(ws, _req, { now = () => performance.now() } = {}) {
  ...
  async function startTurn(userText) {
    ...
    try {
       ...
       */
      const swapPromiseForHoldLine = async (holdKind) => {
        ...
        if (ev.type === "delta") {
          ...
          if (ready) {
            ...
            if (promiseGate) {
              ...
              if (pendingHoldKind) {
                // The tool call already arrived, so there is nothing to wait
                // for and nothing to gate: drop this sentence on the spot.
                await swapPromiseForHoldLine(pendingHoldKind);
              } else {
                promiseGate = {
                  spoken,
                  timer: setTimeout(() => {
          ...
          if (engineFillerEnabled() && promiseGate && holdKindForTool(ev.name)) {
            await swapPromiseForHoldLine(holdKindForTool(ev.name));
