...
import { getStrings, holdKindForTool, holdLineFor, holdLineVariants } from "./strings.js";
...
function buildUtteranceWarmEntries(config) {
  ...
  const entries = [
    ...
    ...["holdAvailability", "holdLookup", "holdWrite"].flatMap((k) =>
      holdLineVariants(S, k).map((text) => ({ kind: k, text }))
    ),
