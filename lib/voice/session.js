...
import { isPromiseOnly, promiseGateMs } from "./promiseGate.js";
import {
  judgeTurnComplete,
...
import { applyReplyState, systemNoteEntry, spellingSettled } from "./replyState.js";
...
export function handleVoiceSessionConnection(ws, _req, { now = () => performance.now() } = {}) {
  ...
  // { startAtMs, wireLogged } — see greetingPrerollMs(). The greeting is spoken
  // with isTurn=false, so none of the per-turn latency marks are recorded for
  // it and its own timing has never appeared in any number we report.
  ...
  async function arbitrateHold({ fragment, rule, holdMs, seq }) {
    ...
    if (verdict.complete) {
      ...
      bumpCounter("semantic_endpoint_flushed");
      clearTimeout(holdTimer);
      holdTimer = null;
      onHoldExpired();
      return;
    ...
    // Mid-sentence. Give them the rest of the chain budget rather than a fixed
    // extension: this is the case a fixed hold provably cannot cover, and the
    // ceiling is what stops it running away.
    const elapsed = holdStartedAt === null ? 0 : Date.now() - holdStartedAt;
    const remaining = holdChainCeiling() - elapsed;
  ...
  async function startTurn(userText) {
    ...
    try {
      ...
      if (contextWasPending) metrics?.mark("context_wait_start");
      await ensureContext();
      if (contextWasPending) metrics?.mark("context_wait_end");
      ...
      let spokenThisTurn = "";
      // A leading sentence that is nothing but "hold on", held back until we
      // know whether a tool call follows it. See lib/voice/promiseGate.js.
  ...
  function applyReply(userText, reply, callerSeq, markName, producedText, spokenText = "") {
    ...
    const { intentSet, ended } = applyReplyState(
      ...
      { userText, reply: historySafe },
      {
        ...
        // The caller's half: lets the reducer see letters and refusals, not
        // just our own questions. Without it the block would reopen on every
        // ask and never close on an answer.
        strings: getStrings(state.config),
      }
    );
      ...
      case "start": {
        ...
        audioOut = createAudioOut({
          ...
          onFirstFrameWire: () => {
            ...
            if (greetingTiming && !greetingTiming.wireLogged) {
              greetingTiming.wireLogged = true;
        ...
        speakText(
          ...
          // The greeting's own timing, which nothing has ever reported. Emitted
          // on its first synthesized byte rather than on completion so a call
          // that dies mid-greeting still yields the numbers.
          //
          ...
          // ElevenLabs' first byte. The gap between them is how much of the
          // preroll was actually covering dead air rather than adding to it: if
          // ttsFirstByteMs is reliably under prerollMs, the pad is pure added
          // latency and should come down.
          () => {
            if (!greetingTiming) return;
            log.info("greeting_audio_timing", {
              callSid,
              prerollMs,
              firstWireMs: greetingTiming.firstWireMs ?? null,
              ttsFirstByteMs: Date.now() - greetingTiming.startAtMs,
              greetingChars: greetingText.length,
            });
          },
        );
