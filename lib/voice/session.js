...
// Twilio <Dial> ringback tone locale (see redialForTransfer).
const DIAL_RING_TONE = "us";
...
function buildGreeting(config) {
  ...
  if (!config._hasCustomGreeting) {
    ...
    // instead: time-of-day integrated, business name present.
    ...
    text += `${tod}, thanks for calling ${businessName}. How can I help you today?`;
  } else {
...
export function handleVoiceSessionConnection(ws, _req) {
  ...
  function armSilenceTimer(reset = true) {
    ...
    if (state.processingTurn || audioOut?.isPlaying()) {
  ...
  function handleCallerFinal(text) {
    if ((text || "").trim()) state.sawCallerFinal = true;
  ...
  async function redialForTransfer(transferNumber, trigger) {
    try {
      ...
      await client.calls(callSid).update({
        twiml: `<Response><Dial ringTone="${DIAL_RING_TONE}"${callerIdAttr}>${escapeXml(transferNumber)}</Dial></Response>`,
      });
  ...
  ws.on("message", async (raw) => {
    ...
    switch (msg.event) {
      ...
      case "mark": {
        ...
        if (state && /^turn-\d+-done$/.test(markName)) {
