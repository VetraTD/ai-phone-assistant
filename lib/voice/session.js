        ...
        if (ev.type === "delta") {
          ...
          if (VOICE_ENGINE_FILLER && !toolHoldTimer && !holdLinePlayed) {
            toolHoldTimer = setTimeout(() => {
              ...
              // The model volunteered its own "one moment" this turn — saying
              // it again is worse than not saying it at all.
              if (getStrings(state.config).promiseRe?.test(spokenThisTurn)) return;
              holdLinePlayed = true;
              playHoldLine("filler");
            }, TOOL_HOLD_DELAY_MS);
            toolHoldTimer.unref?.();
