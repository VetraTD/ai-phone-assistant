...
export function handleVoiceSessionConnection(ws, _req) {
  ...
  function beginSpeaking(markName, isTurn, onSettled) {
    ...
    const tts = createTtsTurn({
      ...
      onDone: (payload = {}) => {
        ...
        // A REAL full-turn Google fallback just played (usedFallback, and this
        // was an actual caller turn — not a fixed micro-utterance, isTurn) for
        // a business whose intended engine was ElevenLabs (!forceFallback).
        // Make the rest of THIS call sticky-Google so later turns don't
        // ping-pong back to EL as the breaker half-opens. A micro-utterance
        // fallback (isTurn=false) deliberately does NOT stick.
   ...
   */
  function playCachedBuffer(buf, markName) {
    activeTts = null;
    const epoch = state.speakEpoch;
  ...
  function speakTextCacheable(text, markName) {
    ...
    if (!forceFallback) {
      ...
      if (elCached) {
        ...
        playCachedBuffer(elCached, markName);
        return null;
      }
      ...
      if (gCached) {
        playCachedBuffer(gCached, markName);
        return null;
      }
      return speakText(speakable, markName);
    }
    ...
    playCachedBuffer(cached, markName);
    return null;
  }
