...
// off the front of the call, and the caller then has no idea who answered. The
// cost is bounded because a final spoken over the greeting is HELD and replayed
// into the first turn rather than discarded (see greetingHeldText) — the caller
// never has to repeat themselves, they just wait out the greeting.
...
export function handleVoiceSessionConnection(ws, _req, { now = () => performance.now() } = {}) {
  ...
  // caller says meanwhile accumulates in greetingHeldText and is replayed as
  // the first turn, so "uninterruptible" costs the caller a wait rather than
  // their words. The timer is the backstop for a completion mark that never
  // arrives — see GREETING_GUARD_MAX_MS.
  let greetingActive = false;
  let greetingHeldText = "";
  ...
  /**
   * Reopen the barge-in gate and hand the first turn whatever the caller said
   * over the greeting.
   *
   * Idempotent: the completion mark and the watchdog can both reach this, and
   * on a fast call they race. Replays through handleCallerFinal rather than
   * startTurn so the held text meets the same hold/echo/intent handling as any
   * other final — it IS an ordinary final, it just arrived early.
   */
  function endGreetingPhase() {
    ...
    const carried = greetingHeldText.trim();
    greetingHeldText = "";
    if (carried) {
      log.info("greeting_speech_carried", { callSid, chars: carried.length });
      bumpCounter("greeting_speech_carried");
      handleCallerFinal(carried);
    }
  }
  ...
  function startStt(language, endpointing, keyterms = []) {
    createSttStream({
      ...
      onFinal: (text, meta) => {
        ...
        // Spoken over the greeting. Keep it — endGreetingPhase replays it as
        // the first turn, so an impatient caller is made to wait, never to
        // repeat themselves. Accumulated because a caller talking through a
        // long greeting produces several finals.
        if (decision?.action === "ignore" && decision.reason === "greeting") {
          const clean = (text || "").trim();
          if (clean) greetingHeldText = greetingHeldText ? `${greetingHeldText} ${clean}` : clean;
          armSilenceTimer(false);
        }
      },
      onInterim: (text, meta) => {
  ...
  ws.on("message", async (raw) => {
    ...
    switch (msg.event) {
      ...
      case "mark": {
        ...
        // rather than incidental: the gate reopens and any speech held over
        // the greeting becomes the first turn, which arms the ladder itself.
