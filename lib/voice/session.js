...
// How long a tool round may run before the caller is told to hold.
//
// 1500ms, and the number is derived, not chosen. A hold line takes ~1.5s to
// SAY, and audio plays serially — so firing at T only helps if the answer
// arrives after T + 1.5s. Anything sooner and the line delays the answer it was
// meant to cover.
//
// Measured over one live call, 2026-08-30, llm_reply_after_tool_ms:
//
//   685  722  734  764  794  818  827  835  861  ...  3133
//
// Almost every turn answers in well under a second — those must stay SILENT,
// and at 1500 they never reach the timer. The 3133ms outlier (a reschedule
// after an identity check) fires at 1500 with 1633ms still to cover, which is
// just enough for the line to earn its place.
//
// This has moved three times: 600 (debounced, fired on turns that did not need
// it), 0 (spoke on every single tool call — reported as chatter), and now a
// threshold picked from the distribution instead of from a preference.
function toolHoldDelayMs() {
  const v = Number.parseInt(process.env.VOICE_TOOL_HOLD_DELAY_MS, 10);
  return Number.isFinite(v) && v >= 0 && v <= 5_000 ? v : 1500;
}
...
export function handleVoiceSessionConnection(ws, _req, { now = () => performance.now() } = {}) {
  ...
  async function startTurn(userText) {
    ...
    try {
       ...
       */
      const swapPromiseForHoldLine = (holdKind) => {
      ...
      for await (const ev of gen) {
        ...
        if (ev.type === "delta") {
          ...
          if (ready) {
            ...
            if (promiseGate) {
              ...
              if (sawToolThisTurn) {
                ...
                swapPromiseForHoldLine(pendingHoldKind);
              } else {
                promiseGate = {
                  spoken,
                  timer: setTimeout(() => {
          ...
          // check..." either before or after this event. Only the FIRST kind
          // this turn: a turn that looks up and then books should announce the
          // lookup, which is what is happening when the caller is waiting.
          if (!pendingHoldKind) pendingHoldKind = holdKindForTool(ev.name);
          ...
          if (engineFillerEnabled() && promiseGate) {
            swapPromiseForHoldLine(holdKindForTool(ev.name));
          } else if (engineFillerEnabled() && holdKindForTool(ev.name) && !toolHoldTimer && !holdLinePlayed) {
