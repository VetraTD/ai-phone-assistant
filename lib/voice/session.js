...
import { applyReplyState, systemNoteEntry, hasSpentSpellingAsk } from "./replyState.js";
...
export function handleVoiceSessionConnection(ws, _req, { now = () => performance.now() } = {}) {
  ...
  function buildExtras(config) {
    return {
      ...
      // Has this call already spent its one spelling request? Counted from the
      // assistant's own spoken text (see runLlmTurn) rather than trusted to
      // the prompt, and surfaced here so the dynamic tail can state it as a
      // fact instead of asking the model to remember.
      spellingAlreadyAsked: hasSpentSpellingAsk(state),
    };
  }
