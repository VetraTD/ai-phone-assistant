...
// Read at CALL time, not module load — same convention as engineFillerEnabled
// and lib/transcriptUtils.js. A module-level read freezes on first import.
function secondHoldMs() {
  const v = Number.parseInt(process.env.VOICE_SECOND_HOLD_MS, 10);
  return Number.isFinite(v) && v >= 0 && v <= 10_000 ? v : 2500;
}
...
export function handleVoiceSessionConnection(ws, _req, { now = () => performance.now() } = {}) {
  ...
  async function startTurn(userText) {
    ...
    try {
      ...
      // A turn can run SEVERAL tool rounds — a reschedule looks the appointment
      // up, checks identity, then answers — and holdLinePlayed caps the
      // acknowledgement at one. A live call on 2026-08-30 spent ~4s inside the
      // model on exactly that shape: acknowledged once, then silent for the
      // rest. The stall watchdog cannot help, because it only arms after the
      // model produces text and a pure tool turn produces none.
      let secondHoldTimer = null;
      ...
      /**
       * A second, different acknowledgement for a turn still grinding through
       * tool rounds. Fires once, only while the caller is hearing nothing and
       * the model has still produced no text — so a turn that answers promptly
       * never reaches it, and it can never talk over the first line.
       */
      function armSecondHold() {
        const delay = secondHoldMs();
        if (secondHoldTimer || !delay) return;
        secondHoldTimer = setTimeout(() => {
          secondHoldTimer = null;
          if (state.speakEpoch !== myEpoch) return;
          if (producedText) return;              // the answer is already coming
          if (audioOut?.isPlaying()) return;     // never over the first line
          playHoldLine("stillWorking");
        }, delay);
        secondHoldTimer.unref?.();
      }
        ...
        if (ev.type === "delta") {
          ...
          if (engineFillerEnabled() && holdKindForTool(ev.name) && !toolHoldTimer && !holdLinePlayed) {
            ...
            toolHoldTimer = setTimeout(() => {
              toolHoldTimer = null;
              if (state.speakEpoch !== myEpoch) return;
              // Same predicate as the `slow` branch above — see shouldPlayHoldLine.
              if (
                !shouldPlayHoldLine({
                  audioPlaying: !!audioOut?.isPlaying(),
                  holdLinePlayed,
