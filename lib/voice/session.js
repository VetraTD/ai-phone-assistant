...
export function handleVoiceSessionConnection(ws, _req) {
  ...
  async function playFiller() {
    ...
    try {
      ...
      const cached = utteranceCache.get(googleVoice, "filler", FILLER_TEXT);
      const buf = cached || fillerBuf || (fillerBuf = await googleTts.synthesizeMulaw(FILLER_TEXT, googleVoice, callSid));
