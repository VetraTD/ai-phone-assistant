...
// The engine, not the model, announces a slow tool round.
//
// The prompt used to require the model to say "One moment while I check that
// for you" in the same response as a lookup call. It obeyed the speech half and
// dropped the call half on three consecutive turns of a live call, each time
// leaving the caller in silence. Emitted here it cannot be a false promise:
// it fires only because a tool actually started.
const VOICE_ENGINE_FILLER = process.env.VOICE_ENGINE_FILLER !== "false";
...
const TOOL_HOLD_DELAY_MS = (() => {
  ...
  return Number.isFinite(v) && v >= 0 && v <= 5_000 ? v : 600;
})();
    ...
    try {
      ...
      for await (const ev of gen) {
        ...
        if (ev.type === "delta") {
          ...
          if (VOICE_ENGINE_FILLER && !toolHoldTimer && !holdLinePlayed) {
