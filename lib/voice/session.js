...
// micro-utterances (fillers/nudges/goodbye) play in the Google fallback
// voice, not the business's chosen ElevenLabs voice — but that mismatch
// already exists today for playFiller's on-demand synth, so this isn't a new
// inconsistency, just a broader application of the same existing choice.
// ---------------------------------------------------------------------------
...
/**
 * Fixed micro-utterances worth pre-warming for a call's voice: the greeting
 * (business-specific), the slow-LLM filler, and the silence-nudge/goodbye
 * lines that don't depend on which step the call is later in. (Stage-2
 * nudge text varies by step/intent — see buildSilenceNudge — so only the
 * step-only variants are warmed here; a nudge for a less common
 * intent-specific phrasing simply falls through to live synthesis, same as
 * before this feature.)
 */
function buildUtteranceWarmEntries(config) {
  const entries = [
    { kind: "greeting", text: buildGreeting(config) },
    { kind: "filler", text: FILLER_TEXT },
    { kind: "checking", text: "Let me check that for you…" },
    { kind: "ack", text: "Sorry, go ahead." },
  ...
  ws.on("message", async (raw) => {
    ...
    switch (msg.event) {
      ...
      case "start": {
        ...
        // Cache check MUST happen before warm() is kicked off below: a hit
        // here can only ever be from a PREVIOUS call's warm(), never this
        // call's own (which hasn't started yet) — greeting audio must never
        // block pickup waiting on synthesis of any kind.
        speakTextCacheable(buildGreeting(config), "greeting-done");

        // ---- Background: warm this call's voice's micro-utterance cache
        // (greeting/filler/nudges/goodbye) for later in this call and for
        // whichever call comes next — never awaited, never blocks pickup.
        utteranceCache
