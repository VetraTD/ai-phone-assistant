...
import { getStrings, holdKindForTool } from "./strings.js";
...
function buildUtteranceWarmEntries(config) {
  ...
  const entries = [
    ...
    { kind: "holdAvailability", text: S.holdAvailability },
    { kind: "holdLookup", text: S.holdLookup },
    { kind: "holdWrite", text: S.holdWrite },
...
export function handleVoiceSessionConnection(ws, _req, { now = () => performance.now() } = {}) {
  ...
  async function playHoldLine(kind = "filler") {
    ...
    try {
      ...
      if (!cached) holdBufs.set(kind, buf);
