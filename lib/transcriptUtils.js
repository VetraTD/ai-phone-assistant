...
const QUESTION_STARTERS =
  /\b(what|when|where|which|who|how|why|can i|may i|could you|can you|would you|do you|are you|is there)\b/gi;

export function asksMoreThanOneThing(text) {
  for (const sentence of String(text || "").split(/(?<=[.!?])\s+/)) {
    if (!sentence.includes("?")) continue;
    const starters = (sentence.match(QUESTION_STARTERS) || []).length;
    if (starters >= 2) return true;
    if (/,\s*and\b/i.test(sentence) || /\balso\b/i.test(sentence)) return true;
    if (starters >= 1 && /\s+and\s+/i.test(sentence)) return true;
  }
  return false;
}
