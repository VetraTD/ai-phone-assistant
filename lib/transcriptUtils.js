 ...
 * Recommended 800 once the simulator shows the cutoff rate moving — NOT 2000:
 * the comment on TRAILING_CONJUNCTION records a live call where a longer hold
 * bought nothing and cost 6.7s voice-to-voice.
 */
function holdTrailingMs() {
  const v = Number.parseInt(process.env.VOICE_HOLD_TRAILING_MS, 10);
  return Number.isFinite(v) && v >= 0 && v <= 3_000 ? v : 0;
}
