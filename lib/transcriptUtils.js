...
// ---------------------------------------------------------------------------
// WIDENED 2026-09-14, against 90 real caller turns rather than by invention.
//
// CA3972ee6a lost a booking to this pattern. The assistant asked "Would you
// like me to book that for you?", the caller said "I go for it.", and
// callerAgreed came back FALSE. `go ahead`, `do it` and `book it` were all
// already here; `go for it` was not, and the caller hung up with no row.
//
// WHY IT MATTERED THAT DAY AND NOT BEFORE. A miss here used to be absorbed: the
// write returned a turn later on the silent-turn token path and committed on
// whatever agreement existed anywhere on the call. The consent latch made that
// token action-aware -- a booking can no longer ride a cancellation's consent --
// so a missed yes is now a lost booking instead of a wrong-basis one. Closing
// the hole made the vocabulary load-bearing.
//
// EVERY ADDITION IS EITHER THE OBSERVED MISS OR A NEAR-NEIGHBOUR OF AN ENTRY
// ALREADY PRESENT. No invented phrasings: `do it` gains `do that`, `book it`
// gains `book that`, `sounds good` gains great/perfect, and `right` gains the
// single-word `alright` that \b never matched inside.
//
// CHECKED BEFORE IT WAS WRITTEN, which is the part that makes it safe. Run over
// all 90 distinct caller turns from the 2026-09-13/14 calls, exactly ONE
// verdict changed: "I go for it." Every withdrawal and amendment still reads
// false. A false negative costs one turn, bounded by the refusal ceiling; a
// false positive writes a row nobody agreed to, so the direction of the risk is
// why the delta was inspected one line at a time.
const AFFIRMATIVE_RE =
  /\b(?:yes|yeah|yep|yup|yah|sure|correct|right|alright|okay|ok|absolutely|definitely|exactly|perfect|great|please\s+do|go\s+ahead|go\s+for\s+it|sounds?\s+(?:good|great|perfect)|that'?s\s+(?:right|correct|it|fine)|(?:that|this)\s+works|works\s+for\s+me|do\s+(?:it|that)|book\s+(?:it|that)|confirm(?:ed)?|mm-hmm|uh-huh|s[í]|claro|vale|correcto|perfecto|adelante|de\s+acuerdo|por\s+supuesto|est[aá]\s+bien|me\s+parece\s+bien|h[aá]galo|res[eé]rvelo)\b/i;
...
//
// "NO WORRIES" AND "NO PROBLEM" ARE NOT WITHDRAWALS. CAb4427e, 2026-09-18:
//
//   00:42:47  A: "Shall I reschedule your appointment to Monday, September
//                 twenty-first at eleven-thirty AM?"
//   00:42:51  caller: "Yeah, no worries."   -> agreed_now=FALSE, write refused
//
// The bare word `no` matched inside an idiom that means the opposite, on a
// sentence that opens with "Yeah". There is no reading of that turn in which
// the caller took anything back. It is the only case in the corpus where a
// caller agreed in plain English and the gate refused anyway -- every other
// refusal was a genuine non-answer, a restated time, or a transcript that had
// lost the word.
//
// Scoped as a lookahead on `no` alone, so every withdrawal this list exists for
// is untouched: "yeah, no -- sorry, I meant Tuesday" still reads false on the
// dash, on `sorry`, and on nothing to do with this.
const NEGATION_RE =
  /\b(?:no(?!\s+(?:worries|problems?|probs))|nope|nah|not|don'?t|do\s+not|wait|hold\s+on|hang\s+on|actually|instead|change|different|wrong|cancel\s+that|never\s*mind|sorry|nunca|espere|un\s+momento|en\s+realidad|mejor|cambie|cambiar|diferente|equivocad[oa])\b/i;
...
/**
 * Does this turn take back or amend whatever was just read back?
 *
 * The SAME list `isAffirmative` vetoes on, exported so a caller that recognises
 * agreement by some other route can still be overruled by it. LVX144 needs
 * exactly that: a caller who answers "Shall I book ten a.m.?" by saying "ten
 * a.m." has agreed, and one who says "no, ten a.m. is no good" has named the
 * same slot and has not.
 *
 * Deliberately NOT `!isAffirmative(text)`. That is false for "hello", which
 * withdraws nothing, and a gate reading it as a withdrawal would refuse every
 * turn it simply did not understand.
 *
 * @param {unknown} text
 * @returns {boolean}
 */
export function withdrawsAgreement(text) {
  if (!text || typeof text !== "string") return false;
...
// ---------------------------------------------------------------------------
// THE SAME QUESTION ASKED OF THE ASSISTANT, WHERE THE ANSWER MEANS SOMETHING
// ELSE ENTIRELY.
//
// Three calls in eighteen have had the assistant answer an English caller in
// another language: CAd48d7b in Spanish, CAfcd28789 reported as French, and
// CAdc602f in Spanish again -- "Sí, es completamente normal. ¿En qué puedo
// ayudarte hoy?" on a session with language_code en-US and
// output_language_pinned true. Nothing anywhere watched for it, which is why it
// was reported three times by a human instead of once by a counter.
//
// WHY SPANISH IS INCLUDED HERE AND EXCLUDED ABOVE. The caller-side list leaves
// Spanish out on purpose, and its reasoning is sound: Spanish is a supported
// locale, so "Sí, quiero una cita para el martes" is a legitimate caller rather
// than a transcription fault. The assistant side inverts that exactly. On a
// tenant whose languages_spoken is ["en"] there is no such thing as a
// legitimate Spanish assistant turn -- every one is a defect. Same machinery,
// opposite inclusion rule, because "legitimate" means opposite things on the
// two sides of the call.
//
// The caller has to be told apart from a fault. The assistant does not.
//
// ACCENT-BLIND, and the reason is narrower than it first looks. It was
// originally justified here by the log pipeline, which replaces non-ASCII with
// "?" so that the stored form of the turn above is "S?, es completamente
// normal. ?En qu? puedo ayudarte hoy?". That justification is WRONG: the
// mangled form is caught by its unaccented markers -- "es", "en", "puedo",
// "hoy" -- and would be caught with no normalisation at all. A sabotage row
// staying green is what exposed the claim.
//
// What stripping actually buys is the marker that exists ONLY in an accented
// form. "Você prefere amanhã ou quinta feira" carries exactly one marker,
// "Você", and every unaccented word in it is a non-marker; several of the
// inherited French, German and Portuguese markers are accented the same way.
// Both sides are normalised so those match. That case, and not the log
// pipeline, is what tests/liveAssistantLanguage.test.js pins.
//
// COUNT ONLY. Nothing is refused, no audio is cut, and no reply is suppressed.
// LVX21 is what cutting a caller's audio on a heuristic cost.
// ---------------------------------------------------------------------------

/** Diacritics removed, so "qué" and a log's mangled "qu?" reduce alike. */
const stripDiacritics = (s) => s.normalize("NFD").replace(/\p{Diacritic}/gu, "");

/**
 * The caller-side markers plus Spanish, all accent-stripped to match.
 *
 * Short words are safe here BECAUSE the English-function-word guard runs first:
 * "el", "la" and "de" appear in English text only alongside words like "the",
 * "at" or "is", which veto the check before these are ever consulted. A turn
 * reaches this set only if it contains no English function word at all.
 */
const REPLY_NON_EN_MARKERS = new Set(
  [
    ...NON_EN_MARKERS,
    // Spanish, the language all three observed incidents actually used.
    "si", "es", "en", "el", "la", "los", "las", "un", "una", "de", "del", "por",
    "con", "para", "como", "puedo", "ayudarte", "hoy", "dia", "hora", "cita",
    "gracias", "favor", "esta", "estoy", "tiene", "hablo", "idioma", "espanol",
    "ingles", "completamente", "tu", "te", "lo", "prefieres", "continuar",
  ].map(stripDiacritics)
);

/**
 * Did the ASSISTANT answer in something other than English?
 *
 * Only meaningful on an English-only tenant; the caller decides that, not this
 * function. See the block above for why its marker set differs from
 * looksNonEnglish's.
 *
 * Scored before shipping against every assistant turn on disk: 166 real English
 * turns, zero false positives, and both known foreign turns caught in their
 * accented and log-mangled forms.
 *
 * @param {unknown} text
 * @returns {boolean}
 */
export function replyLooksNonEnglish(text) {
  if (!text || typeof text !== "string") return false;
  const words = stripDiacritics(text)
    ...
    .filter(Boolean);
  if (words.length < MIN_WORDS_FOR_LANGUAGE_CHECK) return false;
  // One English function word is enough to settle it. A mixed turn is therefore
  // NOT counted -- deliberately, because "Your appointment is at La Quinta" is
  // English and must never read as a fault.
  if (words.some((w) => EN_FUNCTION_WORDS.has(w))) return false;
  return words.some((w) => REPLY_NON_EN_MARKERS.has(w));
}
