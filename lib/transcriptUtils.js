...
//
// Every word here is one that CANNOT end a caller's sentence. Words that can
// are deliberately absent, and the exclusions are the load-bearing part:
//   know  — "I don't know."
//   have  — "Yes I have."
...
//
// Keep this list TIGHT. Every false positive is dead air on a finished turn,
// and onHoldExpired can extend it further. Check
// holdRules.trailing_incomplete in /api/debug/latency before adding to it.
const TRAILING_INCOMPLETE =
  /\b(a|an|the|my|your|our|their|its|this|these|those|be|been|being|book|booking|schedule|scheduling|reschedule|rescheduling|cancel|canceling|cancelling|get|getting|make|making|take|taking|need|needing|want|wanting|having|bring|bringing|un|una|el|la|los|las|mi|su|reservar|hacer|cancelar|cambiar)\s*$/i;
 ...
 *
 * Default 0 -> 800 on 2026-08-31. The recommendation to move it once the
 * simulator showed the cutoff rate moving has been sitting here unacted-on
 * while the branch stayed dead in production; sim/cutoffSim.sim.js now reports
 * the matched pair it was waiting for. Same script, same pauses, same
 * endpointing, the flag the only difference:
 *
...
export function classifyHold(text, rawText = text) {
  ...
  if (trailingMs > 0 && TRAILING_INCOMPLETE.test(t)) {
