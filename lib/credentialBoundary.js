...
export const FORBIDDEN_IN_PHI_PROJECTS = Object.freeze([
  ...
  Object.freeze({
    vendor: "Deepgram",
    // Added when Google STT v2 landed, and deliberately absent before that.
    // While the Deepgram BAA was an unanswered email, a rule either way
    // encoded a guess: failing the deploy would have blocked on someone else's
    // inbox, and an explicit allow would have looked like a decision.
    //
    // It is no longer a guess. The US lane transcribes with Google STT v2,
    // secrets.tf lists deepgram-api-key as `lanes = ["uk"]`, and a Deepgram
    // credential in a PHI project is the same class of finding as an
    // ElevenLabs one. A late "yes" from Deepgram would change what the UK lane
    // MAY use and nothing about what a US project may HOLD.
    squashed: ["deepgram"],
    tokenPairs: [],
    why:
      "No BAA. The US lane transcribes with Google Speech-to-Text v2; a Deepgram credential in " +
      "a PHI project also refuses to boot (checkCoveredVendors), so this is the deploy-time half " +
      "of a guard that would otherwise only fire after the service was already built.",
  }),
  Object.freeze({
