...
export function createTextSession({
  ...
  async function sendTurn(userText) {
    ...
    // Same placement as lib/voice/session.js startTurn: BEFORE the extras are
    // assembled, because the prompt block and the write gate are both decided
    // from them. Reading it after the reply made every signal a turn late.
    ...
    const turnExtras = {
      ...
      callerContext,
      // Same reason: the spelling state is tracked in the reducer and read by
      // buildDynamicTail, so it has to be threaded here or the eval suite
