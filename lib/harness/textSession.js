...
import { applyReplyState, spellingSettled } from "../voice/replyState.js";
...
export function createTextSession({
  ...
  // The live pipeline prefetches this once at call start (session.js
  // fetchCallerContext) and then refreshes it in-process as the call books or
  // cancels things. Here the scenario supplies the starting value and the same
  // refresh applies, so KNOWN CALLER FACTS and CALLER CONTEXT agree in eval the
  // way they do live. `let`, not `const`: setCallerContext below reassigns it.
  ...
  function dispatchEffects(effectsList) {
    return dispatchCapabilityEffects(effectsList, {
      ...
      // Mirrors lib/voice/session.js. Present here so an eval scenario exercises
      // the REAL refresh path: a scenario that books and then asks for a second
      // appointment is only meaningful if the caller snapshot moves the way it
      // does live.
  ...
  async function sendTurn(userText) {
    ...
    // Mirrors lib/voice/session.js's buildExtras: capabilityState must be
    // threaded every turn, or a pack's callerFacts (and any other cross-turn
    // scratchpad) written on turn N is invisible to buildDynamicTail on turn
    // N+1 — the whole KNOWN CALLER FACTS mechanism goes inert for every eval
    // scenario and the CLI even though it works live.
    const turnExtras = {
      ...extras,
      capabilityDeps: fakes.deps,
      capabilityState: state.capabilityState,
      // The live-refreshed value, not the one the scenario started with —
      // otherwise a booking made on turn 1 is invisible to turn 2 and the
      // scenario proves nothing about the guard.
      callerContext,
      // Same reason: the spelling state is tracked in applyReplyState and read
      // by buildDynamicTail, so it has to be threaded here or the eval suite
      // measures a gate that is never actually in the prompt.
    ...
    if (!reply) {
      ...
      return {
        ...
        // No done reply → no usage/finishReason telemetry for this turn.
        usage: null,
        finishReason: null,
        notes: [],
      };
    ...
    const { capabilityNotes } = applyReplyState(
      ...
      { userText, reply },
      {
        ...
        // Both drivers pass this or the eval suite silently measures different
        // behaviour from the live call — the trap the comment in
        // applyReplyState (2b) records.
        strings: getStrings(config),
      }
    );
    ...
    return {
      ...
      state: { step: state.step, intent: state.intent },
      timings: { firstEventMs, totalMs, slowCount },
      // Additive truncation telemetry off the done reply (Task 11 / plan 2.5).
