...
import { applyReplyState, hasSpentSpellingAsk } from "../voice/replyState.js";
...
export function createTextSession({
  ...
  async function sendTurn(userText) {
    ...
    const turnExtras = {
      ...
      callerContext,
      // Same reason: the spelling cap is counted in applyReplyState and read
      // by buildDynamicTail, so it has to be threaded here or the eval suite
      // measures a cap that is never actually in the prompt.
    ...
    const { capabilityNotes } = applyReplyState(
      state,
      { userText, reply },
      { STEPS, mergeCapabilityState, dispatchEffects, spellRequestRe: getStrings(config).spellRequestRe }
    );
