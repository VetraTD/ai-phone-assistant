 ...
 * Shapes mirror the real implementations in services/db.js,
...
/** Digits-only, last 10, mirroring db.js's phone match. */
function phoneKey(v) {
 ...
 * length apart (services/db.js countScheduledOverlapping / the internal
...
export function makeFakeDeps({
  ...
  const deps = {
    // Ledger O25. Returns a synthetic id so the pack's success path runs; a
    // harness that returned null would exercise only the write-failed branch.
    async recordSmsConsent(args) {
      record("recordSmsConsent", args);
      return nextId("consent");
    },
    ...
    async listAppointmentsByCaller(businessId, opts = {}) {
      ...
      // upcomingOnly was ignored entirely here, which meant no eval scenario
      // could reproduce — or verify a fix for — the live bug where the
      // assistant found appointments whose date had already passed and offered
      // to book "in addition to your existing ones". A fake that silently drops
      // an option makes the behaviour it gates untestable.
      const now = Date.now();
      return appointments
        .filter((r) => r.business_id === businessId && r.status === "scheduled")
        .filter((r) => {
    ...
    // THE null/[] DISTINCTION IS LOAD-BEARING and must survive into the fake.
    // lib/postCallVerify.js treats `null` as "the read failed" and `[]` as "the
    // model claimed a booking that does not exist" -- a database outage versus
    // an accusation. Most of services/db.js flattens those into one value;
    // listAppointmentsByCallId deliberately does not, so a fake that returned
    // [] for a missing tenant would make an outage indistinguishable from a
    // fabrication and quietly green the wrong branch.
    async listAppointmentsByCallId(callId, businessId) {
      record("listAppointmentsByCallId", { callId, businessId });
      if (!businessId || !callId) return null;
      return appointments
        .filter((r) => r.business_id === businessId && r.call_id === callId)
        .sort((a, b) => Date.parse(a.scheduled_at) - Date.parse(b.scheduled_at))
        .map(publicRow);
    },
...
export function makeFakeEffectsDeps() {
  ...
  const notifications = {
    ...
    // Ledger O25: the sms_consent pack's deferred effect calls this after a
    // grant commits, to release a text sendCallerSms held while consent was
    // missing. Recorded rather than performed, like everything else here.
    async releaseHeldCallerSms(businessId, callerNumber) {
      recordNotify("releaseHeldCallerSms", { businessId, callerNumber });
      return 0;
    },
  };
