...
export const NON_COVERED_VENDORS = Object.freeze({
  ...
  // Brevo is GONE — both call sites moved to SMTP. The entry stays as a
  // TRIPWIRE rather than being deleted with it: if a BREVO_API_KEY ever
  // reappears in a `hipaa` process, that is a vendor coming back without a
  // decision, and the boot check should say so. It costs one object and it
  // fails closed.
  //
  // Kept honest: there is no covered equivalent to name, because after A1.3
  // there is no PHI in email at all. The reason to refuse it is that it is an
  // unreviewed third party in a covered stack, not that it would see patients.
  brevo: {
    label: "Brevo (removed — SMTP replaced it)",
    credentials: ["BREVO_API_KEY"],
    covered: "SMTP, via the credentials this stack already has",
  },
  // The Gemini Developer API (AI Studio) is NOT a Google Cloud service, and the
  // Google Cloud BAA covers Google Cloud services. Vertex AI is one; AI Studio
  // is not. An uncovered LLM call carries the caller's entire utterance, which
  // makes this the largest single disclosure in the stack if it is got wrong.
  //
  // GEMINI_API_KEY is deliberately NOT listed as a credential to refuse at
  // boot. Unlike ElevenLabs, the same variable is what the eval harness and
  // every local run use, and the boot check's job is to catch a credential
  // that reached a project it does not belong in — not to stop a developer
  // running the suite. services/gemini.js getClient() refuses at construction,
  // which is where it matters.
  "gemini-developer-api": {
    label: "the Gemini Developer API (AI Studio)",
    credentials: [],
    covered: "Vertex AI — set VERTEX_ENABLED=true with GOOGLE_CLOUD_PROJECT and VERTEX_LOCATION",
  },
});
