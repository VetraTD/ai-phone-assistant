...
// ONE LINE HERE IS SPECULATIVE, and it is marked as such rather than deleted:
// `confirmation_code` probes something the product does not currently say. It
// was written on the assumption that a receptionist reads codes back, and
// checking afterwards showed nothing generates one. It stays because the check
// is cheap if that ever changes — and because a fixture that permanently flags
// for an unresolvable reason is exactly the noise this harness exists to avoid,
// so it says so in its own `why`.
...
export const READBACK_LINES = [
  ...
  },
  {
    ...
    why:
      "SPECULATIVE — the product does not speak codes today. Nothing generates a " +
      "reference or confirmation code; appointmentId is a uuid used for identity " +
      "checks and never read aloud. Kept for if that changes. NOTE the readback " +
      "cannot settle this one: four different formattings (as written, spaced, " +
      "comma-grouped, dashed) all came back empty or truncated, and C5b already " +
      "showed the recogniser fails on spelled-out sequences — so a flag here says " +
      "nothing about whether a HUMAN could understand it.",
  },
  {
    label: "insurance",
    category: "alphanumeric",
