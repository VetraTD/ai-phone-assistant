...
/**
 * Did the caller actually say this name during the call?
 *
 * ANY token of two letters or more is enough, not the whole string, and the
 * looseness is deliberate in the same direction normalizeName's is: this
 * decides whether to ASK a question. A false match costs one un-asked question;
 * a false miss re-interrogates a returning caller about a name the business has
 * had right for months, which is the repetition the on-file bypass exists to
 * prevent. Surnames come back from transcription imperfectly far more often
 * than first names, so demanding the full name back would fire constantly.
 *
 * @param {string} name
 * @param {string} said - everything the caller has been transcribed saying
 * @returns {boolean}
 */
export function nameSpokenIn(name, said) {
  const haystack = normalizeName(said);
  if (!haystack) return false;
  const tokens = normalizeName(name).split(" ").filter((t) => t.length >= 2);
  if (tokens.length === 0) return false;
  return tokens.some((t) => new RegExp(`(?:^| )${t}(?: |$)`).test(haystack));
}
 ...
 *
 * Revised 2026-08-31. "Ask once" turned out to mean "ask once, then write
 * whatever we heard" — the gate reopened on the assistant having SPOKEN a
 * question, not on the caller having answered one. The name-on-file skip and
 * the once-per-call spirit are unchanged; what changed is that the call now
 * has to hear letters, a refusal, or a couple of unanswered attempts before
 * the question is considered closed.
 *
 * @param {object} opts
 * @param {*} opts.name - the name as the model captured it
 * @param {object|null} [opts.callerContext] - the call-start caller snapshot
 * @param {boolean} [opts.spellingSettled] - is the spelling question closed for
 *   this call: the caller spelled it, declined to, or used up their attempts.
 *   Replaces the old `spellingAlreadyAsked`, which closed as soon as the
 *   ASSISTANT had asked — so a caller who ignored the question had their
 *   mis-heard name written down. See lib/voice/replyState.js spellingSettled().
...
export function shouldConfirmSpelling({
  ...
  policy = "always",
  callerSaidThisCall = null,
} = {}) {
  if (policy === "off") return false;
  if (spellingSettled) return false;
  ...
  const onFile = (callerContext?.upcomingAppointments || [])
    ...
    .filter(Boolean);
  if (onFile.includes(normalized)) {
    // ...but only when the name came from the CALLER on this call.
    //
    // LVX53, observed 2026-09-03: the caller's name never transcribed
    // intelligibly at all -- the turn came through as "Hay en el Tíndala" --
    // and the assistant replied "Thanks, Nithin Vodla", took the name off an
    // existing row, and booked under it. This bypass is why nothing asked: the
    // string was on file, so the record was treated as the spelling.
    //
    // The reasoning above is right for a caller who names themselves and is
    // recognised, and wrong when the name did not come from the caller at all.
    // LVX28 wrote this down before it was seen: "'already on file' trusts a row
    // that may itself never have been spelled... every later call treats that
    // row as authority and never asks again." The database now holds two rows
    // reading "Nithin Vodla", the second written on a call where nobody said it.
    //
    // A record can confirm a spelling. It cannot supply the fact that this
    // caller identified themselves.
    //
    // ABSENT transcript means the OLD behaviour, exactly. The cascade threads
    // no call transcript, so it keeps the bypass unchanged -- the same
    // construction the hesitation gate uses to stay byte-identical for tier 3.
    if (callerSaidThisCall == null) return false;
    if (nameSpokenIn(name, callerSaidThisCall)) {
      bumpCounter("write_name_provenance_ok");
      return false;
    }
    bumpCounter("write_refused_name_provenance");
    return true;
  }
