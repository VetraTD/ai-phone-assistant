...
export async function recoverOwedBooking(input, deps = {}) {
  const {
    ...
    agreed = false,
    // What actually ran on this call, built by the engine at teardown and
    // forwarded verbatim. Optional: absent renders "not recorded" in the
    // judge's prompt rather than a block of zeros, because a call where nobody
    // looked and a call where nothing happened are different answers.
    toolTraffic = null,
    mode = "off",
  } = input || {};
  ...
  // EVERY OUTCOME LOGGED, including the ordinary ones. Each early return below
  // bumps a counter and nothing else, and a counter is not readable per call --
  // so "the recovery ran and correctly stood down" and "the recovery never ran"
  // looked identical in production, and the only way to tell them apart on
  // CAc5070c2d was to notice that verifyCall is chained after this and fired.
  //
  // That is the same trap silent_turn_verdict was added to close for LVX117, and
  // it is the one this whole session has been about: an instrument that reads the
  // same on a clean run and on a run that never happened.
  const decline = (reason, extra = {}) => {
    log.info("recover_declined", { callSid, reason, ...extra });
    return { ran: false, reason, booked: false };
  };
  ...
  // Before the log line too: a call on a deployment where this is off must not
  // emit anything at all.
  if (mode !== "act") return { ran: false, reason: "not_act", booked: false };
  if (!businessId || !callId) return decline("no_tenant");
  ...
  if (existing == null) {
    ...
    return decline("row_read_failed");
  }
  ...
  if (existing.length > 0) {
    ...
    return decline("row_exists", { rows: existing.length });
  }
  ...
  if (agreed !== true) {
    ...
    return decline("never_agreed", { candidates: Array.isArray(slots) ? slots.length : 0 });
  }
  ...
  if (!Array.isArray(slots) || slots.length === 0) {
    ...
    return decline("no_candidate_slots");
  }
  ...
  // bookedRowCount stays 0 and that is CORRECT here, not an oversight: the
  // row-exists gate above already proved this call recorded nothing.
  ...
  if (!verdict?.ran) return decline("judge_did_not_run");
  if (verdict.agreedAction !== "book") {
    log.info("recover_declined", { callSid, reason: "nothing_owed", agreed_action: verdict.agreedAction });
    return { ran: true, reason: "nothing_owed", booked: false };
  }
