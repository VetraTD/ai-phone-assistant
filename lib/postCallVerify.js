...
// Live-only behind POSTCALL_VERIFY because the cascade serves a paying clinic
// and this path has never run.
//
    ...
    for (const { row, kind } of confirmable.values()) {
      ...
      try {
        await notifications.sendCallerSms(
          ...
          // is TCPA and HIPAA 164.522(b), both US and both written for the
          // paying clinic. The owner's decision on 2026-09-03 is that a
