...
export async function verifyCall(input, deps = {}) {
  ...
  // TWO ANSWERS, NOT ONE. LVX121.
  //
  // The verdict is a single headline and the verdicts are ordered, so a call
  // that BOTH abandoned a write AND told the caller something was done reports
  // as `write_abandoned` -- and the reconciliation below, which is gated on
  // `claim_without_row`, never runs. On 2026-09-11 that is exactly what
  // happened: the caller was told "your appointment is all set", no row was
  // written, and NOBODY WAS TOLD, because a more specific verdict won.
  //
  // A fabricated claim is a fact about the call whatever else went wrong on it.
  // It is now reported alongside the verdict rather than encoded in it.
  const { verdict, claimUnsatisfied } = reconcile({
    claims,
    writes,
    bookedRows,
    changedRows,
  ...
  // Gated on the FACT, not on the verdict. See LVX121 above: a caller who was
  // told a thing was done, when nothing did it, reaches a human whatever the
  // headline says about the rest of the call.
  if (claimUnsatisfied) {
    try {
  ...
  if (mode === "send" && !SUPPRESSES_CONFIRMATION.has(verdict)) {
    for (const { row, kind } of confirmable.values()) {
      ...
      //
      // The booking-time sender texts the number that RANG US; this one texts
      // the number on the row. They are usually the same and the message is
      // then a duplicate. When they differ, the caller booked for somebody else
      // -- the caller has been told and the client has not, so suppressing here
      // would silence the only message that was going to reach them.
...
function reconcile({ claims, writes, bookedRows, changedRows, changedIds = [], abandoned = [] }) {
  ...
  // COMPUTED FIRST AND RETURNED ALONGSIDE THE VERDICT. LVX121.
  //
  // Whether a claim went unbacked is a fact about the call, and it used to be
  // reachable only by the verdict coming back `claim_without_row`. The verdicts
  // are ordered and only one wins, so a call that also abandoned a write
  // reported `write_abandoned` and the caller-facing fabrication vanished from
  // the ledger -- and with it the escalation, which was gated on the verdict.
  const writtenTools = new Set(writes.map((w) => w?.tool).filter(Boolean));
  const wroteAnythingAtAll = bookedRows.length > 0 || changedRows.length > 0 || writes.length > 0;
  const claimUnsatisfied = claims.some((c) => {
    if (c?.kind !== "claim") return false;
    const satisfiedBy = Array.isArray(c.satisfiedBy) ? c.satisfiedBy.filter(Boolean) : [];
    if (satisfiedBy.length === 0) return !wroteAnythingAtAll;
    return !satisfiedBy.some((tool) => writtenTools.has(tool));
  });

  if (bookedEffect && bookedRows.length === 0) return { verdict: "row_mismatch", claimUnsatisfied };
  ...
  if (changedRows.length < changedIds.length) return { verdict: "row_mismatch", claimUnsatisfied };
  ...
  if (abandoned.length > 0) return { verdict: "write_abandoned", claimUnsatisfied };
  ...
  // invisible to tier 3.
  if (claimUnsatisfied) return { verdict: "claim_without_row", claimUnsatisfied };

  if (!claimed && wroteAnything) return { verdict: "row_without_claim", claimUnsatisfied };
  return { verdict: "ok", claimUnsatisfied };
}
