...
export async function verifyCall(input, deps = {}) {
  const {
    ...
    // failed or timed out. Undefined is the common case and must leave this
    // module behaving exactly as it did before the judge was consulted.
    judgeClaimedDone = undefined,
  ...
  // -------------------------------------------------------------------------
  // THE CANDIDATE THIS RULE MISSES, COUNTED RATHER THAN ACTED ON.
  //
  // CA03558d, 2026-09-17: the caller agreed to a specific time, the model tried
  // to book it six times, every attempt was refused, and the line read
  //
  //   verdict=write_abandoned  booked_rows=0  booking_owed=FALSE  claims=2
  //   point_verified_slots=0   verified_slots=15
  //
  // Fifteen listed slots, no point check, so `bookingWasOwed` read false on a
  // call that had plainly converged. The obvious change is to accept a LISTED
  // slot here -- and it is not made, because the rule above states its reason
  // and this evidence does not overturn it: "a caller shown a list of times and
  // agreeing to something, with no point check behind it, is not a booking that
  // was owed, it is a conversation." A caller who browses and drifts off looks
  // identical on listed slots alone.
  //
  // That call is escalated anyway, by the abandoned-write branch below, which
  // is a far stronger signal: the system TRIED to write and could not. So
  // widening this one would buy only the case where a caller agreed and no
  // write was ever attempted -- which is precisely the browsing shape.
  //
  // So it is a counter, and a future round can read the rate instead of
  // guessing at it. If listed-only-owed fires often on calls that also show a
  // claim or an abandoned write, the discriminator is doing nothing and can be
  // widened on evidence.
  const listedOnlyOwed =
    Boolean(bookingOwed?.agreed) &&
    !bookingWasOwed &&
    Number(bookingOwed?.verified) > 0 &&
    bookedRows.length === 0;
  if (listedOnlyOwed) bumpCounter("postcall_booking_owed_listed_only");
  if (bookingWasOwed) bumpCounter("postcall_booking_owed");
  if (bookingOwedNoRow) bumpCounter("postcall_booking_owed_no_row");

  const claimedByMatcher = claims.some((c) => c?.kind === "claim");
  const claimedByJudge = judgeClaimedDone === true;
  ...
  // The population this change added, countable on its own: the matcher said no
  // and the judge said yes. If it never fires, consulting the judge bought
  // nothing; if it fires often, the matcher is further behind than three calls
  // suggested.
  if (claimedByJudge && !claimedByMatcher) bumpCounter("postcall_claim_judge_only");
  ...
  // -------------------------------------------------------------------------
  // AN ABANDONED WRITE WITH NO ROW IS ALSO SOMETHING A HUMAN HAS TO SEE.
  //
  // CA03558d again, and this is the second half of why nobody was told about
  // it. The call abandoned `book_appointment` -- six attempts, every one
  // refused, nothing completed -- and `reconcile()` returns ONE ordered
  // verdict, in which `write_abandoned` is tested BEFORE `claim_without_row`.
  // So the call that told a caller "Yes, I have confirmed that your appointment
  // is booked" produced verdict=write_abandoned, the escalation was gated on
  // claim_without_row, and no customer_requests row and no notification
  // followed.
  //
  // The comment on bookingWasOwed above already records this trap once, in
  // 259de15's words: a headline competing for one slot is not a fact about the
  // call. The fix there was to compute booking-owed separately. This is the
  // same fix applied to the other half -- an abandoned write is a fact too, and
  // it does not stop being one because a different verdict won the ordering.
  //
  // -------------------------------------------------------------------------
  // AND `bookedRows.length === 0` WAS THE WRONG NARROWING. LVX142, 2026-09-18.
  //
  // It was justified as "a call that abandoned an attempt and then succeeded
  // has nothing outstanding". That reading is already guaranteed one level up:
  // `abandoned` is built in lib/voice/live/index.js as
  //
  //     [...refusedToolsThisCall].filter((n) => !completedToolsThisCall.has(n))
  //
  // so a tool that later succeeded is excluded BY NAME before this line ever
  // sees it. The conjunct therefore added nothing in the direction it claimed,
  // and suppressed in the direction that matters.
  //
  // CAd978554, production:
  //
  //     verdict=write_abandoned  abandoned=["reschedule_appointment_db"]
  //     booked_rows=1  changed_rows=1  claims=2  sent=0  skipped=[]
  //
  // The abandoned write was a RESCHEDULE. The row in `bookedRows` came from an
  // unrelated `book_appointment` four minutes later. A caller was told their
  // appointment had been moved, it had not been, the fiction then took their
  // consent to cancel an appointment that did not exist -- and nobody was told,
  // because a different action had happened to succeed on the same call.
  //
  // That is precisely the trap reconcile() names for itself at the bottom of
  // this file: "An unrelated success must not vouch for an abandoned write."
  // The escalation gate re-introduced it two hundred lines above the comment.
  //
  // The counter-example is in the corpus too: CAb4427e, the same night, same
  // verdict, `booked_rows: 0` -- and it DID escalate. `postcall_claim_reconciled`
  // is in its log. So this had been silent once, not twice.
  //
  // THE OLD CONDITION IS KEPT AS A COUNTER. Its series stays comparable with
  // every call taken since 3407ac0, and the gap between the two names exactly
  // what this widening added.
  // -------------------------------------------------------------------------
  const abandonedOutstanding = verdict === "write_abandoned";
  const abandonedWithNoRow = abandonedOutstanding && bookedRows.length === 0;
  if (abandonedWithNoRow) bumpCounter("postcall_abandoned_no_row");
  if (abandonedOutstanding) bumpCounter("postcall_abandoned_outstanding");

  if (verdict === "claim_without_row" || bookingOwedNoRow || abandonedOutstanding) {
    try {
      const requestId = await db.withTenantSafe(
        ...
        () =>
          db.createCustomerRequest({
            ...
            // Three reasons, three sentences. A person triaging these needs to
            // know whether the caller AGREED to something (ring them, they are
            // expecting an appointment) or whether the assistant merely said so
            // (check, then decide) -- and the third is new: the call tried to
            // write and every attempt was refused, which is the one shape where
            // the caller's intent is certain and the system's failure is total.
            notes: bookingOwedNoRow
              ...
              : // REWORDED WITH THE WIDENING, because the old sentence was only
                // true of the narrow case. "every attempt was refused, so
                // nothing was recorded" describes CA03558d, where zero rows
                // existed. It is FALSE of CAd978554, where the reschedule was
                // abandoned and a booking landed four minutes later -- and a
                // triage note that overstates is one the next person learns to
                // distrust.
                abandonedOutstanding
                ? "Automated check: this call tried to book or change an appointment and the " +
                  "attempt was refused, so that change was never recorded. Other parts of the " +
                  "call may have gone through. Please listen to this call and contact the caller."
  ...
  if (mode === "send" && !SUPPRESSES_CONFIRMATION.has(verdict)) {
    for (const { row, kind } of confirmable.values()) {
      ...
      try {
        // LVX122: the return value is whether the message was handed to Twilio.
        // This array used to be pushed unconditionally, so `sent: 2` on a
        // postcall_verify line meant two sends were ATTEMPTED -- and on
        // CA6b773e2a the caller had declined, the consent gate blocked all
        // three, and the line still read sent=2.
        const delivered = await notifications.sendCallerSms(
          config,
          to,
          kind,
          {
        ...
        if (delivered) {
          ...
          // Not a fault: the tenant may have SMS off, or the caller may have
          // declined. It is simply not a send, and the line must not say it was.
          skipped.push({ appointmentId: row.id, reason: "not_sent" });
          bumpCounter("postcall_confirm_not_sent");
        }
      } catch (err) {
  ...
  // -------------------------------------------------------------------------
  // THE ONE MESSAGE THAT GOES TO THE NUMBER THAT RANG US.
  //
  // A NARROW EXCEPTION to the rule on `callerNumber` above, taken on the owner's
  // decision and scoped to the single case that cannot satisfy it: a booking the
  // call owed and never wrote has NO ROW, so there is no client_phone to read.
  //
  // The original rule guards against texting whoever happened to ring a
  // CONFIRMATION quoting somebody else's appointment. This message quotes no
  // appointment and no person -- {business} is its entire vocabulary, enforced in
  // lib/smsConsent.js -- and it goes to a number that just finished speaking to
  // us. Every other send keeps the rule.
  //
  // NOT gated on SUPPRESSES_CONFIRMATION, and that is deliberate: those verdicts
  // withhold a message that PROMISES A ROW IS RIGHT. This one promises the
  // opposite. `write_abandoned` in particular is a call that owed a write and
  // abandoned it, which is precisely when the caller most needs telling.
  //
  // It cannot double up with a confirmation: bookingOwedNoRow requires
  // bookedRows.length === 0, so there is nothing for the loop above to have sent.
  if (mode === "send" && bookingOwedNoRow) {
    if (!isValidE164(callerNumber)) {
      skipped.push({ appointmentId: null, reason: "pending_no_number" });
      bumpCounter("postcall_pending_skipped_no_number");
    } else {
      try {
        const delivered = await notifications.sendCallerSms(
          config,
          ...
          { business: config.businessName },
          // Transactional for the same reason the confirmations are, by the same
          // owner decision of 2026-09-03: this is a service message about a call
          // the caller themselves placed minutes ago. An explicit decline still
          // blocks it, inside sendCallerSms.
          { transactional: true }
        );
        if (delivered) {
          sent.push({ appointmentId: null, kind: "appointment_request_pending" });
          bumpCounter("postcall_pending_sent");
        } else {
          skipped.push({ appointmentId: null, reason: "pending_not_sent" });
          bumpCounter("postcall_pending_not_sent");
        }
      } catch (err) {
        skipped.push({ appointmentId: null, reason: "pending_send_failed" });
        bumpCounter("postcall_pending_failed");
        log.error("postcall_pending_failed", { callSid, reason: err?.message, severity: "warn" });
      }
    }
  }
  ...
  log.info("postcall_verify", {
    ...
    // BOTH READERS ON THE LINE, so a disagreement is visible on the call it
    // happened on rather than reconstructed by reasoning backwards from the
    // verdict. That is the mistake made with write_consent_restated_slot, which
    // shipped as a bare counter and could not be read on the call that fired it.
    claimed_by_matcher: claimedByMatcher,
    claimed_by_judge: claimedByJudge,
...
function reconcile({
  ...
  // -------------------------------------------------------------------------
  // LVX147. TWO READERS, AND THE SECOND ONE NOW COUNTS.
  //
  // `claims` is filled by a phrase matcher. It lost three times on three
  // consecutive production calls and the judge was right on all three:
  //
  ...
  // the next call -- and LVX143 measured why: the WIDE variant, built precisely
  // to catch what the narrow one misses, fires on the same 26 of 207 corpus
  // turns and has never once caught anything extra.
  //
  // A UNION, NEVER A REPLACEMENT, and that is the whole safety argument. The
  // judge can ADD a claim and can never remove one, so a judge that is down,
  // slow, or wrong in the quiet direction leaves exactly the behaviour this
  // module had before it was consulted. Shipping on an LLM's answer is only
  // defensible while that holds, so it is asserted in tests rather than
  // described here.
  // -------------------------------------------------------------------------
  const claimed = claims.some((c) => c?.kind === "claim") || judgeClaimedDone === true;
