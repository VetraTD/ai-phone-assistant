...
export async function verifyCall(input, deps = {}) {
  ...
  //
  // `bookedRows.length === 0` keeps it narrow: a call that abandoned an attempt
  // and then succeeded has nothing outstanding. And readFailed has already
  // returned above, so zero here means the database genuinely holds nothing
  // rather than that we could not look.
  ...
  if (verdict === "claim_without_row" || bookingOwedNoRow || abandonedWithNoRow) {
    try {
        ...
        () =>
          db.createCustomerRequest({
            ...
            notes: bookingOwedNoRow
              ...
              : abandonedWithNoRow
                ? "Automated check: this call tried to book or change an appointment and every " +
                  "attempt was refused, so nothing was recorded. Please listen to this call and " +
                  "contact the caller."
