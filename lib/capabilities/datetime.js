...
/**
 * A bare calendar date, "YYYY-MM-DD", with no time at all.
 *
 * Deliberately its own shape rather than an optional tail on NAIVE_DATETIME_RE:
 * a date is a DIFFERENT question ("what is open that day?") from a datetime
 * ("is this open?"), and conflating them is what let a caller who said only
 * "next Tuesday" have a time invented for them.
 */
export const NAIVE_DATE_RE = /^(\d{4})-(\d{2})-(\d{2})$/;

/**
 * Parse a bare "YYYY-MM-DD" into numeric components at midnight, rejecting
 * calendar-impossible dates the same way parseNaiveDateTime does.
 * @param {string} str
 * @returns {{year:number,month:number,day:number,hour:number,minute:number,second:number}|null}
 */
export function parseNaiveDate(str) {
  const m = NAIVE_DATE_RE.exec(str);
  if (!m) return null;
  return parseNaiveDateTime(`${m[1]}-${m[2]}-${m[3]}T00:00:00`);
}
...
/**
 * "today" / "tomorrow" for an instant, in the BUSINESS's timezone — or null
 * when neither word applies.
 *
 * The prompt already carries the current date and an explicit instruction to
 * calculate from it, and on 2026-09-03 at 21:37 on a Thursday the assistant
 * still told a caller their Friday appointment was "today". A model that has
 * been told the date and asked to do the arithmetic is a model that can get
 * the arithmetic wrong; this does it once, on the server, so there is nothing
 * left to compute. Callers that get null are told to name the weekday and the
 * date instead. See LVX61.
 *
 * Compared as CALENDAR DATES in the target zone, not as a 24-hour offset: an
 * appointment 25 hours away can still be tomorrow, one 23 hours away can be the
 * day after, and a DST boundary makes "add 86,400,000" wrong twice a year.
 *
 * @param {string|number|Date|null|undefined} instant
 * @param {string|undefined} timeZone - business timezone; DEFAULT_TIMEZONE when unset
 * @param {number} [now] - epoch ms, injectable for tests
 * @returns {"today"|"tomorrow"|null}
 */
export function relativeDayLabel(instant, timeZone, now = Date.now()) {
  // toLocalNaiveDateTime takes a string or a Date and runs everything else
  // through Date.parse, which returns NaN for an epoch NUMBER. Normalise here
  // rather than widening that helper: it is on the hot path for every spoken
  // datetime and its contract is pinned by tests elsewhere.
  const asDate = (v) => (typeof v === "number" && Number.isFinite(v) ? new Date(v) : v);
  const targetDay = toLocalNaiveDateTime(asDate(instant), timeZone).slice(0, 10);
  const todayDay = toLocalNaiveDateTime(asDate(now), timeZone).slice(0, 10);
  if (!targetDay || !todayDay) return null;
  if (targetDay === todayDay) return "today";

  // Calendar arithmetic on the local date, via UTC so the rollover is pure
  // day-counting with no zone of its own: 2026-08-31 + 1 -> 2026-09-01.
  const [y, m, d] = todayDay.split("-").map(Number);
  const tomorrowDay = new Date(Date.UTC(y, m - 1, d + 1)).toISOString().slice(0, 10);
  return targetDay === tomorrowDay ? "tomorrow" : null;
}
