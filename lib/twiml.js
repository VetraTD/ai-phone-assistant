 ...
 * @param {number} [timeoutSeconds] - Optional seconds to wait for input before re-POST (e.g. 10)
 * @param {string} [typingSoundUrl] - Optional URL of a short (1-2s) audio to play before the reply (e.g. typing sound)
 * @returns {string} TwiML XML string
 */
export function buildSayGatherRedirect(voiceUrl, sayText, timeoutSeconds, typingSoundUrl) {
  const timeoutAttr = timeoutSeconds != null ? ` timeout="${timeoutSeconds}"` : "";
  const playTag =
 ...
 * @returns {string} TwiML XML string
 */
export function buildSayAndDial(sayText, phoneNumber) {
  return (
    `<Response>` +
    `<Say voice="Polly.Joanna">${escapeXml(sayText)}</Say>` +
