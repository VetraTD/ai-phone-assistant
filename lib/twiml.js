...
export function buildGatherAndRedirect(voiceUrl, promptText, timeoutSeconds, opts) {
  ...
  return (
    `<Response><Gather input="speech" action="${voiceUrl}" method="POST" language="${lang}" speechTimeout="auto" speechModel="phone_call" enhanced="true"${timeoutAttr}>` +
    sayTag +
    `</Gather>` +
...
export function buildSayGatherRedirect(voiceUrl, sayText, timeoutSeconds, typingSoundUrl, opts) {
  ...
  const playTag =
    typingSoundUrl && typeof typingSoundUrl === "string" && typingSoundUrl.trim()
      ...
      : "";
  const speechVerb = opts?.audioUrl
    ? `<Play>${escapeXml(opts.audioUrl)}</Play>`
    : `<Say voice="${voice}">${buildSayContent(sayText)}</Say>`;

  if (bargeIn) {
    // Caller can interrupt: speech verb is INSIDE Gather
    return (
      `<Response>` +
      playTag +
      `<Gather input="speech" action="${voiceUrl}" method="POST" language="${lang}" speechTimeout="auto" speechModel="phone_call" enhanced="true"${timeoutAttr}>` +
      speechVerb +
      `</Gather>` +
      `<Redirect method="POST">${voiceUrl}</Redirect></Response>`
    );
  ...
  // Default: speech verb is OUTSIDE Gather (AI finishes before listening)
  return (
    `<Response>` +
    playTag +
    speechVerb +
...
export function buildSayAndHangup(sayText, opts) {
  ...
  const speechVerb = opts?.audioUrl
    ? `<Play>${escapeXml(opts.audioUrl)}</Play>`
    : `<Say voice="${voice}">${buildSayContent(sayText)}</Say>`;
  return `<Response>${speechVerb}<Hangup/></Response>`;
}
...
/**
 * Build TwiML for multiple sequential <Play> verbs then Gather + Redirect.
 * Used when a reply has been pre-synthesized as per-sentence audio chunks.
 * All audio plays before the Gather listens (bargeIn=false behaviour).
 * @param {string} voiceUrl - Full URL for action and redirect
 * @param {string[]} audioUrls - Ordered list of MP3 URLs to play
 * @param {string|string[]} [lang] - Language code(s) for speech recognition
 * @returns {string} TwiML XML string
 */
export function buildMultiPlayGatherRedirect(voiceUrl, audioUrls, lang) {
  const langStr = resolveLanguage(lang);
  const playTags = audioUrls.map((url) => `<Play>${escapeXml(url)}</Play>`).join("");
  return (
    `<Response>` +
    playTags +
    `<Gather input="speech" action="${voiceUrl}" method="POST" language="${langStr}" speechTimeout="auto" speechModel="phone_call" enhanced="true">` +
    `</Gather>` +
    `<Redirect method="POST">${voiceUrl}</Redirect></Response>`
  );
}
...
/**
 * Build TwiML for multiple sequential <Play> verbs then Hangup.
 * Used for goodbye messages pre-synthesized as per-sentence audio chunks.
 * @param {string[]} audioUrls - Ordered list of MP3 URLs to play
 * @returns {string} TwiML XML string
 */
export function buildMultiPlayAndHangup(audioUrls) {
  const playTags = audioUrls.map((url) => `<Play>${escapeXml(url)}</Play>`).join("");
  return `<Response>${playTags}<Hangup/></Response>`;
}
