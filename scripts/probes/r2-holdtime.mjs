...
async function geminiHold(armName, label, idx) {
  ...
  const c = price(VERTEX_MODEL, state.usage);
  commit({ probe: "R2", arm: `hold:gemini:${armName}:${label}`, run: idx + 1, model: VERTEX_MODEL, location: VERTEX_LOCATION, usage: state.usage, usd: c.usd });
  row.usd = c.usd;
  return row;
}
...
async function main() {
  ...
  writeRaw("r2-holdtime", { max_hold_ms: MAX_HOLD_MS, n: N, rows, at: new Date().toISOString() });
